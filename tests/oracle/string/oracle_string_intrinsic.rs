...
const GRAPH_CASES: &[(&str, &str)] = &[
    ...
    (
        ...
        r#"(function(){
            var selected=["Number","Boolean","String","Math","Reflect","Symbol","globalThis","BigInt"];
...
fn string_intrinsic_graph_matches_pinned_quickjs() {
    compare_eval_completion_cases_with_prelude(CASE_PRELUDE, "String intrinsic graph", GRAPH_CASES);
}
...
fn string_call_and_construct_match_pinned_quickjs() {
    compare_eval_completion_cases_with_prelude(CASE_PRELUDE, "String calls", CALL_CASES);
    compare_eval_completion_cases_with_prelude(
        CASE_PRELUDE,
        "String construction",
        CONSTRUCT_CASES,
...
fn string_static_values_and_conversion_order_match_pinned_quickjs() {
    compare_eval_completion_cases_with_prelude(
        CASE_PRELUDE,
        "String.fromCharCode",
        FROM_CHAR_CODE_CASES,
    );
    compare_eval_completion_cases_with_prelude(
        CASE_PRELUDE,
        "String.fromCodePoint",
        FROM_CODE_POINT_CASES,
    );
    compare_eval_completion_cases_with_prelude(CASE_PRELUDE, "String.raw", RAW_CASES);
}
...
fn string_intrinsic_exotic_surfaces_match_pinned_quickjs() {
    compare_eval_completion_cases_with_prelude(
        CASE_PRELUDE,
        "String intrinsic exotic surfaces",
        EXOTIC_CASES,
    );
}
