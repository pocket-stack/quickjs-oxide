...
pub(crate) fn observe_completion(oracle: &OsStr, source: &str, description: &str) -> String {
    observe_completion_output(oracle, source, description)
        .trim_end()
        .to_owned()
}

pub(crate) fn observe_completion_output(oracle: &OsStr, source: &str, description: &str) -> String {
    run_stdin_utf8(
        oracle,
        COMPLETION_OBSERVER,
        Some(STDIN_SENTINEL),
...
pub(crate) fn eval_std_lines(oracle: &OsStr, source: &str, description: &str) -> Vec<String> {
    run_stdin_utf8(
        oracle,
        STD_LINES_EVALUATOR,
        None,
