...
- Source `MemberExpression` lowering follows QuickJS's typed
  ...
  UTF-16/search methods, the `substring`/`substr`/`slice` subrange trio, the
...
- The realm root set reserves five typed primitive `class_proto` slots. Number,
  ...
  `substr` and `slice` in the pinned table-relative order ahead of the
  ...
  reuse plus wide-to-Latin-1 compression. Together with `length`, the conversion
  pair, `Symbol.iterator` and the `constructor`
  back-reference, this twenty-key list is only the QuickJS-relative order
  ...
  validation.
  The remaining 33 String-prototype own keys, Context-level observable
...
publish the remaining 33 prototype own keys, Context/C pointer embedding
...
remaining 33 entries of String's 53-key prototype surface, the RegExp object
...
index-search pair, regexp-aware includes family and subrange trio live in
...
lines to 9,886 lines. Realm-aware property completion wrappers and storage
