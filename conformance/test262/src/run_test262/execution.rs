...
impl AgentRunGuard {
    fn new(enabled: bool) -> Self {
        Self {
            session: enabled.then(|| {
                Test262AgentSession::new(|| {
                    Runtime::new_with_host_services(
...
fn authenticate_dynamic_import_bytecode(
    runtime: &Runtime,
    function: &quickjs_oxide::engine::api::FunctionBytecodeRef,
...
fn authenticate_dynamic_import_module_graph(
    runtime: &Runtime,
    module: &quickjs_oxide::engine::api::ModuleBytecodeRef,
    policy: Option<DynamicImportRootPolicy>,
    path: &Path,
) -> Result<(), String> {
    let contains_dynamic_import = runtime
...
fn own_diagnostic_string(
    runtime: &Runtime,
    object: &ObjectRef,
    key: &quickjs_oxide::engine::api::PropertyKey,
...
mod tests {
    ...
    fn admissions_path() -> PathBuf {
        PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/../.."))
            .join("dev-support/test262/admissions.tsv")
    }
    ...
    impl ModuleLoader for SingleDependencyLoader {
        fn load(
            ...
            quickjs_oxide::engine::api::ModuleLoadResult,
            quickjs_oxide::engine::api::ModuleLoaderError,
        > {
    ...
    impl ModuleLoader for VerifyDfsLoader {
        ...
        > {
            let name = String::from_utf16(&normalized_name.utf16_units().collect::<Vec<_>>())
                .map_err(|_| {
                    quickjs_oxide::engine::api::ModuleLoaderError::new("invalid test module name")
                })?;
            match name.as_str() {
                "test/verify-dfs-a_FIXTURE.js" => {
                    Ok(quickjs_oxide::engine::api::ModuleLoadResult::SourceText(
                        "import './verify-dfs.js'; import('./verify-dfs-b_FIXTURE.js');".to_owned(),
                    ))
                }
                "test/verify-dfs-b_FIXTURE.js" => {
                    Ok(quickjs_oxide::engine::api::ModuleLoadResult::SourceText(
                        "import './verify-dfs.js';".to_owned(),
                    ))
                }
                _ => Err(quickjs_oxide::engine::api::ModuleLoaderError::new(format!(
    ...
    ) -> quickjs_oxide::engine::api::ModuleBytecodeRef {
    ...
    fn exact_test262_loader_selects_json_text_only_for_type_json() {
        ...
        assert!(
            loader
                .check_attributes(&mut context, json.effective().unwrap())
                .is_ok()
        );
        for attributes in [&absent, &empty, &javascript, &wrong_key] {
            assert_eq!(
                loader.load(&mut context, &name, attributes).unwrap(),
                ModuleLoadResult::SourceText(JSON_SOURCE.to_owned())
            );
        }
        assert_eq!(
            loader.load(&mut context, &name, &json).unwrap(),
...
"#,
    ...
    fn dependency_free_request_shaped_module_negatives_stop_during_parse() {
        let suite = PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/../.."))
