...
// payload bits, so it is reflexive.
...
pub trait ModuleLoader: fmt::Debug {
    /// Normalize the request before cache lookup. Names are ECMAScript
    /// strings; filesystem hosts may encode them with JsString::try_to_wtf8_bytes.
    /// The runtime removes text after an embedded NUL before invoking hooks.
    fn normalize(
        &self,
    ...
    /// Check attributes before normalization, cache lookup or subsequent
    /// source parsing. Static requests call this for nonempty effective
    /// attributes; dynamic imports also call it for an explicitly empty object.
    /// The initiating Context exposes the parse-in-progress module and may
    /// be used for synchronous nested compilation.
    fn check_attributes(
        &self,
        _context: &mut Context,
    ...
    /// Load a cache-missing module. Return source text, exact source bytes,
    /// JSON text, or a module compiled through the initiating Context.
    /// Attributes preserve the distinction between absent and explicitly empty.
    fn load(
        &self,
        context: &mut Context,
    ...
    fn check(&mut self, attributes: &[ModuleImportAttribute]) -> Result<(), ModuleCompileFailure> {
        ...
        let outcome = match self
            ...
            .invoke_module_host_callback(&mut self.context, |context| {
                loader.check_attributes(context, attributes)
            }) {
            Ok(outcome) => outcome,
            Err(error) => {
                self.runtime_failure = Some(error);
...
impl Runtime {
    ...
    pub(crate) fn check_dynamic_import_attributes(
        &self,
        realm: ContextId,
            ...
            while let Some(frame) = stack.last() {
                    ...
                    if let Some(loader) = loader {
                        ...
                        let outcome = self
                            .invoke_module_host_callback(&mut context, |context| {
                                loader.normalize(context, &base_name, &specifier)
                            })?;
                ...
                let dependency = if let Some(cached) = cached {
                    ...
                    let loaded = {
                        ...
                        let outcome = self
                            .invoke_module_host_callback(&mut context, |context| {
                                loader.load(context, &normalized_name, attributes)
                            })?;
                        match self.propagate_module_host_throw(outcome)? {
                            Ok(loaded) => loaded,
                            Err(error) => {
    ...
    pub(crate) fn resolve_dynamic_import_module(
        &self,
        realm: ContextId,
        base_name: &JsString,
        specifier: &JsString,
    ...
    pub(crate) fn get_module_namespace_raw(
        &self,
        module: RawModuleRef,
        realm: ContextId,
    ) -> Result<ObjectRef, RuntimeError> {
    ...
    pub(crate) fn call_dynamic_import_handler(
        &self,
        realm: ContextId,
