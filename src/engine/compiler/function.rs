    ...
    pub(super) fn parse_function_definition_header(
        ...
        if async_function {
            ...
            if self.current().line_terminator_before
                || !matches!(
                    self.current().kind,
                    TokenKind::Keyword(crate::engine::compiler::lexer::Keyword::Function)
                )
            {
        ...
        let name = match self.current().kind.clone() {
            ...
            TokenKind::Keyword(crate::engine::compiler::lexer::Keyword::Yield)
            ...
            TokenKind::Keyword(crate::engine::compiler::lexer::Keyword::Await)
    ...
    pub(super) fn parse_function_definition_tail(
        ...
        if private_name_binding
            ...
            && (!header.parent_context.generator
                ...
                || identifier.keyword_hint != Some(crate::engine::compiler::lexer::Keyword::Yield))
        {
        ...
        if private_name_binding
            ...
            && (!header.parent_context.async_function
                ...
                || identifier.has_escape
                || identifier.keyword_hint != Some(crate::engine::compiler::lexer::Keyword::Await))
        {
            return Err(Error::syntax(
