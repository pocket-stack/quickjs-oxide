...
impl<'a> Lexer<'a> {
    ...
    pub fn with_source_text(source: &'a SourceText, options: LexerOptions) -> Self {
        Self {
    ...
    pub fn source_range_to_js_string(
        &self,
        range: Range<usize>,
...
pub fn quickjs_simple_lookahead_has_line_terminator(mut source: &str) -> bool {
    loop {
...
pub fn quickjs_simple_lookahead_is_of(mut source: &str) -> bool {
    loop {
        let Some(ch) = source.chars().next() else {
            return false;
        };
