...
#[cfg_attr(
    ...
    expect(
        dead_code,
        reason = "The verifier and VM accept instruction forms that current compiler paths do not emit."
    )
)]
...
#[cfg(test)]
impl<T: TestConstant> DetachedBytecode<T> {
    #[must_use]
    pub fn constant(&self, index: u32) -> Option<&T> {
    ...
    for (pc, instruction) in code.iter().enumerate() {
        match instruction {
            Instruction::PushAtomValueIndex(index)
                if *index > crate::engine::atom::ATOM_MAX_INT =>
            {
                return Err(Error::internal(
...
#[cfg(test)]
pub trait TestConstant {
    fn is_string(&self) -> bool;
}
#[cfg(test)]
impl TestConstant for Value {
    fn is_string(&self) -> bool {
...
#[cfg(test)]
impl<T> Default for DetachedBytecode<T> {
    fn default() -> Self {
        Self {
            code: Vec::new(),
            constants: Vec::new(),
...
mod tests {
    use super::{
        ApplyKind, ArgumentsKind, DefineMethodKind, DetachedBytecode, DynamicEnvironmentSource,
        EvalVariableSource, Instruction, MAX_LOCAL_SLOTS, WithObjectSource,
    };
    use crate::engine::api::JsString;
    use crate::engine::value::PrimitiveValue as Value;
    ...
    fn verifier_models_generator_suspension_resume_stack_shapes() {
        ...
        let underflow = DetachedBytecode::<Value> {
    ...
    fn verifier_models_dynamic_import_as_two_inputs_to_one_promise() {
        ...
        let undersized = DetachedBytecode::<Value> {
            max_stack: 1,
            ..function
        };
        assert_eq!(
    ...
    fn verifier_models_typed_class_definition_operations() {
        let function = DetachedBytecode::<Value> {
            ...
            constants: vec![Value::String(crate::engine::api::JsString::from_static(
                "C",
            ))],
            local_count: 0,
            max_stack: 2,
        ...
        let invalid_name = DetachedBytecode::<Value> {
        ...
        let heritage = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::PushI32(7),
                Instruction::DefineClass {
    ...
    fn verifier_models_arguments_creation_as_one_fresh_value() {
        ...
        let undersized_rest = DetachedBytecode::<Value> {
            max_stack: 0,
            ..rest
        };
        assert_eq!(
    ...
    fn verifier_models_membership_operators_as_binary_booleans() {
        for operator in [Instruction::InstanceOf, Instruction::In] {
            let valid = DetachedBytecode::<Value> {
                code: vec![
                    Instruction::PushI32(1),
                    Instruction::PushI32(2),
                    operator.clone(),
    ...
    fn verifier_models_eval_variable_object_stack_and_operands() {
        ...
        for instruction in [
            ...
            let bad_name = DetachedBytecode::<Value> {
        ...
        let bad_local = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Return,
                Instruction::HasEvalVariable { source, name: 0 },
    ...
    fn verifier_models_typed_dynamic_environment_stack_and_operands() {
        ...
        let undersized_global_reference = DetachedBytecode::<Value> {
        ...
        for instruction in [
            ...
            let malformed = DetachedBytecode::<Value> {
                code: vec![Instruction::Undefined, Instruction::Return, instruction],
                constants: vec![Value::Int(0)],
                local_count: 2,
                max_stack: 1,
    ...
    fn verifier_accepts_closed_non_terminating_control_flow() {
        let cycle = DetachedBytecode::<Value> {
        ...
        let reachable_fallthrough = DetachedBytecode::<Value> {
    ...
    fn verifier_tracks_catch_markers_and_exceptional_edges() {
        ...
        let inconsistent_handlers = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushTrue,
                Instruction::IfFalse(4),
                Instruction::Catch(5),
        ...
        let marker_exceeds_declared_stack = DetachedBytecode::<Value> {
            max_stack: 0,
            ..normal
        };
    ...
    fn verifier_tracks_typed_gosub_return_addresses() {
        ...
        let abrupt_cleanup = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(9),
                Instruction::Gosub(4),
                Instruction::Return,
        ...
        let nip_catch_return_address = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Catch(6),
                Instruction::Undefined,
                Instruction::Gosub(4),
        ...
        for code in [
            ...
            let forged = DetachedBytecode::<Value> {
                code,
                constants: vec![],
                local_count: 0,
                max_stack: 1,
        ...
        let return_address_exceeds_declared_stack = DetachedBytecode::<Value> {
            max_stack: 1,
            ..returning
        };
    ...
    fn verifier_dynamic_cleanup_discards_intermediate_gosub_return_addresses() {
        ...
        let nip_catch = DetachedBytecode::<Value> {
            code: vec![
        ...
        let iterator_close = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
    ...
    fn verifier_tracks_for_of_records_offsets_and_dynamic_close() {
        ...
        let offset = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::ForOfStart,
                Instruction::PushI32(2),
        ...
        let preserve = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::ForOfStart,
                Instruction::PushI32(42),
        ...
        let drop_preserve = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::ForOfStart,
                Instruction::PushI32(42),
    ...
    fn verifier_tracks_for_await_records_phases_and_dynamic_detach() {
        ...
        for start in [Instruction::ForOfStart, Instruction::ForAwaitOfStart] {
            let detach = DetachedBytecode::<Value> {
                code: vec![
                    Instruction::PushI32(1),
                    start,
                    Instruction::PushI32(42),
    ...
    fn verifier_rejects_malformed_for_await_region_protocols() {
        ...
        let pending_close = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::ForAwaitOfStart,
                Instruction::ForAwaitOfNext,
    ...
    fn verifier_rejects_for_await_kind_and_phase_join_mismatches() {
        let kind_mismatch = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushTrue,
                Instruction::IfFalse(5),
                Instruction::PushI32(1),
        ...
        let phase_mismatch = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::ForAwaitOfStart,
                Instruction::PushTrue,
    ...
    fn verifier_orders_iterator_catch_and_gosub_regions() {
        ...
        let iterator_inside_catch = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Catch(7),
                Instruction::PushI32(1),
                Instruction::ForOfStart,
        ...
        let catch_inside_iterator = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::ForOfStart,
                Instruction::Catch(7),
        ...
        let iterator_above_outer_gosub = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Gosub(5),
                Instruction::Return,
        ...
        let gosub_above_iterator = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::ForOfStart,
                Instruction::Gosub(6),
    ...
    fn verifier_rejects_iterator_marker_and_return_address_misuse() {
        ...
        let preserve_return_address = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::ForOfStart,
                Instruction::Gosub(6),
        ...
        let inconsistent_regions = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushTrue,
                Instruction::IfFalse(5),
                Instruction::PushI32(1),
    ...
    fn verifier_rejects_bad_constants_and_stack_joins() {
        let bad_constant = DetachedBytecode::<Value> {
        ...
        let bad_atom_value_index = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Return,
        ...
        let runtime_regexp_constant = DetachedBytecode::<Value> {
        ...
        let bad_join = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushTrue,
                Instruction::IfFalse(4),
                Instruction::PushI32(1),
        ...
        let excessive_declared_stack = DetachedBytecode::<Value> {
        ...
        let valid_nip = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::PushI32(2),
                Instruction::Nip,
        ...
        let nip_underflow = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::Nip,
                Instruction::Return,
        ...
        let postfix_readonly = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::PostInc,
    ...
    fn verifier_rejects_malformed_operands_even_in_unreachable_code() {
        ...
        let bad_set_name = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Return,
        ...
        let non_string_set_name = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
        ...
        let non_string_read_only_name = DetachedBytecode::<Value> {
        ...
        let non_string_field = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
        ...
        let non_string_keep_field = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
        ...
        let non_string_put_field = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Undefined,
        ...
        let bad_jump = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Return,
                Instruction::Goto(99),
        ...
        for instruction in [Instruction::Catch(99), Instruction::Gosub(99)] {
            let bad_handler_target = DetachedBytecode::<Value> {
                code: vec![Instruction::Undefined, Instruction::Return, instruction],
                constants: vec![],
                local_count: 0,
                max_stack: 1,
    ...
    fn verifier_enforces_the_declared_quickjs_local_frame() {
        ...
        let missing_frame = DetachedBytecode::<Value> {
            local_count: 0,
            ..valid.clone()
        };
        assert_eq!(
        ...
        let unreachable = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Return,
        ...
        let last_slot = DetachedBytecode::<Value> {
            code: vec![
        ...
        let excessive_frame = DetachedBytecode::<Value> {
            code: vec![Instruction::Undefined, Instruction::Return],
            constants: vec![],
            local_count: u16::MAX,
            max_stack: 1,
    ...
    fn verifier_models_quickjs_lexical_frame_operations() {
        let initialized_then_updated = DetachedBytecode::<Value> {
            code: vec![
        ...
        let checked_var_ref = DetachedBytecode::<Value> {
            code: vec![
    ...
    fn verifier_models_typed_derived_constructor_operations() {
        let default_derived = DetachedBytecode::<Value> {
            code: vec![
        ...
        let captured_initializer = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::InitializeDerivedVarRef(0),
                Instruction::Undefined,
        ...
        let return_underflow = DetachedBytecode::<Value> {
            code: vec![Instruction::ReturnDerived(0)],
            local_count: 1,
            max_stack: 0,
            ..DetachedBytecode::<Value>::default()
        };
        assert_eq!(
        ...
        let terminal = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::ReturnDerived(0),
                Instruction::PushI32(1),
    ...
    fn verifier_protects_typed_super_call_pairs_across_control_flow() {
        let conditional_argument = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Undefined,
                Instruction::MarkSuperCall,
        ...
        let unmarked = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Undefined,
                Instruction::ConstructSuper(0),
        ...
        let wrong_arity = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Undefined,
                Instruction::MarkSuperCall,
        ...
        let generic_apply = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Undefined,
                Instruction::MarkSuperCall,
        ...
        let inconsistent_join = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Undefined,
                Instruction::PushTrue,
    ...
    fn verifier_models_array_literal_stack_contracts() {
        let fixed = DetachedBytecode::<Value> {
            code: vec![
                Instruction::PushI32(1),
                Instruction::PushI32(2),
            ...
            constants: vec![Value::String(crate::engine::value::JsString::from_static(
                "2",
            ))],
            local_count: 0,
            max_stack: 3,
        ...
        let dynamic = DetachedBytecode::<Value> {
            code: vec![
                Instruction::ArrayFrom(0),
                Instruction::PushI32(0),
                Instruction::Undefined,
        ...
        let trailing_hole = DetachedBytecode::<Value> {
            code: vec![
                Instruction::ArrayFrom(0),
                Instruction::PushI32(2),
                Instruction::Dup1,
    ...
    fn verifier_models_object_literal_stack_contracts() {
        let fixed_proto_spread = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Object,
                Instruction::PushI32(1),
                Instruction::DefineField(0),
            ...
            constants: vec![
                Value::String(crate::engine::value::JsString::from_static("field")),
                Value::String(crate::engine::value::JsString::from_static("method")),
            ],
            local_count: 0,
            max_stack: 2,
        };
        ...
        let computed = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Object,
                Instruction::PushI32(1),
    ...
    fn verifier_rejects_malformed_array_literal_operands() {
        ...
        let non_string_method = DetachedBytecode::<Value> {
            code: vec![
                Instruction::Undefined,
                Instruction::Undefined,
                Instruction::DefineMethod {
        ...
        let array_from_underflow = DetachedBytecode::<Value> {
