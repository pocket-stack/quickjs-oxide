...
pub const MODULE_DEFAULT_BINDING_NAME: &str = "<module-default>";

/// Unspellable compiler/runtime cell which caches one module's `import.meta`.
pub const MODULE_IMPORT_META_BINDING_NAME: &str = "<import.meta>";

/// Source-order index into [`UnlinkedModule::requested_modules`].
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct ModuleRequestIndex(pub u32);
...
pub struct ModuleRequest {
    ...
    pub attributes: ModuleImportAttributes,
}

/// One imported binding linked to a root closure slot.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ModuleImport {
    pub request: ModuleRequestIndex,
    pub import_name: ModuleImportName,
    pub closure_index: u16,
}
...
pub enum ModuleLinkInitializerValue {
    Undefined,
    Function {
        constant: u32,
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ModuleLinkInitializer {
    pub closure_index: u16,
    pub value: ModuleLinkInitializerValue,
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ModuleImportCollision {
    pub closure_index: u16,
    pub declaration: ModuleImportCollisionDeclaration,
}

/// Declaration flavor sharing an import's effective closure slot.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ModuleImportCollisionDeclaration {
    Var,
    Lexical,
    Function,
}
...
pub struct ModuleExport {
    pub export_name: JsString,
    pub target: ModuleExportTarget,
}

/// One `export * from` edge.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ModuleStarExport {
    pub request: ModuleRequestIndex,
}
...
pub struct UnlinkedModuleTables {
    pub declaration_order: Vec<u16>,
    pub link_initializers: Vec<ModuleLinkInitializer>,
    pub import_collisions: Vec<ModuleImportCollision>,
    pub requested_modules: Vec<ModuleRequest>,
    pub imports: Vec<ModuleImport>,
    pub exports: Vec<ModuleExport>,
    pub star_exports: Vec<ModuleStarExport>,
}
...
pub struct UnlinkedModuleParts {
    ...
    pub import_collisions: Box<[ModuleImportCollision]>,
    pub requested_modules: Box<[ModuleRequest]>,
    pub imports: Box<[ModuleImport]>,
    pub exports: Box<[ModuleExport]>,
    pub star_exports: Box<[ModuleStarExport]>,
}
...
impl UnlinkedModule {
    ...
    #[must_use]
    pub fn into_parts(self) -> UnlinkedModuleParts {
        UnlinkedModuleParts {
            name: self.name,
