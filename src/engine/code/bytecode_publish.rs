...
use crate::engine::code::function::{UnlinkedVariableDefinition, metadata::EvalEnvironment};
...
use crate::engine::code::function::metadata::{
    ...
    ConstructorKind, EvalBinding, EvalCallerProfile, EvalCallerVariableTarget, EvalKind,
    EvalRootBinding, EvalScope, FunctionKind,
};
...
fn explicit_control_flow_target(
    instruction: &crate::engine::code::bytecode::Instruction,
) -> Option<usize> {
    let target = match instruction {
        crate::engine::code::bytecode::Instruction::Goto(target)
        | crate::engine::code::bytecode::Instruction::IfFalse(target)
        | crate::engine::code::bytecode::Instruction::IfTrue(target)
        | crate::engine::code::bytecode::Instruction::Catch(target)
        | crate::engine::code::bytecode::Instruction::Gosub(target) => *target,
        _ => return None,
    };
    ...
    )) = pending.pop()
    {
                    ...
                    RootPublication::Module(_) => match descriptor.source {
                        ...
                        ClosureSource::ModuleImportMeta => {
                            if descriptor.kind != ClosureVariableKind::Normal
                                || !descriptor.is_lexical
                                || !descriptor.is_const
                                || name != Some(&JsString::from_static(
                                    crate::engine::code::module::MODULE_IMPORT_META_BINDING_NAME,
                                ))
                            {
                                return Err(RuntimeError::Engine(Error::internal(
        ...
        for (pc, instruction) in function.code().iter().enumerate() {
            ...
            if let Some((source, name)) = match instruction {
                ...
                | crate::engine::code::bytecode::Instruction::DeleteDynamicBinding {
                    source,
                    name,
            ...
            match instruction {
                ...
                | crate::engine::code::bytecode::Instruction::PutVarRefCheck(index)
                | crate::engine::code::bytecode::Instruction::InitializeVarRef(index)
                | crate::engine::code::bytecode::Instruction::InitializeModuleImportCollision(
                    index,
                )
                | crate::engine::code::bytecode::Instruction::GetVar(index)
                | crate::engine::code::bytecode::Instruction::GetVarUndef(index)
...
mod tests {
    ...
    fn parameter_binding_pattern_metadata_authenticates_anonymous_entry_segments() {
        ...
        let named_slot =
            UnlinkedFunction::fixture(ordinary_code(), Vec::new(), ordinary_metadata())
                .with_fixture_definitions(
                    vec![UnlinkedVariableDefinition::ordinary(Some(
