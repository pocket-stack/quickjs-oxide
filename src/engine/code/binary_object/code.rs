...
pub(in crate::engine::code) struct CodeLimits {
    max_bytes: usize,
...
impl CodeLimits {
    ...
    #[must_use]
    pub(in crate::engine::code::binary_object) const fn intersect(
        self,
        max_bytes: usize,
        max_instructions: usize,
        max_atom_relocations: usize,
    ...
    pub(in crate::engine::code::binary_object) const fn limit(
        self,
    ...
    pub(in crate::engine::code::binary_object) fn check(
        self,
        kind: CodeResourceKind,
        requested: usize,
    ) -> Result<(), CodeError> {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::engine::code) enum CodeResourceKind {
    Bytes,
    Instructions,
    AtomRelocations,
}
...
pub(in crate::engine::code) enum CodeError {
    InvalidAtomMode {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::engine::code) struct InstructionSpan {
    offset: u32,
    opcode: PinnedOpcode,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::engine::code) struct AtomRelocation {
    operand_offset: u32,
    atom: BinaryAtom,
}
...
impl AtomRelocation {
    ...
    #[must_use]
    pub(in crate::engine::code) const fn atom(self) -> BinaryAtom {
        self.atom
    }
}
...
/// successful scan.
#[derive(Clone, Debug, Eq, PartialEq)]
pub(in crate::engine::code) struct CodeImage {
    bytes: Vec<u8>,
...
pub(in crate::engine::code::binary_object) struct CodeImageParts {
    pub(in crate::engine::code::binary_object) bytes: Vec<u8>,
    pub(in crate::engine::code::binary_object) instructions: Vec<InstructionSpan>,
    pub(in crate::engine::code::binary_object) atom_relocations: Vec<AtomRelocation>,
    pub(in crate::engine::code::binary_object) atom_space: AtomIndexSpace,
    pub(in crate::engine::code::binary_object) payload_offset: usize,
}
...
impl CodeImage {
    ...
    pub(in crate::engine::code) fn scan(
        payload: &[u8],
        atom_space: AtomIndexSpace,
        payload_offset: usize,
        limits: CodeLimits,
    ...
    #[must_use]
    pub(in crate::engine::code) fn instructions(&self) -> &[InstructionSpan] {
        &self.instructions
    }

    /// Return every semantic atom operand in instruction order.
    #[must_use]
    pub(in crate::engine::code) fn atom_relocations(&self) -> &[AtomRelocation] {
        &self.atom_relocations
    }

    /// Return the exact bytecode atom namespace used to scan the payload.
    #[must_use]
    pub(in crate::engine::code) const fn atom_space(&self) -> AtomIndexSpace {
        self.atom_space
    }

    /// Consume this scanner image for semantic whole-image relocation.
    pub(in crate::engine::code::binary_object) fn into_parts(self) -> CodeImageParts {
        CodeImageParts {
            bytes: self.bytes,
    ...
    pub(in crate::engine::code) fn canonical_bytes(&self) -> Result<Vec<u8>, CodeError> {
    ...
    fn every_pinned_catalog_entry_has_exact_framing() {
        ...
        for raw in 1..PINNED_OPCODE_COUNT {
            ...
            if opcode.has_atom_operand() {
                ...
                assert!(usize::from(opcode.size()) > ATOM_OPERAND_BYTES);
            } else {
