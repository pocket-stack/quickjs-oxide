...
pub(in crate::engine::code) const PINNED_OPCODE_COUNT: usize = 244;
...
pub(in crate::engine::code) enum OpcodeFormat {
    None,
    NoneInt,
    NoneLoc,
    NoneArg,
...
impl OpcodeFormat {
    ...
    #[must_use]
    pub(in crate::engine::code) const fn has_atom_operand(self) -> bool {
        self.atom_operand_offset().is_some()
    }

    /// Byte offset of the raw atom operand from the instruction start.
    #[must_use]
    pub(in crate::engine::code) const fn atom_operand_offset(self) -> Option<u8> {
        match self {
...
pub(in crate::engine::code) struct PinnedOpcode(u8);
...
impl PinnedOpcode {
    ...
    #[must_use]
    pub(in crate::engine::code) const fn info(self) -> &'static PinnedOpcodeInfo {
        &PINNED_OPCODE_INFO[self.0 as usize]
    }

    #[must_use]
    pub(in crate::engine::code) const fn name(self) -> &'static str {
        self.info().name()
    }

    #[must_use]
    pub(in crate::engine::code) const fn size(self) -> u8 {
        self.info().size()
    }

    #[must_use]
    pub(in crate::engine::code) const fn n_pop(self) -> u8 {
        self.info().n_pop()
    }

    #[must_use]
    pub(in crate::engine::code) const fn n_push(self) -> u8 {
        self.info().n_push()
    }

    #[must_use]
    pub(in crate::engine::code) const fn format(self) -> OpcodeFormat {
        self.info().format()
    }
...
pub(in crate::engine::code) struct PinnedOpcodeInfo {
    name: &'static str,
    size: u8,
    n_pop: u8,
    n_push: u8,
