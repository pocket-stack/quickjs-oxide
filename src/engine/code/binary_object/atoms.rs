...
use crate::engine::atom::{ATOM_MAX_INT, ATOM_MAX_TABLE_INDEX, ATOM_TAG_INT};
...
pub(in crate::engine::code) enum BinaryObjectMode {
...
pub(in crate::engine::code) struct HeaderAtomSlot(u32);

impl HeaderAtomSlot {
    /// Return the zero-based position in the binary-object header.
    #[must_use]
    pub(in crate::engine::code) const fn index(self) -> u32 {
        self.0
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::engine::code) enum BinaryAtom {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::engine::code) struct AtomIndexSpace {
...
impl AtomIndexSpace {
    ...
    #[must_use]
    pub(in crate::engine::code) const fn mode(self) -> BinaryObjectMode {
        self.mode
    }

    /// Return the first raw atom index backed by the image header.
    #[must_use]
    pub(in crate::engine::code) const fn first_atom(self) -> u32 {
        self.first_atom
    }

    /// Return the number of atom strings stored in the image header.
    #[must_use]
    pub(in crate::engine::code) const fn header_count(self) -> u32 {
        self.atom_count
    }

    /// Validate a zero-based header position and return its strong slot type.
    #[must_use]
    pub(in crate::engine::code) const fn header_slot(self, index: u32) -> Option<HeaderAtomSlot> {
    ...
    pub(in crate::engine::code) fn decode_metadata_atom<'input, C>(
        self,
        cursor: &mut C,
    ...
    pub(in crate::engine::code) fn resolve_metadata_atom(
        self,
        encoded: u32,
    ...
    pub(in crate::engine::code) fn encode_metadata_atom(
        self,
    ...
    pub(in crate::engine::code) fn validate_metadata_atom(
        self,
        atom: BinaryAtom,
        diagnostic_offset: usize,
    ) -> Result<(), WireError> {
    ...
    pub(in crate::engine::code) fn resolve_opcode_atom(
        self,
        raw: u32,
        diagnostic_offset: usize,
    ) -> Result<BinaryAtom, WireError> {
    ...
    pub(in crate::engine::code) fn encode_opcode_atom(
        self,
        atom: BinaryAtom,
        offset: usize,
