...
impl BcTag {
    #[must_use]
    pub(in crate::engine::code) const fn from_byte(byte: u8) -> Option<Self> {
        match byte {
            1 => Some(Self::Null),
    ...
    #[must_use]
    pub(in crate::engine::code) const fn to_byte(self) -> u8 {
        self as u8
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::engine::code) enum ReaderMode {
...
/// policy.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::engine::code) struct WireLimits {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::engine::code) enum ResourceKind {
    InputBytes,
    OutputBytes,
    AtomCount,
    StringCodeUnits,
...
pub(in crate::engine::code) enum WireError {
    ResourceLimit {
        kind: ResourceKind,
        requested: usize,
        limit: usize,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(in crate::engine::code) enum WireString {
    Narrow(Box<[u8]>),
    Wide(Box<[u16]>),
}
...
impl WireString {
    #[must_use]
    pub(in crate::engine::code) fn len(&self) -> usize {
        match self {
    ...
    #[must_use]
    pub(in crate::engine::code) const fn is_wide(&self) -> bool {
        matches!(self, Self::Wide(_))
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::engine::code) struct BinaryObjectHeader {
    pub atom_count: u32,
}

/// Checked cursor over one immutable binary-object input.
pub(in crate::engine::code) struct WireCursor<'a> {
    input: &'a [u8],
    offset: usize,
    mode: ReaderMode,
    limits: WireLimits,
...
impl<'a> WireCursor<'a> {
    ...
    #[must_use]
    pub(in crate::engine::code) const fn position(&self) -> usize {
        self.offset
    }

    #[must_use]
    pub(in crate::engine::code) const fn mode(&self) -> ReaderMode {
        self.mode
    }

    #[must_use]
    pub(in crate::engine::code) fn remaining(&self) -> usize {
    ...
    pub(in crate::engine::code) fn read_u8(&mut self) -> Result<u8, WireError> {
        Ok(self.take(1)?[0])
    }

    pub(in crate::engine::code) fn read_u16_le(&mut self) -> Result<u16, WireError> {
        let bytes: [u8; 2] = self
            .take(2)?
            .try_into()
    ...
    /// wire layer.
    pub(in crate::engine::code) fn read_u64_le(&mut self) -> Result<u64, WireError> {
        let bytes: [u8; 8] = self
            .take(8)?
            .try_into()
    ...
    pub(in crate::engine::code) fn read_bytes(
        &mut self,
        length: usize,
    ...
    pub(in crate::engine::code) fn read_tag(&mut self) -> Result<BcTag, WireError> {
    ...
    pub(in crate::engine::code) fn read_uleb128(&mut self) -> Result<u32, WireError> {
    ...
    pub(in crate::engine::code) fn read_i32(&mut self) -> Result<i32, WireError> {
        let encoded = self.read_uleb128()?;
        Ok(decode_zigzag_i32(encoded))
    }

    pub(in crate::engine::code) fn read_f64(&mut self) -> Result<f64, WireError> {
        let bytes: [u8; 8] = self
            .take(8)?
            .try_into()
            .expect("an eight-byte checked slice must convert to an array");
    ...
    pub(in crate::engine::code) fn read_header(&mut self) -> Result<BinaryObjectHeader, WireError> {
    ...
    pub(in crate::engine::code) fn read_string(&mut self) -> Result<WireString, WireError> {
    ...
    pub(in crate::engine::code::binary_object) fn validate_wire_end(
        &self,
    ) -> Result<(), WireError> {
    ...
    pub(in crate::engine::code) fn finish(self) -> Result<(), WireError> {
        self.validate_wire_end()
    }
}

/// Bounded canonical writer for the pure BCv5 primitives.
pub(in crate::engine::code) struct WireWriter {
    output: Vec<u8>,
...
impl WireWriter {
    #[must_use]
    pub(in crate::engine::code) const fn new(max_output_bytes: usize) -> Self {
        Self {
            output: Vec::new(),
            max_output_bytes,
        }
    ...
    pub(in crate::engine::code) fn write_u8(&mut self, value: u8) -> Result<(), WireError> {
        self.reserve(1)?;
        self.output.push(value);
        Ok(())
    }

    pub(in crate::engine::code) fn write_u16_le(&mut self, value: u16) -> Result<(), WireError> {
        self.write_bytes(&value.to_le_bytes())
    }

    /// Write one fixed-width little-endian `uint64_t` wire primitive.
    pub(in crate::engine::code) fn write_u64_le(&mut self, value: u64) -> Result<(), WireError> {
        self.write_bytes(&value.to_le_bytes())
    }

    pub(in crate::engine::code) fn write_bytes(&mut self, bytes: &[u8]) -> Result<(), WireError> {
        self.reserve(bytes.len())?;
        self.output.extend_from_slice(bytes);
        Ok(())
    }

    pub(in crate::engine::code) fn write_tag(&mut self, tag: BcTag) -> Result<(), WireError> {
        self.write_u8(tag.to_byte())
    }

    pub(in crate::engine::code) fn write_uleb128(&mut self, value: u32) -> Result<(), WireError> {
    ...
    pub(in crate::engine::code) fn write_i32(&mut self, value: i32) -> Result<(), WireError> {
        self.write_uleb128(encode_zigzag_i32(value))
    }

    pub(in crate::engine::code) fn write_f64(&mut self, value: f64) -> Result<(), WireError> {
    ...
    pub(in crate::engine::code) fn write_header(
        &mut self,
        atom_count: u32,
    ) -> Result<(), WireError> {
    ...
    pub(in crate::engine::code) fn write_string(
        &mut self,
        value: &WireString,
    ) -> Result<(), WireError> {
    ...
    #[must_use]
    pub(in crate::engine::code) fn as_bytes(&self) -> &[u8] {
        &self.output
    }

    #[must_use]
    pub(in crate::engine::code) fn into_bytes(self) -> Vec<u8> {
        self.output
    }
}
