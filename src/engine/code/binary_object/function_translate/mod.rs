...
pub(in crate::engine::code::binary_object) use dto::{
...
pub(in crate::engine::code::binary_object) enum TranslationTarget {
    Scalar,
    Ordinary,
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(in crate::engine::code::binary_object) struct FunctionTranslateError {
...
impl FunctionTranslateError {
    ...
    #[must_use]
    pub(in crate::engine::code::binary_object) const fn is_unadmitted_operand_error(&self) -> bool {
        matches!(
            self.kind,
...
pub(in crate::engine::code::binary_object) fn translate_function<'image>(
...
mod tests {
    ...
    #[test]
    fn operand_free_to_propkey_translation_is_one_ordinary_typed_operation() {
        let expansion = lower_operation(Recipe::ToPropKey, &NativeOperands::None).unwrap();
        assert_eq!(expansion.len(), 1);
        let mut operations = expansion.into_operations();
        assert!(matches!(
            operations.next(),
