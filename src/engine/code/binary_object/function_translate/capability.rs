...
use crate::engine::code::binary_object::pinned_opcodes::{
...
///
/// Counts are locked by tests: 110 Blocked, 1 ScalarOnly, 104 OrdinaryOnly,
/// and 29 Shared.
#[rustfmt::skip]
pub(super) const CAPABILITY_REGISTRY: [CapabilityRow; PINNED_OPCODE_COUNT] = [
    ...
    row!(49, AtomU8, OrdinaryOnly, Recipe::ThrowReadOnly),
    ...
    row!(111, None, OrdinaryOnly, Recipe::ToObject),
    row!(112, None, OrdinaryOnly, Recipe::ToPropKey),
...
mod tests {
    use super::*;
    use crate::engine::code::binary_object::function_translate::InstructionAudience;
    ...
    #[test]
    fn ordinary_throw_rows_are_the_exact_reviewed_completion_set() {
        assert_eq!(
    ...
    #[test]
    fn ordinary_to_object_is_the_exact_operand_free_raw111_row() {
        let row = CAPABILITY_REGISTRY[111];
        let opcode = PinnedOpcode::from_byte(111).unwrap();
        assert_eq!(row.raw, 111);
        assert_eq!(row.expected_format, OpcodeFormat::None);
    ...
    #[test]
    fn ordinary_to_propkey_is_the_exact_operand_free_raw112_row() {
        let row = CAPABILITY_REGISTRY[112];
        let opcode = PinnedOpcode::from_byte(112).unwrap();
        assert_eq!(row.raw, 112);
        assert_eq!(row.expected_format, OpcodeFormat::None);
        assert_eq!(
            row.policy,
    ...
    fn blocked_frontier_has_stable_typed_category_counts() {
        ...
        for row in CAPABILITY_REGISTRY {
            ...
            let index = match blocker {
                ...
                TranslationBlocker::EvalOrModule => 4,
                TranslationBlocker::Binding => 5,
                TranslationBlocker::Property => 6,
                TranslationBlocker::ObjectConstruction => 7,
                TranslationBlocker::LexicalEnvironment => 8,
                TranslationBlocker::ControlFlow => 9,
                TranslationBlocker::DynamicScope => 10,
                TranslationBlocker::Iteration => 11,
                TranslationBlocker::Suspension => 12,
                TranslationBlocker::Operator => 13,
                TranslationBlocker::Specialized => 14,
            };
            counts[index] += 1;
        }
        assert_eq!(counts, [1, 4, 2, 1, 3, 7, 16, 15, 25, 4, 9, 11, 5, 4, 3]);
