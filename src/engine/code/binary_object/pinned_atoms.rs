...
pub(in crate::engine::code) const PINNED_ATOM_COUNT: u32 = 242;

/// First atom ID available to a binary object's dynamic atom table.
pub(in crate::engine::code) const FIRST_DYNAMIC_ATOM: u32 = PINNED_ATOM_COUNT + 1;
...
pub(in crate::engine::code) enum PinnedAtomKind {
    String,
    Private,
    Symbol,
}
...
pub(in crate::engine::code) struct PinnedAtomId(u32);

impl PinnedAtomId {
    /// Validate a raw QuickJS atom ID against the pinned manifest.
    #[must_use]
    pub(in crate::engine::code) const fn from_raw(raw: u32) -> Option<Self> {
    ...
    #[must_use]
    pub(in crate::engine::code) const fn raw(self) -> u32 {
        self.0
    }

    /// Return the identity class assigned by `quickjs-atom.h`.
    #[must_use]
    pub(in crate::engine::code) const fn kind(self) -> PinnedAtomKind {
    ...
    #[must_use]
    pub(in crate::engine::code) const fn spelling(self) -> &'static str {
...
#[must_use]
pub(in crate::engine::code) fn lookup_string(value: &WireString) -> Option<PinnedAtomId> {
