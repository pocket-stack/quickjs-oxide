...
impl Runtime {
    pub(crate) fn read_trusted_ordinary_function_in_realm(
        &self,
        realm: ContextId,
        bytes: &[u8],
        ...
        let function = UnlinkedFunction::new(
            ...
            (0..metadata.argument_count())
                .map(|_| crate::engine::code::function::UnlinkedVariableDefinition::ordinary(None))
                .collect(),
            (0..metadata.local_count())
                .map(|_| crate::engine::code::function::UnlinkedVariableDefinition::ordinary(None))
                .collect(),
            Vec::new(),
        );
...
mod tests {
    ...
    fn ordinary_leaf_draft_ops_lower_one_for_one_without_reordering() {
        ...
        assert!(matches!(
            lower(OrdinaryLeafOp::ToPropKey),
            Instruction::ToPropKey
        ));
        assert!(matches!(
            lower(OrdinaryLeafOp::PushThis),
    ...
    #[test]
    fn ordinary_to_propkey_publishes_one_for_one_without_a_synthetic_constant() {
        let mut next_synthetic_index = 7;
        assert!(matches!(
    ...
    #[test]
    fn ordinary_push_this_publishes_one_for_one_without_a_synthetic_constant() {
        let mut next_synthetic_index = 7;
        assert!(matches!(
