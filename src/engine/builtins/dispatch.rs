...
impl Runtime {
    ...
    pub(crate) fn dispatch_native_function(
        &self,
        callable: &CallableRef,
        target: NativeFunctionId,
        realm: ContextId,
        ...
        match target {
            ...
            NativeFunctionId::SharedArrayBuffer(kind) => {
                self.call_shared_array_buffer_native(realm, kind, invocation, arguments)
            }
            ...
            NativeFunctionId::Atomics(kind) => {
                self.call_atomics_native(realm, kind, invocation, arguments)
            }
