...
impl Runtime {
    ...
    pub(crate) fn initialize_array_buffer_intrinsic(
        &self,
        realm: ContextId,
    ...
    pub(crate) fn call_array_buffer_native(
        &self,
        realm: ContextId,
    ...
    pub(crate) fn detach_array_buffer_object(
        &self,
        object: &ObjectRef,
    ) -> Result<(), RuntimeError> {
        if !object.belongs_to(self) {
    ...
    pub(crate) fn detach_array_buffer_value(&self, value: &Value) -> Result<(), RuntimeError> {
        let Value::Object(object) = value else {
            return Ok(());
        };
        if !object.belongs_to(self) {
