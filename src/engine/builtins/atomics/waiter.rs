...
pub(crate) struct WaitLocation {
    backing_id: u64,
    byte_offset: usize,
}

impl WaitLocation {
    pub(crate) const fn new(backing_id: u64, byte_offset: usize) -> Self {
        Self {
            backing_id,
            byte_offset,
        }
...
pub(crate) fn with_seq_cst<R>(operation: impl FnOnce() -> R) -> R {
...
pub(crate) fn notify(location: WaitLocation, count: usize) -> usize {
    let mut coordinator = lock_coordinator();
    let mut notified = 0;
    let mut index = 0;
