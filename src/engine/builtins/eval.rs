...
impl Runtime {
    ...
    fn direct_eval_root_bindings(
        ...
        for (scope_index, descriptor_scope) in environment.descriptor.scopes.iter().enumerate() {
            if descriptor_scope.kind == crate::engine::code::function::metadata::EvalScopeKind::With
            ...
            for binding in &descriptor_scope.bindings {
                let is_catch_scope = descriptor_scope.kind
                ...
                let is_with_scope = descriptor_scope.kind
                ...
                {
                    EvalVariableEnvironment::Global | EvalVariableEnvironment::StrictLocal(_) => {
                        false
                    }
    ...
    pub(crate) fn execute_indirect_string_eval(
        &self,
        realm: ContextId,
        source: &JsString,
    ) -> Result<Completion, RuntimeError> {
