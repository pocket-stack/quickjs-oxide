...
use crate::engine::heap::{
    CollectionIteratorCurrentIndices, InternalCallableData, ObjectId, ObjectKind, ObjectPayload,
...
impl QjsValuePrinter<'_, '_> {
    ...
    fn print_float(&mut self, value: f64) {
        if value == 0.0 && value.is_sign_negative() {
            ...
            self.push_ascii(&crate::engine::value::number_to_string(value));
        }
    }
    ...
    fn snapshot_object(&self, object: ObjectId) -> Result<ObjectPrintSnapshot, RuntimeError> {
        ...
        let (class_name, body) = match &object_data.payload {
            ...
            ObjectPayload::NativeFunction {
                internal:
                    Some(crate::engine::heap::InternalCallableData::PromiseResolving { kind, .. }),
                ..
            } => {
