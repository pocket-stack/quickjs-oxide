...
use crate::engine::value::{JsString, JsStringError, decode_quickjs_utf8};
...
pub struct SourceTextUtf16Units<'a> {
...
impl SourceText {
    ...
    pub fn try_from_raw_bytes(source: &[u8]) -> Result<Self, JsStringError> {
    ...
    /// cannot be reserved.
    pub fn try_from_js_string(source: &JsString) -> Result<Self, JsStringError> {
        let raw = source
            .try_to_wtf8_bytes()
            .map_err(|_| JsStringError::OutOfMemory)?;
    ...
    #[must_use]
    pub fn has_surrogate_markers(&self) -> bool {
        !self.surrogates.is_empty()
    }

    #[must_use]
    pub fn surrogate_markers(&self) -> &[SurrogateMarker] {
        &self.surrogates
    }

    /// Return the source surrogate represented at one carrier/raw byte offset.
    #[must_use]
    pub fn surrogate_at(&self, byte_offset: usize) -> Option<u16> {
    ...
    #[must_use]
    pub fn invalid_byte_at(&self, byte_offset: usize) -> bool {
    ...
    #[must_use]
    pub fn slice(&self, range: Range<usize>) -> Option<SourceTextSlice<'_>> {
    ...
    #[must_use]
    pub fn semantic_utf16_units(&self) -> Option<SourceTextUtf16Units<'_>> {
    ...
    pub fn try_to_raw_bytes(&self) -> Result<Vec<u8>, TryReserveError> {
        self.full_slice().try_to_raw_bytes()
    }

    /// Reconstruct the original ECMAScript String.
    pub fn try_to_js_string(&self) -> Result<JsString, JsStringError> {
...
impl<'a> SourceTextSlice<'a> {
    ...
    #[must_use]
    pub fn base_byte_offset(&self) -> u32 {
    ...
    #[must_use]
    pub fn semantic_utf16_units(&self) -> Option<SourceTextUtf16Units<'a>> {
