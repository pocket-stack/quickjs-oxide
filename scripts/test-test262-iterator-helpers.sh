#!/usr/bin/env bash
# Reproduce the dependency-audited synchronous Iterator helpers Test262 gate.
...
verify_inventory() {
    ...
    if [[ "$actual_count" != "$expected_count" || "$actual_hash" != "$expected_hash" ]]; then
        echo "error: Iterator helpers $name inventory drifted" >&2
        exit 1
    fi
}
...
verify_quickjs_oracle() {
    ...
    if ! sloppy_output=$(cd -- "$source_dir" \
        ...
        printf '%s\n' "$sloppy_output" >&2
        echo "error: pinned QuickJS could not execute the sloppy Iterator helpers cohort" >&2
        exit 1
    fi
    if ! strict_output=$(cd -- "$source_dir" \
        && ./run-test262 -s -m -c test262.conf -f "${files[@]}" 2>&1); then
        printf '%s\n' "$strict_output" >&2
        echo "error: pinned QuickJS could not execute the strict Iterator helpers cohort" >&2
        exit 1
    fi
    ...
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])' <<<"$sloppy_output" \
        || ! grep -Fq "Average memory statistics for $(read_value quickjs_sloppy_passes) tests:" \
            <<<"$sloppy_output"; then
        printf '%s\n' "$sloppy_output" >&2
        echo "error: pinned QuickJS no longer passes every sloppy Iterator helpers path" >&2
        exit 1
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])' <<<"$strict_output" \
        || ! grep -Fq "Average memory statistics for $(read_value quickjs_strict_passes) tests:" \
            <<<"$strict_output"; then
        printf '%s\n' "$strict_output" >&2
        echo "error: pinned QuickJS no longer passes every strict Iterator helpers path" >&2
        exit 1
    fi
}
...
if [[ "$(read_value quickjs)" != "2026-06-04" \
    ...
    || "$(read_value oxide_profile_sha256)" != "d9e6f4fcf8cb6f20fb0ebba012451abbaad52bbe676430f2433b9398174e3c83" \
...
[[ "$(manifest_paths | sha256_stream)" == "$(read_value manifest_sha256)" \
    ...
    || { echo "error: Iterator helpers manifest content drifted" >&2; exit 1; }

feature_inventory=
include_inventory=
global_this_inventory=
variant_keys=
while IFS= read -r test_path; do
    [[ -f "$suite/$test_path" ]] \
        || { echo "error: missing Iterator helpers path: $test_path" >&2; exit 1; }
    metadata=$(metadata_block "$test_path")
    if grep -Fq 'negative:' <<<"$metadata"; then
        echo "error: Iterator helpers cohort unexpectedly gained a negative test: $test_path" >&2
        exit 1
    fi
    ...
    case "$flag_line" in
        "" | "flags: []") ;;
        *)
            echo "error: Iterator helpers path no longer has both ordinary variants: $test_path: $flag_line" >&2
            exit 1
            ;;
    esac
    features=$(metadata_list "$test_path" features)
    grep -Fxq 'iterator-helpers' <<<"$features" \
        || { echo "error: selected path lost iterator-helpers metadata: $test_path" >&2; exit 1; }
    feature_inventory+=$'\n'"$features"
    include_inventory+=$'\n'"$(metadata_list "$test_path" includes)"
    if grep -Fxq 'globalThis' <<<"$features"; then
        global_this_inventory+=$'\n'"$test_path"
    fi
    variant_keys+=$'\n'"$test_path"$'\t'sloppy$'\n'"$test_path"$'\t'strict
done < <(manifest_paths)
...
if [[ ${#pending_keys[@]} -ne 0 ]] && ! "$bless"; then
    printf 'error: Iterator helpers Oxide baseline needs refresh after implementation: %s\n' \
        "${pending_keys[*]}" >&2
    exit 1
fi
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: Iterator helpers report metadata drifted" >&2
    exit 1
fi
...
if "$bless"; then
    if [[ "$actual_passes" != "$expected_variants" \
        ...
        echo "error: refusing to bless a non-green Iterator helpers vector" >&2
        exit 1
    fi
...
if [[ "$runner_summary" != "$expected_runner_summary" \
    ...
    echo "error: Iterator helpers Test262 classified vector drifted" >&2
    awk -F'\t' '
...
printf 'Iterator helpers Test262 gate is exact: %s/%s pass across %s clean paths\n' \
