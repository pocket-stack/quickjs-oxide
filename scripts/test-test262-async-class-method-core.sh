#!/usr/bin/env bash
# Reproduce the R3ad public ordinary async class-method Test262 gate.
...
baseline=tests/test262-async-class-method-core-baseline.txt
manifest=tests/test262-async-class-method-core.txt
admission_profile=tests/test262-async-class-method-core.conf
exclusions=tests/test262-async-class-method-core-exclusions.tsv
global_profile=compat/test262-oxide.conf
report=target/test262-async-class-method-core.tsv
json_report=target/test262-async-class-method-core.jsonl
quickjs_log=target/test262-async-class-method-core-quickjs.log
...
expect_value() {
    ...
    if [[ "$(read_value "$key")" != "$expected" ]]; then
        echo "error: async class-method baseline $key drifted" >&2
        exit 1
    fi
}
...
verify_inventory() {
    ...
    if [[ "$(inventory_count "$inventory")" != "$(read_value "${name}_paths")" \
        ...
        echo "error: async class-method $name inventory drifted" >&2
        exit 1
    fi
}
...
verify_quickjs_oracle() {
    ...
    # Audit every applicable strict/non-strict mode in the upstream runner.
    # QuickJS reports the path count in its memory summary; Oxide separately
    # freezes every admitted metadata-selected variant below.
    if ! (cd -- "$source_dir" \
        && ./run-test262 -a -m -c test262.conf -f "${files[@]}") \
        >"$quickjs_log" 2>&1; then
        cat "$quickjs_log" >&2
        echo "error: pinned QuickJS could not execute the async class-method cohort" >&2
        exit 1
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])' "$quickjs_log" \
        || ! grep -Fq \
            ...
            "$quickjs_log"; then
        cat "$quickjs_log" >&2
        echo "error: pinned QuickJS no longer passes the async class-method cohort" >&2
        exit 1
    fi
}
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3ad.XXXXXX")
...
expect_value candidate_paths 313
expect_value candidate_direct_paths 128
expect_value candidate_function_to_string_paths 4
expect_value candidate_definition_contextual_paths 5
expect_value candidate_element_composition_paths 160
expect_value candidate_syntax_paths 12
expect_value candidate_staging_paths 4
expect_value candidate_proxy_paths 0
expect_value excluded_paths 19
expect_value excluded_private_async_method_paths 8
expect_value excluded_private_async_generator_paths 8
expect_value excluded_async_generator_adjacency_paths 3
expect_value admitted_direct_paths 128
expect_value admitted_function_to_string_paths 4
expect_value admitted_definition_contextual_paths 5
expect_value admitted_element_composition_paths 144
expect_value admitted_syntax_paths 12
expect_value admitted_staging_paths 1
expect_value paths 294
expect_value quickjs_passes 313
expect_value positive_paths 236
expect_value negative_paths 58
expect_value async_paths 226
expect_value sync_paths 68
expect_value double_mode_paths 274
expect_value no_strict_paths 20
expect_value only_strict_paths 0
expect_value variants 568
expect_value sloppy_variants 294
expect_value strict_variants 274
expect_value features 13
expect_value includes 2
expect_value flags 3
expect_value runnable 568
expect_value passes 568
...
if [[ "$(sha256_file "$global_profile")" != "$(read_value global_oxide_profile_sha256)" \
    ...
    echo "error: async class-method pinned profile or exclusions drifted" >&2
    exit 1
fi
...

{
    git -C "$suite" ls-files \
        'test/language/expressions/class/async-method/*.js' \
        'test/language/expressions/class/async-method/**/*.js' \
        'test/language/expressions/class/async-method-static/*.js' \
        'test/language/expressions/class/async-method-static/**/*.js' \
        'test/language/statements/class/async-method/*.js' \
        'test/language/statements/class/async-method/**/*.js' \
        'test/language/statements/class/async-method-static/*.js' \
        'test/language/statements/class/async-method-static/**/*.js'
    git -C "$suite" ls-files \
        'test/built-ins/Function/prototype/toString/async-method-class-*.js' \
        ...
        'test/language/statements/class/definition/early-errors-class-async-method-duplicate-parameters.js' \
    ...
    for class_form in expressions statements; do
        git -C "$suite" ls-files \
            "test/language/$class_form/class/elements/same-line-async-method-*.js" \
            "test/language/$class_form/class/elements/after-same-line-static-async-method-*.js"
        for syntax_path in \
            early-errors/grammar-special-meth-contains-super-async.js \
            early-errors/grammar-special-meth-ctor-async-meth.js \
            early-errors/grammar-static-async-meth-prototype.js \
            early-errors/grammar-static-async-meth-super.js \
            valid/grammar-special-prototype-async-meth-valid.js \
            valid/grammar-static-ctor-async-meth-valid.js; do
            git -C "$suite" ls-files \
    ...
    printf '%s\n' \
        ...
        test/staging/sm/BigInt/property-name.js \
        test/staging/sm/Function/function-name-computed-01.js \
        test/staging/sm/Function/function-name-computed-02.js
...
if [[ "$(inventory_count "$candidate")" != "$(read_value candidate_paths)" \
    ...
    || "$(awk '
        /test\/language\/(expressions|statements)\/class\/async-method(-static)?\// {
            count++
        }
        END { print count + 0 }
    ...
    || "$(awk '
        /test\/language\/statements\/class\/async-meth-escaped-async\.js$/ \
            || /test\/language\/statements\/class\/definition\// { count++ }
        END { print count + 0 }
    ' "$candidate")" != "$(read_value candidate_definition_contextual_paths)" \
    || "$(awk '
        /\/class\/elements\/(same-line-async-method|after-same-line-static-async-method)-/ {
            count++
        }
        END { print count + 0 }
    ...
    echo "error: async class-method candidate universe drifted" >&2
    exit 1
fi
...
if awk -F'\t' \
    ...
    "$exclusions" >&2; then
    echo "error: async class-method exclusions must have two populated TSV columns" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$excluded_paths")" != "$(read_value excluded_paths)" \
    ...
    || "$(awk -F'\t' '$1 == "private_async_method" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_private_async_method_paths)" \
    || "$(awk -F'\t' '$1 == "private_async_generator" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_private_async_generator_paths)" \
    || "$(awk -F'\t' '$1 == "async_generator_adjacency" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_async_generator_adjacency_paths)" \
    || "$(awk -F'\t' \
        '$1 != "private_async_method" \
            && $1 != "private_async_generator" \
            && $1 != "async_generator_adjacency" { count++ } \
        END { print count + 0 }' "$exclusions")" \
        != "0" ]]; then
    echo "error: async class-method exclusion ledger drifted" >&2
    exit 1
fi
if [[ -n "$(comm -23 "$excluded_paths" "$candidate")" ]]; then
    echo "error: async class-method exclusion escaped the candidate universe" >&2
    exit 1
fi
...
if [[ -n "$(comm -12 "$manifest" "$excluded_paths")" ]]; then
    echo "error: async class-method manifest and exclusions overlap" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$manifest")" != "$(read_value paths)" \
    ...
    || "$(awk '
        ...
        END { print count + 0 }
    ' "$manifest")" != "$(read_value admitted_direct_paths)" \
    ...
    || "$(awk '
        /test\/language\/statements\/class\/async-meth-escaped-async\.js$/ \
            || /test\/language\/statements\/class\/definition\// { count++ }
        END { print count + 0 }
    ' "$manifest")" != "$(read_value admitted_definition_contextual_paths)" \
    || "$(awk '
        /\/class\/elements\/(same-line-async-method|after-same-line-static-async-method)-/ {
            count++
        }
    ...
    echo "error: async class-method manifest drifted" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$variant_keys")" != "$(read_value variants)" \
    ...
    echo "error: async class-method metadata composition drifted" >&2
    exit 1
fi
...
[[ "$(profile_section execution)" == "async=true" ]] \
    || { echo "error: async class-method profile must opt into only the async host" >&2; exit 1; }

verify_quickjs_oracle
if "$check_only"; then
    printf 'async class-method inputs verified: %s candidates - %s explicit exclusions = %s paths; QuickJS %s passes all candidates; Oxide gate covers %s variants\n' \
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: async class-method report metadata drifted" >&2
    exit 1
fi
...
if [[ "$runner_summary" != "$expected_runner_summary" \
    ...
    echo "error: async class-method all-pass vector drifted" >&2
    awk -F'\t' '
        !/^#/ && !($1 == "path" && $2 == "variant") && $7 != "pass" {
            print $1 "\t" $2 "\t" $7 "\t" $8 "\t" $9 "\t" $10
        }
...
printf 'async class-method Test262 gate passes: %s/%s variants across %s audited paths\n' \
