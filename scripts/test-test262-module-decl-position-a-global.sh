#!/usr/bin/env bash
# Reproduce and authenticate the R3ec-A module declaration-position successor.
...
baseline=tests/test262-module-decl-position-a-global-baseline.txt
predecessor_baseline=tests/test262-import-meta-a-global-baseline.txt
canonical_baseline=tests/test262-full-baseline.txt
profile=compat/test262-oxide.conf
upstream=compat/upstream.toml
generator=scripts/generate-test262-module-decl-position-a.mjs
prepare_suite=scripts/prepare-test262.sh
oracle_builder=scripts/build-quickjs-oracle.sh
manifest=tests/test262-module-decl-position-a.txt
ledger=tests/test262-module-decl-position-a-ledger.tsv
variants=tests/test262-module-decl-position-a-variants.tsv
negatives=tests/test262-module-decl-position-a-negatives.txt
exclusions=tests/test262-module-decl-position-a-exclusions.tsv
quickjs_projection=tests/test262-module-decl-position-a-quickjs-projection.txt
candidate=tests/test262-module-decl-position-a-global-candidate.tsv
workers=${TEST262_WORKERS:-4}
full_workers=${TEST262_FULL_WORKERS:-2}
reuse_full_reports=${TEST262_REUSE_FULL_REPORTS:-false}
full_dir=${TEST262_FULL_REPORT_DIR:-$root/target/test262-module-decl-position-a-global-full}
...
baseline_lines=87
baseline_sha256=c2dfdc3388c84f30c795fb20ca4b130479905d34e521c825213b1c7a238ec657

usage() {
    printf 'usage: %s [--check|--focused|--full]\n' "${0##*/}"
    printf '  --check    authenticate frozen inputs and receipts\n'
    printf '  --focused  rerun the 86-path Oxide and QuickJS cohorts twice (default)\n'
...
check_file() {
    [[ -f "$1" && ! -L "$1" && "$(lines "$1")" == "$2" && "$(sha "$1")" == "$3" ]] \
        || die "authenticated R3ec-A input drifted: $1"
}
header() {
    awk -F= -v wanted="# $2" \
        '$1==wanted{sub(/^[^=]*=/,"");print;found++} END{if(found!=1)exit 1}' \
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3ec-a.XXXXXX")
trap 'rm -rf -- "$tmp"' EXIT
trap 'exit 129' HUP
trap 'exit 130' INT
trap 'exit 143' TERM
...
reverse_tsv() {
    ...
    awk -F'\t' -v OFS='\t' \
        ...
        {print}
        END{if(headers!=1||summaries!=1||changed!=86)exit 5}
    ' "$input" >"$output" || die "could not reverse R3ec-A TSV: $input"
}
...
if (metadata !== 1 || summary !== 1 || changed !== 86) process.exit(5);
fs.writeFileSync(output, `${outputLines.join("\n")}\n`);
NODE
}

check_file "$baseline" "$baseline_lines" "$baseline_sha256"
[[ "$(value schema)" == r3ec-a-test262-module-decl-position-global-v1 \
    && "$(value roots)" == 86 \
    && "$(value export_roots)" == 43 \
    && "$(value import_roots)" == 43 \
    && "$(value generator_roots)" == 12 \
    && "$(value focused_variants)" == 86 \
    && "$(value profile_added_features)" == 0 \
    && "$(value profile_added_negatives)" == 86 \
    && "$(value full_variants)" == 102037 \
    && "$(value full_outcome_changed)" == 86 \
    && "$(value full_detail_only)" == 0 \
    && "$(value full_unchanged)" == 101951 \
    && "$(value full_pass_gains)" == 86 \
    && "$(value full_pass_regressions)" == 0 \
    && "$(value full_candidate_runnable)" == 68347 \
    && "$(value full_candidate_passes)" == 68295 ]] \
    || die 'R3ec-A baseline contract drifted'

check_file "$predecessor_baseline" "$(value predecessor_baseline_lines)" \
    "$(value predecessor_baseline_sha256)"
[[ "$(predecessor_value schema)" == r3eb-a-test262-import-meta-global-v1 \
    ...
    || die 'R3ec-A does not checksum-bridge the frozen R3eb-A receipt'
...
for commit in "$parent_commit" "$candidate_commit"; do
    trusted_git -C "$root" cat-file -e "$commit^{commit}" 2>/dev/null \
        || die "authenticated R3ec-A commit is unavailable: $commit"
done
[[ "$(trusted_git -C "$root" rev-parse "$parent_commit^{tree}")" == "$(value parent_tree)" \
    ...
    || die 'R3ec-A parent or candidate tree drifted'
[[ "$(trusted_git -C "$root" rev-parse "$candidate_commit^")" == "$parent_commit" ]] \
    || die 'R3ec-A admission commit is not a direct child of its frozen parent'
...
[[ "$(trusted_git -C "$root" rev-parse "$predecessor_candidate^{tree}")" \
    ...
    || die 'R3eb-A predecessor candidate tree drifted'
trusted_git -C "$root" merge-base --is-ancestor \
    "$predecessor_candidate" "$parent_commit" \
    || die 'R3eb-A semantic candidate is not an ancestor of the R3ec-A parent layer'
trusted_git -C "$root" diff --quiet "$predecessor_candidate" "$parent_commit" -- src compat \
    || die 'semantic inputs drifted between R3eb-A and the R3ec-A parent layer'

trusted_git -C "$root" diff --quiet "$candidate_commit" -- \
    src compat "$generator" "$prepare_suite" "$oracle_builder" "$manifest" \
    "$ledger" "$variants" "$negatives" "$exclusions" "$quickjs_projection" \
    || die 'live R3ec-A semantic or authenticated evidence input drifted'
[[ -z "$(trusted_git -C "$root" ls-files --others --exclude-standard -- src compat)" ]] \
    || die 'untracked semantic input is outside the R3ec-A admission commit'
...
for section in features audited-negative-tests execution host-agent-tests; do
    for role in parent candidate; do
        ...
        sort -c "$tmp/$role-$section" \
            || die "R3ec-A $role profile section is not bytewise sorted: $section"
        [[ -z "$(uniq -d "$tmp/$role-$section")" ]] \
            || die "R3ec-A $role profile section contains duplicates: $section"
    done
done
cmp -s "$tmp/parent-features" "$tmp/candidate-features" \
    || die 'R3ec-A changed the feature set'
cmp -s "$tmp/parent-execution" "$tmp/candidate-execution" \
    || die 'R3ec-A changed execution policy'
cmp -s "$tmp/parent-host-agent-tests" "$tmp/candidate-host-agent-tests" \
    || die 'R3ec-A changed agent-host admission'
...
[[ ! -s "$tmp/removed-negatives" ]] || die 'R3ec-A removed an audited negative'
diff -u "$negatives" "$tmp/added-negatives"
[[ "$(lines "$tmp/added-negatives")" == "$(value profile_added_negatives)" ]] \
    || die 'R3ec-A audited-negative delta count drifted'
...
grep -Fqx \
    'module-decl-position-a: roots=86 export=43 import=43 generators=12 variants=86 canaries=3' \
    "$tmp/generator.log" || die 'declaration-position generator contract drifted'

sed '1d' "$variants" | cut -f1,2 >"$tmp/focused.keys"
[[ "$(lines "$tmp/focused.keys")" == 86 \
    && "$(sha "$tmp/focused.keys")" == "$(value focused_keys_sha256)" \
    && -z "$(uniq -d "$tmp/focused.keys")" ]] \
    || die 'R3ec-A focused key set drifted'
...
verify_focused_report() {
    ...
    [[ "$(header "$report" oxide_profile_sha256)" == "$(value candidate_profile_sha256)" \
        ...
        && "$(report_count pass "$report")" == 86 \
        && "$(report_runnable "$report")" == 86 ]] \
        || die "R3ec-A focused candidate outcome drifted: $report"
...
[[ "$(canonical_value schema)" == test262-canonical-classified-v2 \
    ...
    || die 'canonical Test262 baseline is not the R3ec-A candidate'

if [[ "$mode" == check ]]; then
    echo 'R3ec-A module declaration-position receipt inputs are authenticated.'
    exit 0
fi
...
[[ -f "$runner" && -x "$runner" && ! -L "$runner" ]] \
    || die 'R3ec-A runner is not an executable regular file'
...
for replay in a b; do
    ...
    cmp -s "$candidate" "$tmp/focused-$replay.tsv" \
        || die "R3ec-A focused TSV replay $replay drifted"
    cmp -s "${candidate%.tsv}.jsonl" "$tmp/focused-$replay.jsonl" \
        || die "R3ec-A focused JSONL replay $replay drifted"
done
cmp -s "$tmp/focused-a.tsv" "$tmp/focused-b.tsv" \
    || die 'R3ec-A focused TSV replays differ'
cmp -s "$tmp/focused-a.jsonl" "$tmp/focused-b.jsonl" \
    || die 'R3ec-A focused JSONL replays differ'
...
for replay in a b; do
    ...
    (cd "$quickjs_source" && ./run-test262 "${quickjs_args[@]}") >"$log" 2>&1 \
        || die "pinned QuickJS rejected the R3ec-A cohort: $log"
    ! grep -Fq 'FAILED' "$log" \
        || die "pinned QuickJS reported a failed R3ec-A test: $log"
    [[ "$(grep -Fxc 'Average memory statistics for 86 tests:' "$log")" == 1 ]] \
        || die "pinned QuickJS did not execute exactly 86 R3ec-A tests: $log"
    {
...
if [[ "$mode" != full ]]; then
    echo 'R3ec-A focused gate passed: Oxide 86/86 twice, QuickJS 86/86 twice, parent 86 unsupported, adjacent frontiers closed.'
    exit 0
fi
...
for full_path in "$full_candidate_a" "$full_candidate_b"; do
    case $full_path in
        /*) ;;
        *) die 'R3ec-A full report paths must be absolute' ;;
    esac
done
[[ "$full_candidate_a" != "$full_candidate_b" \
    && "${full_candidate_a%.tsv}.jsonl" != "${full_candidate_b%.tsv}.jsonl" ]] \
    || die 'R3ec-A full candidate A/B paths must be distinct'
...
for full_path in "$full_candidate_a" "${full_candidate_a%.tsv}.jsonl" \
    ...
    for protected in "$root/$candidate" "$root/${candidate%.tsv}.jsonl" \
        ...
        [[ "$full_path" != "$protected" ]] \
            || die "R3ec-A full report path aliases a protected input: $full_path"
        if [[ -e "$full_path" && "$full_path" -ef "$protected" ]]; then
            die "R3ec-A existing full report aliases a protected input inode: $full_path"
        fi
    done
done
if [[ "$reuse_full_reports" == true ]]; then
    ...
    for report in "$full_candidate_a" "$full_candidate_b"; do
        [[ -f "$report" && -f "${report%.tsv}.jsonl" ]] \
            || die "requested reusable R3ec-A full report is missing: $report"
    done
else
...
if [[ "$full_candidate_a" -ef "$full_candidate_b" \
    ...
    die 'R3ec-A full candidate replays must be distinct files'
fi
for full_path in "$full_candidate_a" "${full_candidate_a%.tsv}.jsonl" \
    "$full_candidate_b" "${full_candidate_b%.tsv}.jsonl"; do
    for protected in "$root/$candidate" "$root/${candidate%.tsv}.jsonl" \
        ...
        if [[ "$full_path" -ef "$protected" ]]; then
            die "R3ec-A full report aliases a protected input inode: $full_path"
        fi
    done
done
...
verify_full_candidate() {
    ...
    [[ "$(header "$report" oxide_profile_sha256)" == "$(value candidate_profile_sha256)" \
        ...
        || die "R3ec-A full candidate outcome drifted: $report"
    ...
    [[ "$(lines "$tmp/full.keys")" == "$(value full_variants)" \
        ...
        || die "R3ec-A full key set drifted: $report"
...
cmp -s "$full_candidate_a" "$full_candidate_b" \
    || die 'R3ec-A full TSV replays are not byte-identical'
cmp -s "${full_candidate_a%.tsv}.jsonl" "${full_candidate_b%.tsv}.jsonl" \
    || die 'R3ec-A full JSONL replays are not byte-identical'
...
printf 'R3ec-A canonical successor passed (%s): 86 declaration-position parse-negative gains, zero regressions, and two distinct byte-identical full reports.\n' \
    "$full_receipt_kind"
