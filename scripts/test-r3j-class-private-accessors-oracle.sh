#!/usr/bin/env bash
# Freeze the QuickJS 2026-06-04 synchronous private-accessor and brand semantics
...
fixture=$root/tests/fixtures/r3j_class_private_accessors.js
expected=$root/tests/fixtures/r3j_class_private_accessors.quickjs-2026-06-04.txt
fixture_sha256=0ee124bbd77f45ae9cd81bc6203cedd03e03b5e78640460abc9670ca77ffca12
expected_sha256=c2656658102e7bfd9ee8da51848e18519afccb9a9ec02cc094d27cb6646d834a
oxide=${OXIDE_QJS:-}
...
verify_hash() {
    ...
    local actual
    if [[ ! -f "$path" ]]; then
        echo "error: R3j oracle input is missing: $path" >&2
        exit 1
    fi
    actual=$(sha256_file "$path")
    if [[ "$actual" != "$pinned" ]]; then
        echo "error: R3j oracle input hash drifted: $path" >&2
...
run_engine() {
    ...
    if ! "$engine" "$@" >"$output" 2>"$errors"; then
        echo "error: $label failed to execute the R3j oracle fixture" >&2
...
compare_transcript() {
    ...
    if ! cmp -s -- "$expected" "$actual"; then
        echo "error: $label R3j transcript drifted" >&2
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3j-oracle.XXXXXX")
trap 'rm -rf -- "$tmp_dir"' EXIT HUP INT TERM

{
    cat "$fixture"
    printf '\nprint(r3jTranscript.join("\\n"));\n'
...
if [[ -n "$oxide" ]]; then
    ...
    echo "R3j class private-accessor differential passed: quickjs-oxide matches QuickJS 2026-06-04"
else
    echo "R3j class private-accessor oracle passed: pinned QuickJS 2026-06-04 transcript is stable"
fi
