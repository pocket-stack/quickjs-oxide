#!/usr/bin/env bash
# Reproduce the checksum-bound active Proxy Test262 outcome vector.
...
baseline=tests/test262-proxy-baseline.txt
manifest=tests/test262-proxy.txt
profile=tests/test262-proxy.conf
report=target/test262-proxy.tsv
json_report=target/test262-proxy.jsonl
oracle_log=target/test262-proxy-quickjs.log
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$expected_profile" != "0c151608ed8cd580238e27188f5e63382ee11e1dc91f7c480db2c366e1232d12" \
    || "$expected_schema" != "test262-canonical-classified-v2" \
    || "$expected_mode" != "both" \
    || "$expected_timeout_ms" != "30000" \
    || "$expected_paths" != "464" \
    || "$expected_variants" != "904" \
    || "$expected_features" != "36" \
    || "$expected_oracle_variants" != "904" ]]; then
    echo "error: Proxy baseline metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_paths" != "$expected_paths" || "$unique_paths" != "$expected_paths" ]]; then
    echo "error: Proxy manifest cardinality drifted" >&2
    exit 1
fi
...
if [[ "$actual_manifest" != "$expected_manifest" \
    ...
    echo "error: Proxy manifest content drifted" >&2
    exit 1
fi
...
' "$profile")
actual_feature_count=$(printf '%s\n' "$actual_features" | awk 'NF { count++ } END { print count + 0 }')
actual_features_hash=$(printf '%s\n' "$actual_features" | sha256_stream)
if [[ "$actual_feature_count" != "$expected_features" \
    || "$actual_features_hash" != "$expected_features_hash" \
    || "$(sha256_file "$profile")" != "$expected_profile" ]]; then
    echo "error: Proxy scoped capability profile drifted" >&2
    exit 1
fi

while IFS= read -r test_path; do
    if [[ ! -f "$suite/$test_path" ]]; then
        echo "error: pinned Proxy path is missing: $test_path" >&2
        exit 1
    fi
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: Proxy report metadata drifted" >&2
    exit 1
fi

diff -u \
    <(printf '%s\n' "$actual_features" | LC_ALL=C sort) \
    <(awk -F'\t' '
        !/^#/ && !($1 == "path" && $2 == "variant") {
...
if [[ "$actual_keys" != "$expected_keys" \
    ...
    echo "error: Proxy Test262 classified vector drifted" >&2
...
if ! (
    ...
    echo "error: pinned QuickJS failed the Proxy manifest" >&2
    tail -n 80 "$oracle_log" >&2
    exit 1
fi
actual_oracle_variants=$(awk '/^Average memory statistics for [0-9]+ tests:$/ { print $5; found=1 } END { if (!found) exit 1 }' "$oracle_log")
if [[ "$actual_oracle_variants" != "$expected_oracle_variants" ]] \
    || awk '/FAILED/ { found=1 } END { exit !found }' "$oracle_log"; then
    echo "error: pinned QuickJS Proxy oracle vector drifted" >&2
    tail -n 80 "$oracle_log" >&2
    exit 1
fi

printf 'Proxy Test262 vector matches: %s/%s pass, %s unsupported, %s executable; QuickJS %s/%s\n' \
    "$expected_passes" "$expected_variants" "$expected_unsupported" \
    "$expected_runnable" "$expected_oracle_variants" "$expected_oracle_variants"
