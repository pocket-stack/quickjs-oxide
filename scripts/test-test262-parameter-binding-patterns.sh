#!/usr/bin/env bash
# Reproduce the dependency-audited R2z synchronous parameter BindingPattern gate.
...
baseline=tests/test262-parameter-binding-patterns-baseline.txt
manifest=tests/test262-parameter-binding-patterns.txt
profile=tests/test262-parameter-binding-patterns.conf
report=target/test262-parameter-binding-patterns.tsv
json_report=target/test262-parameter-binding-patterns.jsonl
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$expected_profile" != "1f25a0648044b6cb3027e23bc58032b2b2fc3517cd0a29b35d5e4d0844fc6e5e" \
    || "$expected_schema" != "test262-canonical-classified-v2" \
    || "$expected_mode" != "both" \
    || "$expected_timeout_ms" != "30000" \
    || "$expected_paths" != "149" \
    || "$expected_variants" != "298" \
    || "$expected_runnable" != "298" \
    || "$expected_passes" != "298" \
    || "$expected_failures" != "0" \
    || "$expected_unsupported" != "0" \
    || "$expected_skipped" != "0" ]]; then
    echo "error: parameter-BindingPattern baseline metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_paths" != "$expected_paths" || "$unique_paths" != "$expected_paths" ]]; then
    echo "error: parameter-BindingPattern manifest cardinality drifted" >&2
    exit 1
fi
...
if [[ "$actual_manifest" != "$expected_manifest" \
    ...
    echo "error: parameter-BindingPattern manifest content drifted" >&2
    exit 1
fi
if [[ "$(sha256_file "$profile")" != "$expected_profile" ]]; then
    echo "error: parameter-BindingPattern scoped capability profile drifted" >&2
    exit 1
fi
diff -u \
...
for surface in \
    ...
    if [[ "$count" != "37" ]]; then
        echo "error: parameter-BindingPattern generated surface drifted: $surface" >&2
        exit 1
    fi
done
...
while IFS= read -r test_path; do
    if [[ ! -f "$suite/$test_path" ]]; then
        echo "error: pinned parameter-BindingPattern path is missing: $test_path" >&2
        exit 1
    fi
done < <(awk 'NF && $1 !~ /^#/ { print }' "$manifest")
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: parameter-BindingPattern report metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_passes" != "$expected_passes" \
    ...
    echo "error: parameter-BindingPattern Test262 classified vector drifted" >&2
    awk -F'\t' '!/^#/ && !($1 == "path" && $2 == "variant") && $7 != "pass" { print $1 "\t" $2 "\t" $7 "\t" $8 "\t" $9 "\t" $10 }' "$report" >&2
    exit 1
fi

printf 'parameter-BindingPattern Test262 gate passes: %s/%s variants across %s paths\n' \
