#!/usr/bin/env bash
# Reproduce the dependency-audited R2u flat array assignment Test262 gate.
...
baseline=tests/test262-array-assignment-flat-baseline.txt
manifest=tests/test262-array-assignment-flat.txt
profile=tests/test262-array-assignment-flat.conf
report=target/test262-array-assignment-flat.tsv
json_report=target/test262-array-assignment-flat.jsonl
...
derive_manifest() {
    local test_file basename features feature admitted
    for test_file in "$suite"/test/language/expressions/assignment/dstr/array-*.js; do
        basename=${test_file##*/}
        ...
        for feature in "${feature_list[@]}"; do
            ...
            case "$feature" in
                Symbol|Symbol.iterator|const|destructuring-binding|let) ;;
                *) admitted=false ;;
            esac
        done
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$expected_profile" != "b2133d90974566c72ab788525254de68d260b44756a8c5981111873fb38727af" \
    || "$expected_schema" != "test262-canonical-classified-v2" \
    || "$expected_mode" != "both" \
    || "$expected_timeout_ms" != "30000" \
    || "$expected_paths" != "70" \
    || "$expected_variants" != "131" \
    || "$expected_runnable" != "131" \
    || "$expected_passes" != "131" \
    || "$expected_failures" != "0" \
    || "$expected_unsupported" != "0" \
    || "$expected_skipped" != "0" ]]; then
    echo "error: flat array assignment baseline metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_paths" != "$expected_paths" || "$unique_paths" != "$expected_paths" ]]; then
    echo "error: flat array assignment manifest cardinality drifted" >&2
    exit 1
fi
awk 'NF && $1 !~ /^#/ { print }' "$manifest" | LC_ALL=C sort -c
actual_manifest=$(awk 'NF && $1 !~ /^#/ { print }' "$manifest" | sha256_stream)
if [[ "$actual_manifest" != "$expected_manifest" ]]; then
    echo "error: flat array assignment manifest content drifted" >&2
    exit 1
fi
if [[ "$(sha256_file "$manifest")" != "$expected_manifest_file" ]]; then
    echo "error: flat array assignment manifest file drifted" >&2
    exit 1
fi
if [[ "$(sha256_file "$profile")" != "$expected_profile" ]]; then
    echo "error: flat array assignment scoped capability profile drifted" >&2
    exit 1
fi
diff -u \
...
while IFS= read -r test_path; do
    if [[ ! -f "$suite/$test_path" ]]; then
        echo "error: pinned flat array assignment path is missing: $test_path" >&2
        exit 1
    fi
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: flat array assignment report metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_keys" != "$expected_keys" ]]; then
    echo "error: flat array assignment path/variant key set drifted" >&2
    exit 1
fi
...
if [[ "$actual_passes" != "$expected_passes" \
    ...
    echo "error: flat array assignment Test262 classified vector drifted" >&2
...
printf 'flat array assignment Test262 gate passes: %s/%s variants across %s paths\n' \
