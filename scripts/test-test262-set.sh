#!/usr/bin/env bash
# Reproduce the dependency-audited R2o Set Test262 gate.
...
baseline=tests/test262-set-baseline.txt
manifest=tests/test262-set.txt
profile=tests/test262-set.conf
report=target/test262-set.tsv
json_report=target/test262-set.jsonl
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$expected_profile" != "6869e9d28fff1d5bd4e5b698dcdf6ee677b9134a91781ad7abe226200d669455" \
    || "$expected_schema" != "test262-canonical-classified-v2" \
    || "$expected_mode" != "both" \
    || "$expected_timeout_ms" != "30000" \
    || "$expected_paths" != "322" \
    || "$expected_variants" != "642" \
    || "$expected_runnable" != "642" \
    || "$expected_passes" != "642" \
    || "$expected_failures" != "0" \
    || "$expected_unsupported" != "0" \
    || "$expected_skipped" != "0" ]]; then
    echo "error: Set baseline metadata drifted" >&2
    exit 1
fi
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: Set report metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_keys" != "$expected_keys" ]]; then
    echo "error: Set path/variant key set drifted" >&2
    exit 1
fi

nonpass_count=$(awk -F'\t' '!/^#/ && !($1 == "path" && $2 == "variant") && $7 != "pass" { count++ } END { print count + 0 }' "$report")
actual_nonpass=$(awk -F'\t' '!/^#/ && !($1 == "path" && $2 == "variant") && $7 != "pass" { print $1 "\t" $2 "\t" $7 "\t" $8 "\t" $9 "\t" $10 }' "$report" | sha256_stream)
if [[ "$nonpass_count" != "$expected_failures" \
    ...
    echo "error: Set Test262 classified vector drifted" >&2
...
printf 'Set Test262 gate passes: %s/%s variants across %s paths\n' \
