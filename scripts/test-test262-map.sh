#!/usr/bin/env bash
# Reproduce the dependency-audited R2n Map Test262 gate.
...
baseline=tests/test262-map-baseline.txt
manifest=tests/test262-map.txt
profile=tests/test262-map.conf
report=target/test262-map.tsv
json_report=target/test262-map.jsonl
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$expected_paths" != "186" \
    || "$expected_variants" != "370" \
    || "$expected_runnable" != "370" \
    || "$expected_passes" != "370" \
    || "$expected_failures" != "0" \
    || "$expected_unsupported" != "0" \
    || "$expected_skipped" != "0" ]]; then
    echo "error: Map baseline metadata drifted" >&2
    exit 1
fi
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: Map report metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_keys" != "$expected_keys" ]]; then
    echo "error: Map path/variant key set drifted" >&2
    exit 1
fi
...
printf 'Map Test262 gate passes: %s/%s variants across %s paths\n' \
