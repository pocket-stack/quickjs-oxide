#!/usr/bin/env bash
# Reproduce the R3p complete Promise.all cohort.
...
baseline=tests/test262-promise-all-baseline.txt
manifest=tests/test262-promise-all.txt
admission_profile=tests/test262-promise-all.conf
global_profile=compat/test262-oxide.conf
report=target/test262-promise-all.tsv
json_report=target/test262-promise-all.jsonl
...
verify_inventory() {
    ...
    if [[ "$actual_count" != "$expected_count" || "$actual_hash" != "$expected_hash" ]]; then
        echo "error: Promise.all $name inventory drifted" >&2
        exit 1
    fi
}
...
verify_quickjs_oracle() {
    ...
    if ! output=$(cd -- "$source_dir" && ./run-test262 -a -m -c test262.conf -f "${files[@]}" 2>&1); then
        printf '%s\n' "$output" >&2
        echo "error: pinned QuickJS could not execute the Promise.all cohort" >&2
        exit 1
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])' <<<"$output" \
        || ! grep -Fq "Average memory statistics for $(read_value quickjs_passes) tests:" <<<"$output"; then
        printf '%s\n' "$output" >&2
        echo "error: pinned QuickJS no longer passes the Promise.all cohort" >&2
        exit 1
    fi
}
...
if [[ "$(read_value quickjs)" != "2026-06-04" \
    ...
    || "$(read_value paths)" != "98" \
    || "$(read_value variants)" != "196" \
    || "$(read_value quickjs_passes)" != "98" \
    || "$(read_value async_variants)" != "114" \
    || "$(read_value sync_variants)" != "82" \
    || "$(read_value proxy_variants)" != "0" \
    || "$(read_value host_variants)" != "0" ]]; then
    echo "error: Promise.all baseline identity drifted" >&2
    exit 1
fi

actual_paths=$(manifest_paths | wc -l | tr -d '[:space:]')
unique_paths=$(manifest_paths | LC_ALL=C sort -u | wc -l | tr -d '[:space:]')
[[ "$actual_paths" == "98" && "$unique_paths" == "98" ]] \
    || { echo "error: Promise.all manifest cardinality drifted" >&2; exit 1; }
...
[[ "$(manifest_paths | sha256_stream)" == "$(read_value manifest_sha256)" \
    ...
    || { echo "error: Promise.all manifest content drifted" >&2; exit 1; }

derived_manifest=$(git -C "$suite" ls-files \
    'test/built-ins/Promise/all/*.js' \
    | awk -F/ 'NF == 5' \
    | LC_ALL=C sort)
...
while IFS= read -r test_path; do
    [[ -f "$suite/$test_path" ]] \
        || { echo "error: missing Promise.all path: $test_path" >&2; exit 1; }
    metadata=$(metadata_block "$test_path")
    if grep -Fq 'negative:' <<<"$metadata"; then
        echo "error: Promise.all cohort unexpectedly gained a negative test: $test_path" >&2
        exit 1
    fi
    ...
    case "$flag_line" in
        ...
        "flags: [async]")
            ...
            ;;
        *)
            echo "error: Promise.all flags drifted: $test_path: $flag_line" >&2
            exit 1
            ;;
    esac
    variant_keys+=$'\n'"$test_path"$'\t'sloppy$'\n'"$test_path"$'\t'strict
    ...
    if grep -Eq 'Promise\.(allSettled|any|race)[[:space:]]*\(' "$suite/$test_path"; then
        echo "error: Promise.all cohort gained another aggregate dependency: $test_path" >&2
        exit 1
    fi
done < <(manifest_paths)
...
[[ "$(sha256_file "$global_profile")" == "$(read_value global_oxide_profile_sha256)" \
    ...
    || { echo "error: Promise.all capability profile drifted" >&2; exit 1; }
...
verify_quickjs_oracle
if "$check_only"; then
    printf 'Promise.all inputs verified: %s complete paths; QuickJS %s passes all; Oxide gate covers %s variants\n' \
        "$(read_value paths)" \
        "$(read_value quickjs)" \
        "$(read_value variants)"
    exit 0
...
if [[ ${#pending_keys[@]} -ne 0 ]]; then
    printf 'error: Promise.all Oxide baseline needs refresh before execution: %s\n' \
        "${pending_keys[*]}" >&2
    exit 1
fi
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: Promise.all report metadata drifted" >&2
    exit 1
fi
...
if [[ "$runner_summary" != "$expected_runner_summary" \
    ...
    echo "error: Promise.all Test262 classified vector drifted" >&2
    awk -F'\t' '
...
printf 'Promise.all Test262 gate is exact: %s/%s pass, %s expected failures across %s complete paths\n' \
