#!/usr/bin/env bash
# Reproduce the dependency-audited R2y synchronous identifier-default gate.
...
baseline=tests/test262-identifier-defaults-baseline.txt
manifest=tests/test262-identifier-defaults.txt
profile=tests/test262-identifier-defaults.conf
report=target/test262-identifier-defaults.tsv
json_report=target/test262-identifier-defaults.jsonl
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$expected_profile" != "5c98d19ccb72c7e2c577ddc98ee4ac83d43a0ba7d49175a8ebe271866d0feab6" \
    || "$expected_schema" != "test262-canonical-classified-v2" \
    || "$expected_mode" != "both" \
    || "$expected_timeout_ms" != "30000" \
    || "$expected_paths" != "76" \
    || "$expected_variants" != "143" \
    || "$expected_runnable" != "143" \
    || "$expected_passes" != "143" \
    || "$expected_failures" != "0" \
    || "$expected_unsupported" != "0" \
    || "$expected_skipped" != "0" ]]; then
    echo "error: identifier-default baseline metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_paths" != "$expected_paths" || "$unique_paths" != "$expected_paths" ]]; then
    echo "error: identifier-default manifest cardinality drifted" >&2
    exit 1
fi
...
if [[ "$actual_manifest" != "$expected_manifest" \
    ...
    echo "error: identifier-default manifest content drifted" >&2
    exit 1
fi
if [[ "$(sha256_file "$profile")" != "$expected_profile" ]]; then
    echo "error: identifier-default scoped capability profile drifted" >&2
    exit 1
fi
diff -u \
...
while IFS= read -r test_path; do
    if [[ ! -f "$suite/$test_path" ]]; then
        echo "error: pinned identifier-default path is missing: $test_path" >&2
        exit 1
    fi
    case "$test_path" in
        test/language/eval-code/direct/* | \
        */async-*/* | */generators/* | */class/* | \
        *accessor* | *getter* | *setter*)
            echo "error: identifier-default manifest admitted a later frontier: $test_path" >&2
            exit 1
            ;;
    esac
    if body_has_direct_eval "$test_path"; then
        echo "error: identifier-default manifest admitted direct eval: $test_path" >&2
        exit 1
    fi
done < <(awk 'NF && $1 !~ /^#/ { print }' "$manifest")
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: identifier-default report metadata drifted" >&2
    exit 1
fi
...
if [[ "$expected_keys" == PENDING_* || "$expected_tsv" == PENDING_* || "$expected_jsonl" == PENDING_* ]]; then
    echo "error: identifier-default report hashes await the verified R2y run" >&2
...
if [[ "$actual_passes" != "$expected_passes" \
    ...
    echo "error: identifier-default Test262 classified vector drifted" >&2
    awk -F'\t' '!/^#/ && !($1 == "path" && $2 == "variant") && $7 != "pass" { print $1 "\t" $2 "\t" $7 "\t" $8 "\t" $9 "\t" $10 }' "$report" >&2
    exit 1
fi

printf 'identifier-default Test262 gate passes: %s/%s variants across %s paths\n' \
