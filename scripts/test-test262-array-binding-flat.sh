#!/usr/bin/env bash
# Reproduce the dependency-audited R2q flat array binding Test262 gate.
...
baseline=tests/test262-array-binding-flat-baseline.txt
manifest=tests/test262-array-binding-flat.txt
profile=tests/test262-array-binding-flat.conf
report=target/test262-array-binding-flat.tsv
json_report=target/test262-array-binding-flat.jsonl
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$expected_profile" != "8232e2c11e908f7cbf5a9e0f34fbd5223a9551b49ae64647f2a72b2314bcaf84" \
    || "$expected_schema" != "test262-canonical-classified-v2" \
    || "$expected_mode" != "both" \
    || "$expected_timeout_ms" != "30000" \
    || "$expected_paths" != "90" \
    || "$expected_variants" != "180" \
    || "$expected_runnable" != "180" \
    || "$expected_passes" != "180" \
    || "$expected_failures" != "0" \
    || "$expected_unsupported" != "0" \
    || "$expected_skipped" != "0" ]]; then
    echo "error: flat array binding baseline metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_paths" != "$expected_paths" || "$unique_paths" != "$expected_paths" ]]; then
    echo "error: flat array binding manifest cardinality drifted" >&2
    exit 1
fi
awk 'NF && $1 !~ /^#/ { print }' "$manifest" | LC_ALL=C sort -c
actual_manifest=$(awk 'NF && $1 !~ /^#/ { print }' "$manifest" | sha256_stream)
if [[ "$actual_manifest" != "$expected_manifest" ]]; then
    echo "error: flat array binding manifest content drifted" >&2
    exit 1
fi
if [[ "$(sha256_file "$manifest")" != "$expected_manifest_file" ]]; then
    echo "error: flat array binding manifest file drifted" >&2
    exit 1
fi
if [[ "$(sha256_file "$profile")" != "$expected_profile" ]]; then
    echo "error: flat array binding scoped capability profile drifted" >&2
    exit 1
fi
while IFS= read -r test_path; do
    if [[ ! -f "$suite/$test_path" ]]; then
        echo "error: pinned flat array binding path is missing: $test_path" >&2
        exit 1
    fi
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: flat array binding report metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_keys" != "$expected_keys" ]]; then
    echo "error: flat array binding path/variant key set drifted" >&2
    exit 1
fi
...
if [[ "$actual_passes" != "$expected_passes" \
    ...
    echo "error: flat array binding Test262 classified vector drifted" >&2
...
printf 'flat array binding Test262 gate passes: %s/%s variants across %s paths\n' \
