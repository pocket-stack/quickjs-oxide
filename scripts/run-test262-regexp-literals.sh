#!/usr/bin/env bash
# Reproduce the complete classified outcome vector for the RegExp-literal slice.
...
baseline=tests/test262-regexp-literals-baseline.txt
manifest=tests/test262-regexp-literals.txt
report=target/test262-regexp-literals.tsv
json_report=target/test262-regexp-literals.jsonl
...
expected_r1a_full_tsv=$(read_value r1a_full_tsv_sha256)
expected_r1a_selected=$(read_value r1a_selected_sha256)
expected_r1a_variants=$(read_value r1a_variants)
expected_r1a_outcome=$(read_value r1a_outcome)
expected_r1a_detail=$(read_value r1a_detail)
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$timeout_ms" != "30000" \
    || "$expected_paths" != "48" \
    || "$expected_variants" != "96" \
    || "$expected_runnable" != "96" \
    || "$expected_passes" != "96" \
    || "$expected_manifest" != "9a6e0f88165514284b647847bc5af1c8f38d9dc15f9180641e3374e0ae3a08bc" \
    || "$expected_r1a_full_tsv" != "77147ada7e07de9f9f78dc75687f7c0fdf5891be954de9193824aa752046da05" \
    || "$expected_r1a_selected" != "77ea1b326480b21d98c98bc01736d948b3535e0426eca7081776e3596221fa06" \
    || "$expected_r1a_variants" != "96" \
    || "$expected_r1a_outcome" != "unsupported-parser" \
    || "$expected_r1a_detail" != "this literal form is not implemented yet" ]]; then
    echo "error: RegExp-literal R1a provenance metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_manifest_paths" != "$expected_paths" \
    ...
    echo "error: RegExp-literal manifest cardinality drifted" >&2
...
if ! awk 'NF && $1 !~ /^#/ { print }' "$manifest" | LC_ALL=C sort -c; then
    echo "error: RegExp-literal manifest is not bytewise sorted" >&2
    exit 1
fi
actual_manifest=$(awk 'NF && $1 !~ /^#/ { print }' "$manifest" | sha256_stream)
if [[ "$actual_manifest" != "$expected_manifest" ]]; then
    echo "error: RegExp-literal manifest content drifted" >&2
...
if [[ "$actual_quickjs" != "$expected_quickjs" \
    ...
    echo "error: RegExp-literal Test262 baseline metadata drifted" >&2
...
if ! diff -u \
    ...
    echo "error: RegExp-literal Test262 report keys drifted from the frozen manifest" >&2
    exit 1
fi

actual_summary=$(tail -n 1 "$report")
if [[ "$actual_summary" != "# summary $expected_summary" ]]; then
    echo "error: RegExp-literal Test262 classified summary drifted" >&2
...
if [[ "$expected_passes" == 0 ]]; then
    if [[ " $expected_summary " == *" pass="* ]]; then
        echo "error: zero-pass RegExp-literal baseline unexpectedly records a pass outcome" >&2
        exit 1
    fi
elif [[ " $expected_summary " != *" pass=$expected_passes "* ]]; then
    echo "error: RegExp-literal pass count is inconsistent with the pinned summary" >&2
    exit 1
fi
...
if [[ "$actual_tsv" != "$expected_tsv" || "$actual_jsonl" != "$expected_jsonl" ]]; then
    echo "error: RegExp-literal Test262 classified vector drifted" >&2
...
printf 'RegExp-literal Test262 vector matches: %s pass of %s variants across %s paths\n' \
