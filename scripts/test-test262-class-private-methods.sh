#!/usr/bin/env bash
# Reproduce the dependency-audited R3i ordinary private-method cohort.
...
baseline=tests/test262-class-private-methods-baseline.txt
manifest=tests/test262-class-private-methods.txt
admission_profile=tests/test262-class-private-methods.conf
global_profile=compat/test262-oxide.conf
report=target/test262-class-private-methods.tsv
json_report=target/test262-class-private-methods.jsonl
...
verify_quickjs_oracle() {
    ...
    if ! output=$(
        ...
        printf '%s\n' "$output" >&2
        echo "error: pinned QuickJS could not execute the private-method cohort" >&2
        exit 1
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])' <<<"$output" \
        || ! grep -Fq "Average memory statistics for $expected_quickjs_passes tests:" <<<"$output"; then
        printf '%s\n' "$output" >&2
        echo "error: pinned QuickJS no longer passes the complete private-method cohort" >&2
        exit 1
    fi
}
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$(read_value accessor_partition_paths)" != "305" \
    || "$(read_value accessor_partition_variants)" != "610" \
    || "$expected_paths" != "267" \
    || "$expected_variants" != "534" \
    || "$expected_quickjs_passes" != "267" \
    || "$(read_value positive_paths)" != "219" \
    || "$(read_value negative_paths)" != "48" \
    || "$(read_value manifest_sha256)" != "7ea0bbef5d3b5b27aa5e661574fbb0f53cc65fa785874bd1baabb1d83339b375" ]]; then
    echo "error: private-method baseline identity drifted" >&2
    exit 1
fi
...
if [[ "$actual_paths" != "$expected_paths" || "$unique_paths" != "$expected_paths" ]]; then
    echo "error: private-method manifest cardinality drifted" >&2
    exit 1
fi
...
if [[ "$actual_manifest" != "$(read_value manifest_sha256)" \
    ...
    echo "error: private-method manifest content drifted" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$tagged_inventory")" != "$(read_value tagged_paths)" \
    ...
    || "$(inventory_count "$accessor_inventory")" != "$(read_value accessor_partition_paths)" \
    || "$(inventory_sha256 "$accessor_inventory")" != "$(read_value accessor_partition_sha256)" ]]; then
    echo "error: private-method candidate, exclusion, or partition inventory drifted" >&2
    exit 1
fi

diff -u <(printf '%s\n' "$method_inventory") <(manifest_paths)
...
for category in eval dynamic-function proxy await-yield async-syntax generator-syntax static-block optional-chaining; do
    ...
    if [[ "$(inventory_count "$category_inventory")" != "$(read_value "${category_key}_excluded_paths")" \
        || "$(inventory_sha256 "$category_inventory")" != "$(read_value "${category_key}_excluded_sha256")" ]]; then
        echo "error: private-method $category exclusion inventory drifted" >&2
        exit 1
    fi
done
...
if [[ "$(sha256_file "$global_profile")" != "$expected_global_profile" \
    ...
    echo "error: private-method capability profile drifted" >&2
    exit 1
fi
if awk '
    ...
    END { exit !found }
' "$global_profile"; then
    echo "error: global Test262 profile must remain fail-closed for private methods" >&2
    exit 1
fi
...
while IFS= read -r test_path; do
    if [[ ! -f "$suite/$test_path" ]]; then
        echo "error: pinned private-method path is missing: $test_path" >&2
        exit 1
    fi
    metadata=$(metadata_block "$test_path")
    features=$(metadata_features "$test_path")
    if ! grep -q '^features: \[' <<<"$metadata" \
        ...
        echo "error: private-method feature scope drifted: $test_path" >&2
        exit 1
    fi
    ...
    case "$flag_line" in
        ...
        *)
            echo "error: private-method flags drifted: $test_path: $flag_line" >&2
            exit 1
            ;;
    esac
    ...
    if grep -Fq 'negative:' <<<"$metadata"; then
        if ! grep -Fq '  phase: parse' <<<"$metadata" \
            ...
            echo "error: private-method negative provenance drifted: $test_path" >&2
            exit 1
        fi
    ...
    case "$include_line" in
        ...
        "includes: [nativeFunctionMatcher.js]")
            ...
            ;;
        *)
            echo "error: private-method harness dependency drifted: $test_path" >&2
            exit 1
            ;;
    esac
    ...
    if [[ -n "$(printf '%s\n' "$test_path" | source_exclusion_records)" \
        ...
        echo "error: private-method manifest crossed a source or accessor frontier: $test_path" >&2
        exit 1
    fi
done < <(manifest_paths)
...
for pair in \
    ...
    if [[ "$actual" != "$(read_value "$key")" ]]; then
        echo "error: private-method metadata composition drifted for $key" >&2
        exit 1
    fi
done
...
if [[ "$(derive_positive_tests | sha256_stream)" != "$(read_value positive_sha256)" \
    ...
    echo "error: private-method positive or negative inventory drifted" >&2
    exit 1
fi

verify_quickjs_oracle
if "$check_only"; then
    printf 'private-method inputs verified: QuickJS %s passes %s paths; Oxide admission gate expands to %s variants\n' \
        "$expected_quickjs" "$expected_quickjs_passes" "$expected_variants"
    exit 0
fi

pending_keys=()
for key in passes failures unsupported skipped nonpass_sha256 tsv_sha256 jsonl_sha256 summary; do
    if [[ "$(read_value "$key")" == "PENDING" ]]; then
        pending_keys+=("$key")
    fi
done
if [[ ${#pending_keys[@]} -ne 0 ]]; then
    printf 'error: private-method Oxide baseline needs refresh before execution: %s\n' \
        "${pending_keys[*]}" >&2
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: private-method report metadata drifted" >&2
    exit 1
fi
...
if [[ "$runner_summary" != "$expected_runner_summary" ]]; then
    echo "error: private-method runner summary drifted: $runner_summary" >&2
    exit 1
fi
...
if [[ "$actual_passes" != "$expected_passes" \
    ...
    echo "error: private-method Test262 classified vector drifted" >&2
    awk -F'\t' '
...
printf 'private-method Test262 gate is exact: %s/%s pass across %s audited paths\n' \
