#!/usr/bin/env bash
# Reproduce the R3dr Test262 agent wake/FIFO global admission.
...
baseline=tests/test262-agent-wake-fifo-global-baseline.txt
predecessor_baseline=tests/test262-agent-wait-bounded-a-global-baseline.txt
predecessor_profile=tests/test262-agent-wait-bounded-a-global-candidate.conf
successor_baseline=tests/test262-is-html-dda-global-baseline.txt
successor_gate=scripts/test-test262-is-html-dda-global.sh
successor_parent_profile=tests/test262-is-html-dda-global-parent.conf
successor_global_profile=tests/test262-is-html-dda-global-candidate.conf
successor_scoped_profile=tests/test262-is-html-dda-scoped.conf
wake_scoped_baseline=tests/test262-agent-wake-count-location-baseline.txt
wake_scoped_gate=scripts/test-test262-agent-wake-count-location.sh
fifo_scoped_baseline=tests/test262-agent-fifo-wake-order-baseline.txt
fifo_scoped_gate=scripts/test-test262-agent-fifo-wake-order.sh
scoped_candidate_profile=tests/test262-agent-fifo-wake-order-candidate.conf
canonical_baseline=tests/test262-full-baseline.txt
upstream=compat/upstream.toml
live_profile=compat/test262-oxide.conf
parent_profile=tests/test262-agent-wake-fifo-global-parent.conf
candidate_profile=tests/test262-agent-wake-fifo-global-candidate.conf
manifest=tests/test262-agent-wake-count-location-universe.txt
wake_activation=tests/test262-agent-wake-count-location.txt
fifo_activation=tests/test262-agent-fifo-wake-order.txt
legacy_manifest=tests/test262-agent-wait-bounded-a-retained.txt
quickjs_receipt=tests/test262-agent-wake-fifo-global-quickjs-receipt.txt
parent_report=tests/test262-agent-wake-fifo-global-parent.tsv
candidate_report=tests/test262-agent-wake-fifo-global-candidate.tsv
transition=tests/test262-agent-wake-fifo-global-transitions.tsv
parent_replay=target/test262-agent-wake-fifo-global-parent-replay.tsv
candidate_replay=target/test262-agent-wake-fifo-global-candidate-replay.tsv
preferred_parent_full=target/test262-agent-wait-bounded-a-global-candidate-full.tsv
generated_parent_full=target/test262-agent-wake-fifo-global-parent-full.tsv
candidate_full=target/test262-agent-wake-fifo-global-candidate-full.tsv
candidate_full_repeat=target/test262-agent-wake-fifo-global-candidate-full-repeat.tsv
oracle_log=target/test262-agent-wake-fifo-global-quickjs.log
...
baseline_lines=134
baseline_sha=1ddec4eb616b59ff9c01f2de64f839404b24e3dfd7dfd724bf9ffb689106d295
successor_lines=129
successor_sha=05c3a2e2aab4051f0666b06aeeb756e666b82e75c93f8954f88ff78614ff76e3

usage() {
    printf 'usage: %s [--check|--full]\n' "${0##*/}"
    printf '  --check  authenticate profiles, focused receipts, canonical state, and QuickJS\n'
    printf '  default  additionally replay the exact 21-path focused transition\n'
...
wake_scoped_value() { value_from "$wake_scoped_baseline" "$1"; }
fifo_scoped_value() { value_from "$fifo_scoped_baseline" "$1"; }
...
json_result_projection() {
    ...
    awk -v report="$report" '
        function fail(message) {
            printf "error: R3dr JSONL projection %s: %s\n", report, message \
                >"/dev/stderr"
            failed=1
            exit 2
        }
...
make_transition() {
    ...
    {
        echo '# Exhaustive R3dr Test262 agent wake/FIFO global transition.'
...
check_profiles() {
    ...
    cmp -s "$candidate_profile" "$live_profile" \
        || die 'live profile is not byte-identical to the R3dr candidate'
    cmp -s "$parent_profile" "$predecessor_profile" \
        || die 'R3dr parent is not byte-identical to the R3do live profile'
    ...
    [[ "$(lines "$tmp/parent.features")" == "$(value profile_features)" \
        ...
        || die 'R3dr profile inventory drifted'
    ...
    [[ ! -s "$tmp/removed-agent-paths" ]] || die 'R3dr removed a prior agent admission'
    ...
    diff -u "$manifest" "$tmp/agent-delta"
    profile_section host-agent-tests "$scoped_candidate_profile" \
        >"$tmp/scoped-candidate.host-agent-tests"
    diff -u "$tmp/scoped-candidate.host-agent-tests" \
        "$tmp/candidate.host-agent-tests"
}

check_manifest_and_sources() {
    check_file "$legacy_manifest" "$(predecessor_value retained_paths)" \
        "$(predecessor_value retained_sha256)"
    check_file "$manifest" "$(value manifest_paths)" "$(value manifest_sha256)"
    check_file "$wake_activation" "$(value wake_activation_paths)" \
        "$(value wake_activation_sha256)"
    check_file "$fifo_activation" "$(value fifo_activation_paths)" \
        "$(value fifo_activation_sha256)"
    check_file "$quickjs_receipt" "$(value quickjs_receipt_lines)" \
        "$(value quickjs_receipt_sha256)"
    local file path
    cmp -s "$legacy_manifest" "$manifest" \
        || die 'R3dr universe is not byte-identical to the R3do retained manifest'
    for file in "$manifest" "$wake_activation" "$fifo_activation"; do
        sort -c "$file" || die "R3dr manifest is not bytewise sorted: $file"
        [[ -z "$(uniq -d "$file")" ]] || die "R3dr manifest contains duplicates: $file"
    done
    sort -u "$wake_activation" "$fifo_activation" >"$tmp/partition"
    diff -u "$manifest" "$tmp/partition"
    [[ -z "$(comm -12 "$wake_activation" "$fifo_activation")" \
        && "wake=$(lines "$wake_activation") fifo=$(lines "$fifo_activation")" == \
            "$(value manifest_partition)" ]] \
        || die 'R3dr manifest partition drifted'
    ...
    while IFS= read -r path; do
        printf '%s\t%s\n' "$path" "$(sha "$suite/$path")"
    done <"$manifest" >"$tmp/source-ledger.tsv"
...
check_receipts() {
    ...
    [[ "$(report_runnable "$parent_report")" == "$(value parent_runnable)" \
        ...
        || die 'R3dr focused outcome counts drifted'

    rows_for_paths "$manifest" "$parent_report" >"$tmp/parent.activation"
    rows_for_paths "$manifest" "$candidate_report" >"$tmp/candidate.activation"
    [[ "$(lines "$tmp/parent.activation")" == "$(value manifest_variants)" \
        && "$(lines "$tmp/candidate.activation")" == "$(value manifest_variants)" ]] \
        || die 'R3dr focused report partition drifted'
    awk -F'\t' '{if($7!="unsupported-host-agent"||$8!="selection"||
        $9!="HostCapability"||$10!="missing execution capabilities: agent")exit 2}' \
        "$tmp/parent.activation" || die 'R3dr parent activation frontier drifted'
    awk -F'\t' '{if($7!="pass"||$8!="normal"||$9!=""||$10!="")exit 2}' \
        "$tmp/candidate.activation" || die 'R3dr candidate activation drifted'
    ...
    [[ "$(header "$transition" parent_commit)" == "$(value parent_commit)" \
        ...
        || die 'R3dr focused transition drifted'
}
...
check_history_and_canonical() {
    ...
    check_file "$wake_scoped_baseline" "$(value wake_scoped_baseline_lines)" \
        "$(value wake_scoped_baseline_sha256)"
    check_file "$wake_scoped_gate" "$(value wake_scoped_gate_lines)" \
        "$(value wake_scoped_gate_sha256)"
    check_file "$fifo_scoped_baseline" "$(value fifo_scoped_baseline_lines)" \
        "$(value fifo_scoped_baseline_sha256)"
    check_file "$fifo_scoped_gate" "$(value fifo_scoped_gate_lines)" \
        "$(value fifo_scoped_gate_sha256)"
    check_file "$scoped_candidate_profile" "$(value scoped_candidate_profile_lines)" \
        "$(value scoped_candidate_profile_sha256)"
    ...
    git cat-file -e "${parent_commit}^{commit}" 2>/dev/null \
        || die "R3dr parent commit is unavailable: $parent_commit"
    git show "${parent_commit}:${live_profile}" >"$parent_snapshot" 2>/dev/null \
        || die "R3dr parent commit has no $live_profile snapshot"
    cmp -s "$parent_profile" "$parent_snapshot" \
        || die 'R3dr parent profile does not match its recorded commit'
    ...
    git show "${parent_commit}:${canonical_baseline}" >"$parent_canonical_snapshot" 2>/dev/null \
        || die "R3dr parent commit has no $canonical_baseline snapshot"
    ...
    [[ "$(predecessor_value candidate_profile_sha256)" == "$(value parent_profile_sha256)" \
        ...
        && "$(wake_scoped_value universe_sha256)" == "$(value manifest_sha256)" \
        && "$(wake_scoped_value activation_sha256)" == "$(value wake_activation_sha256)" \
        && "$(wake_scoped_value retained_sha256)" == "$(value fifo_activation_sha256)" \
        && "$(fifo_scoped_value universe_sha256)" == "$(value fifo_activation_sha256)" \
        && "$(fifo_scoped_value candidate_profile_sha256)" == \
            "$(value scoped_candidate_profile_sha256)" \
        && "$(fifo_scoped_value candidate_agent_allowlist_sha256)" == \
            "$(value candidate_agent_allowlist_sha256)" \
        && "$(( $(wake_scoped_value candidate_passes) + $(fifo_scoped_value candidate_passes) ))" == \
        ...
        && "$(toml_test262_value oxide_profile_sha256)" == \
            "$(value candidate_profile_sha256)" ]] \
        || die 'R3dr predecessor, scoped, canonical, or upstream bridge drifted'
}
...
verify_profile_handshake() {
    ...
    for label in parent candidate; do
        ...
        if ! cmp -s "$expected" "$tmp/$label.tsv" \
            ...
            die "R3dr runner failed the exact $label profile handshake"
        fi
        for rejected in "$wake_activation" "$fifo_activation" "$legacy_manifest" Cargo.toml; do
            if "$runner" --suite "$suite" --config "$source_dir/test262.conf" \
                ...
                >/dev/null 2>&1; then
                die "R3dr $label profile accepted a non-canonical manifest: $rejected"
            fi
        done
        if "$runner" --suite "$suite" --config "$source_dir/test262.conf" \
            ...
            --timeout-ms "$(value timeout_ms)" --workers 1 --allow-failures \
            >/dev/null 2>&1; then
            die "R3dr $label profile accepted --test"
        fi
    done
...
verify_quickjs() {
    ...
    [[ "${#files[@]}" == "$(value quickjs_paths)" ]] \
        || die 'R3dr QuickJS path vector drifted'
    if ! (cd -- "$source_dir" && \
        ./run-test262 -m -c test262.conf -a -T "$workers" -f "${files[@]}") \
        >"$root/$oracle_log" 2>&1; then
        tail -n 100 "$oracle_log" >&2
        die 'pinned QuickJS could not execute the R3dr activation'
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])|SKIPPED FEATURE' "$oracle_log" \
        || ! grep -Fq "Average memory statistics for $(value quickjs_variants) tests:" \
            "$oracle_log"; then
        tail -n 100 "$oracle_log" >&2
        die 'pinned QuickJS no longer passes the R3dr wake/FIFO union'
    fi
    {
        echo '# Pinned QuickJS oracle receipt for the R3dr Test262 agent wake/FIFO union.'
        echo "quickjs=$(value quickjs)"
        echo "quickjs_source_sha256=$(value quickjs_source_sha256)"
        echo "test262=$(value test262)"
        echo "activation_manifest_sha256=$(value manifest_sha256)"
    ...
    cmp -s "$quickjs_receipt" "$tmp/quickjs-receipt.txt" \
        || die 'pinned QuickJS R3dr receipt drifted'
}
...
verify_full_join() {
    ...
    rows_for_paths "$manifest" "$parent" >"$tmp/$label.parent.scope"
    rows_for_paths "$manifest" "$candidate" >"$tmp/$label.candidate.scope"
    rows_without_paths "$manifest" "$parent" >"$tmp/$label.parent.outside"
    rows_without_paths "$manifest" "$candidate" >"$tmp/$label.candidate.outside"
    json_rows_for_paths "$manifest" "$parent_json" >"$tmp/$label.parent.scope.json"
    json_rows_for_paths "$manifest" "$candidate_json" >"$tmp/$label.candidate.scope.json"
    json_rows_without_paths "$manifest" "$parent_json" >"$tmp/$label.parent.outside.json"
    json_rows_without_paths "$manifest" "$candidate_json" >"$tmp/$label.candidate.outside.json"
    rows_for_paths "$manifest" "$parent_report" >"$tmp/focused.parent"
    rows_for_paths "$manifest" "$candidate_report" >"$tmp/focused.candidate"
    json_rows_for_paths "$manifest" "${parent_report%.tsv}.jsonl" \
        >"$tmp/focused.parent.json"
    json_rows_for_paths "$manifest" "${candidate_report%.tsv}.jsonl" \
    ...
    [[ "$(lines "$tmp/$label.parent.scope")" == "$(value full_scope_rows)" \
        ...
        || die "R3dr $label full partition row counts drifted"
    ...
    ' "$parent" "$candidate") || die "R3dr $label exact full join failed"
    ...
    [[ "$counts" == "$expected" ]] || die "R3dr $label full transition drifted: $counts"
}
...
replay_full() {
    ...
    if ! cmp -s "$candidate_full" "$candidate_full_repeat" \
        ...
        die 'R3dr independent candidate full runs are not byte-identical'
    fi
...
bridge_r3ds_successor() {
    ...
    check_file "$candidate_report" "$(value candidate_report_lines)" \
        "$(value candidate_tsv_sha256)"
    check_file "${candidate_report%.tsv}.jsonl" \
        "$(value candidate_jsonl_lines)" "$(value candidate_jsonl_sha256)"
    ...
    check_file "$successor_parent_profile" \
        ...
        "$(successor_value historical_parent_oxide_profile_sha256)"
    check_file "$successor_global_profile" \
        "$(successor_value global_candidate_profile_lines)" \
        "$(successor_value global_candidate_oxide_profile_sha256)"
    check_file "$successor_scoped_profile" \
        "$(successor_value scoped_candidate_profile_lines)" \
        "$(successor_value scoped_candidate_oxide_profile_sha256)"
    ...
    [[ -x "$successor_gate" \
        ...
        && "$(successor_value parent_commit)" \
            == de52c950c9cf5ad8aea562005e89a96bc02d45c5 \
        && "$(successor_value historical_parent_oxide_profile_sha256)" \
            == "$(value candidate_profile_sha256)" \
        && "$(successor_value runtime_parent_oxide_profile_sha256)" \
            == "$(value candidate_profile_sha256)" \
        && "$(successor_value full_variants)" == "$(value full_variants)" \
        && "$(successor_value full_keys_sha256)" == "$(value full_keys_sha256)" \
        && "$(successor_value historical_full_runnable)" \
            == "$(value candidate_full_runnable)" \
        && "$(successor_value historical_full_passes)" \
            == "$(value candidate_full_passes)" \
        && "$(successor_value historical_full_tsv_sha256)" \
            == "$(value candidate_full_tsv_sha256)" \
        && "$(successor_value historical_full_jsonl_sha256)" \
            == "$(value candidate_full_jsonl_sha256)" \
        && "$(successor_value historical_full_summary)" \
            == "$(value candidate_full_summary)" \
        && "$(successor_value global_candidate_full_runnable)" \
            == "$(canonical_value runnable)" \
        && "$(successor_value global_candidate_full_passes)" \
            == "$(canonical_value passes)" \
        && "$(successor_value global_candidate_full_tsv_sha256)" \
            == "$(canonical_value tsv_sha256)" \
        && "$(successor_value global_candidate_full_jsonl_sha256)" \
            == "$(canonical_value jsonl_sha256)" \
        && "$(successor_value global_candidate_full_summary)" \
            == "$(canonical_value summary)" \
        && "$(successor_value full_transition_counts)" \
            == 'changed=84 outcome=80 detail=4 unchanged=101953 pass_gains=80 pass_regressions=0' \
        && "$(successor_value candidate_full_replay_status)" == passed-twice \
        && "$(successor_value candidate_full_replays)" == 2 ]] \
        || die 'R3ds successor does not checksum-bridge the historical R3dr receipt'
    cmp -s "$candidate_profile" "$successor_parent_profile" \
        || die 'R3ds parent profile is not byte-identical to the R3dr candidate'
    cmp -s "$successor_global_profile" "$live_profile" \
        || die 'live profile is not byte-identical to the R3ds global candidate'
    case $mode in
        check) "$successor_gate" --check ;;
        focused) "$successor_gate" ;;
        full) "$successor_gate" --full ;;
    esac
    echo 'Historical R3dr agent wake/FIFO receipt is checksum-bridged through the R3ds IsHTMLDDA admission.'
    exit 0
}

cd -- "$root"
bridge_r3ds_successor
tmp=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3dr-global.XXXXXX")
...
if [[ "$mode" == full ]]; then
    printf 'R3dr agent wake/FIFO full: 42 new passes, 101995 outside rows unchanged, two byte-identical runs\n'
else
    printf 'R3dr agent wake/FIFO global: 42/42 activated pass, zero retained agent diagnostics\n'
fi
