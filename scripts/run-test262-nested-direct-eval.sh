#!/usr/bin/env bash
# Reproduce the classified recursive direct-eval environment vector.
...
baseline=tests/test262-nested-direct-eval-baseline.txt
manifest=tests/test262-nested-direct-eval.txt
report=target/test262-nested-direct-eval.tsv
json_report=target/test262-nested-direct-eval.jsonl
...
cd -- "$root"
if [[ ! -f "$baseline" ]]; then
    echo "error: nested-direct-eval Test262 baseline is missing: $baseline" >&2
    exit 1
fi
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$timeout_ms" != "30000" \
    || "$expected_paths" != "25" \
    || "$expected_variants" != "30" ]]; then
    echo "error: nested-direct-eval baseline metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_manifest_paths" != "$expected_paths" \
    ...
    echo "error: nested-direct-eval manifest cardinality drifted" >&2
    exit 1
fi
if ! awk 'NF && $1 !~ /^#/ { print }' "$manifest" | LC_ALL=C sort -c; then
    echo "error: nested-direct-eval manifest is not bytewise sorted" >&2
    exit 1
fi
actual_manifest=$(awk 'NF && $1 !~ /^#/ { print }' "$manifest" | sha256_stream)
if [[ "$actual_manifest" != "$expected_manifest" ]]; then
    echo "error: nested-direct-eval manifest content drifted" >&2
    exit 1
fi
...
if [[ "$actual_quickjs" != "$expected_quickjs" \
    ...
    echo "error: nested-direct-eval Test262 report metadata drifted" >&2
    exit 1
fi
...
if ! diff -u \
    ...
    echo "error: nested-direct-eval report paths drifted from the frozen manifest" >&2
    exit 1
fi
...
if [[ "$actual_keys_hash" != "$expected_keys_hash" \
    ...
    echo "error: nested-direct-eval classified outcomes drifted" >&2
    exit 1
fi
...
if [[ "$actual_jsonl_lines" != "$expected_jsonl_lines" \
    ...
    echo "error: nested-direct-eval classified TSV/JSONL vector drifted" >&2
    exit 1
fi

"$script_dir/check-rust-only.sh"
printf 'nested-direct-eval Test262 vector matches: %s pass of %s variants across %s paths; one independent with-statement frontier remains\n' \
