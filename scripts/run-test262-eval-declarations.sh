#!/usr/bin/env bash
# Reproduce the classified eval var/function declaration vector.
...
baseline=tests/test262-eval-declarations-baseline.txt
manifest=tests/test262-eval-declarations.txt
report=target/test262-eval-declarations.tsv
json_report=target/test262-eval-declarations.jsonl
...
cd -- "$root"
if [[ ! -f "$baseline" ]]; then
    echo "error: eval-declarations Test262 baseline is missing: $baseline" >&2
    exit 1
fi
...
expected_quickjs_known_failures=$(read_value quickjs_known_failures)
expected_quickjs_known_failures_hash=$(read_value quickjs_known_failures_sha256)

known_quickjs_failure_paths() {
    cat <<'EOF'
...
test/staging/sm/Function/function-name-for.js
test/staging/sm/lexical-environment/var-in-catch-body-annex-b-eval.js
EOF
}

actual_quickjs_known_failures=$(known_quickjs_failure_paths | wc -l | tr -d '[:space:]')
actual_quickjs_known_failures_hash=$(known_quickjs_failure_paths | LC_ALL=C sort | sha256_stream)
while IFS= read -r known_failure; do
    if [[ ! -f "$suite/$known_failure" ]]; then
        echo "error: pinned QuickJS known-failure path is missing: $known_failure" >&2
        exit 1
    fi
done < <(known_quickjs_failure_paths)
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$expected_profile" != "0f4617ff1678710c97620aa1257c4868b2a4daf0f4f917f9d7393566ee549c45" \
    || "$expected_schema" != "test262-canonical-classified-v2" \
    || "$expected_mode" != "both" \
    || "$timeout_ms" != "30000" \
    || "$expected_paths" != "497" \
    || "$expected_variants" != "519" \
    || "$expected_quickjs_known_failures" != "15" \
    || "$actual_quickjs_known_failures" != "$expected_quickjs_known_failures" \
    || "$actual_quickjs_known_failures_hash" != "$expected_quickjs_known_failures_hash" ]]; then
    echo "error: eval-declarations baseline metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_manifest_paths" != "$expected_paths" \
    ...
    echo "error: eval-declarations manifest cardinality drifted" >&2
    exit 1
fi
if ! awk 'NF && $1 !~ /^#/ { print }' "$manifest" | LC_ALL=C sort -c; then
    echo "error: eval-declarations manifest is not bytewise sorted" >&2
    exit 1
fi
actual_manifest=$(awk 'NF && $1 !~ /^#/ { print }' "$manifest" | sha256_stream)
if [[ "$actual_manifest" != "$expected_manifest" ]]; then
    echo "error: eval-declarations manifest content drifted" >&2
    exit 1
fi
known_failure_overlap=$(comm -12 \
    <(awk 'NF && $1 !~ /^#/ { print }' "$manifest" | LC_ALL=C sort) \
    <(known_quickjs_failure_paths | LC_ALL=C sort))
if [[ -n "$known_failure_overlap" ]]; then
    echo "error: pinned QuickJS collateral failures leaked into the declaration gate" >&2
    exit 1
fi
...
if [[ "$actual_quickjs" != "$expected_quickjs" \
    ...
    echo "error: eval-declarations Test262 baseline metadata drifted" >&2
    exit 1
fi
...
if ! diff -u \
    ...
    echo "error: eval-declarations report paths drifted from the frozen manifest" >&2
    exit 1
fi
actual_keys_hash=$(awk -F'\t' '!/^#/ && !($1 == "path" && $2 == "variant") { print $1 "\t" $2 }' "$report" | LC_ALL=C sort | sha256_stream)
if [[ "$actual_keys_hash" != "$expected_keys_hash" ]]; then
    echo "error: eval-declarations report keys drifted" >&2
    exit 1
fi
...
if [[ "$actual_summary" != "# summary $expected_summary" \
    ...
    echo "error: eval-declarations classified outcomes drifted" >&2
    exit 1
fi
...
if [[ "$actual_jsonl_lines" != "$expected_jsonl_lines" \
    ...
    echo "error: eval-declarations classified TSV/JSONL vector drifted" >&2
    exit 1
fi

"$script_dir/check-rust-only.sh"
printf 'eval-declarations Test262 vector matches: %s pass of %s variants across %s paths; %s pinned QuickJS collateral failures remain separately classified\n' \
    "$expected_passes" "$expected_variants" "$expected_paths" "$expected_quickjs_known_failures"
