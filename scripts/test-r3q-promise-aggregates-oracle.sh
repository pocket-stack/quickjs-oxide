#!/usr/bin/env bash
# Freeze pinned QuickJS 2026-06-04 Promise.allSettled/Promise.any behavior and
...
fixture=$root/tests/fixtures/r3q_promise_aggregates.js
expected=$root/tests/fixtures/r3q_promise_aggregates.quickjs-2026-06-04.txt
fixture_sha256=e053bb7944943607b9a29ef15fd34d44a58c44792afaf5193e6b757f4231d8c4
expected_sha256=992d7e26fa681747b67c49a6cfd296c22ae54a558f1d8a86d70ce9eeea3a71e9
oxide=${OXIDE_QJS:-}
...
verify_hash() {
    ...
    local actual
    if [[ ! -f "$path" ]]; then
        echo "error: R3q oracle input is missing: $path" >&2
        exit 1
    fi
    actual=$(sha256_file "$path")
    if [[ "$actual" != "$pinned" ]]; then
        echo "error: R3q oracle input hash drifted: $path" >&2
...
run_engine() {
    ...
    if ! "$engine" "$@" >"$output" 2>"$errors"; then
        echo "error: $label failed to execute the R3q oracle fixture" >&2
...
compare_transcript() {
    ...
    if ! cmp -s -- "$expected" "$actual"; then
        echo "error: $label R3q transcript drifted" >&2
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3q-oracle.XXXXXX")
trap 'rm -rf -- "$tmp_dir"' EXIT HUP INT TERM

{
    sed '$d' "$fixture"
    printf '\nr3qDone.then(\n'
    printf '    function () { print(r3qTranscript.join("\\n")); },\n'
...
if [[ -n "$oxide" ]]; then
    ...
    echo "R3q Promise aggregate differential passed: quickjs-oxide matches QuickJS 2026-06-04"
else
    echo "R3q Promise aggregate oracle passed: pinned QuickJS 2026-06-04 transcript is stable"
fi
