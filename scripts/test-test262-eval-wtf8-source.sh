#!/usr/bin/env bash
# Reproduce the R3cu dynamic-eval WTF-8 source milestone.
...
baseline=tests/test262-eval-wtf8-source-baseline.txt
predecessor_baseline=tests/test262-regexp-v-character-class-escapes-baseline.txt
canonical_baseline=tests/test262-full-baseline.txt
successor_baseline=tests/test262-array-flatten-global-baseline.txt
successor_gate=scripts/test-test262-array-flatten-global.sh
upstream=compat/upstream.toml
profile=compat/test262-oxide.conf
manifest=tests/test262-eval-wtf8-source.txt
...
parent_report=tests/test262-eval-wtf8-source-parent.tsv
candidate_report=tests/test262-eval-wtf8-source-candidate.tsv
transition=tests/test262-eval-wtf8-source-transitions.tsv
candidate_output=target/test262-eval-wtf8-source-replay.tsv
parent_output=target/test262-eval-wtf8-source-parent-replay.tsv
candidate_full=target/test262-eval-wtf8-source-full.tsv
preferred_parent_full=${TEST262_EVAL_WTF8_PARENT_FULL:-target/test262-regexp-v-character-class-escapes-full.tsv}
generated_parent_full=target/test262-eval-wtf8-source-parent-full.tsv
oracle_log=target/test262-eval-wtf8-source-quickjs.log
...
parent_runner_override=${TEST262_EVAL_WTF8_PARENT_RUNNER:-}

baseline_lines=85
baseline_sha=d48300712eb54c05946ec7adb2117682621aab2e340a779d297f2eb425a20d1a
predecessor_lines=83
predecessor_sha=4605dcdb55debc98b89ff60253f79e4c4f649b738c83a813f85dc46197cfb738
canonical_lines=8
canonical_sha=40970d176b261b7e14bddac96a71aa4b5a8d320e9075c7db98780ed84902d362
successor_lines=92
successor_sha=e0cfd8b8250308e5f2658372c4703bf114b411a983db0d8d297b06175d68754f
...
verify_recovery_report() {
    ...
    [[ -f "$recovery_report" && -f "$json" \
        ...
        || die 'R3ct canonical recovery receipt drifted'
}
...
make_transition() {
    ...
    {
        echo '# Exhaustive R3cu eval WTF-8 source transition.'
...
check_manifest_and_metadata() {
    ...
    sort -c "$manifest" || die 'R3cu manifest is not bytewise sorted'
    [[ -z "$(uniq -d "$manifest")" ]] || die 'R3cu manifest contains duplicates'
    ...
    [[ "$(lines "$tmp/manifest.keys")" == "$(value manifest_variants)" \
        ...
        || die 'R3cu manifest variant keys drifted'

    local test_path metadata
    while IFS= read -r test_path; do
        [[ -f "$suite/$test_path" ]] || die "pinned R3cu test is missing: $test_path"
        metadata=$(sed -n '/\/\*---/,/---\*\//p' "$suite/$test_path")
        ! grep -Eq '^(negative|locale):' <<<"$metadata" \
            || die "R3cu test gained unexpected metadata: $test_path"
        grep -Eq 'eval[[:space:]]*\(' "$suite/$test_path" \
            || die "R3cu test no longer exercises dynamic eval: $test_path"
    done <"$manifest"

    check_file "$recovery_manifest" "$(value canonical_recovery_paths)" \
        "$(value canonical_recovery_manifest_sha256)"
    sort -c "$recovery_manifest" || die 'R3ct recovery manifest is not sorted'
    [[ -z "$(uniq -d "$recovery_manifest")" ]] \
        || die 'R3ct recovery manifest contains duplicates'
    [[ -z "$(comm -12 "$manifest" "$recovery_manifest")" ]] \
        || die 'R3cu runtime and R3ct recovery manifests overlap'
    ...
    [[ "$(lines "$tmp/recovery.keys")" == "$(value canonical_recovery_variants)" \
        ...
        || die 'R3ct recovery keys drifted'
}
...
check_profile() {
    ...
    [[ "$(lines "$tmp/profile.features")" == "$(value profile_features)" \
        ...
        || die 'R3cu live Test262 profile drifted'
}
...
check_static_inputs() {
    ...
    check_file "$parent_report" 33 "$(value parent_focused_tsv_sha256)"
    check_file "${parent_report%.tsv}.jsonl" 24 "$(value parent_focused_jsonl_sha256)"
    check_file "$candidate_report" 33 "$(value candidate_focused_tsv_sha256)"
    check_file "${candidate_report%.tsv}.jsonl" 24 "$(value candidate_focused_jsonl_sha256)"
    check_file "$transition" "$(value transition_lines)" "$(value transition_sha256)"
    check_file "$recovery_report" 41 "$(value canonical_recovery_tsv_sha256)"
    check_file "${recovery_report%.tsv}.jsonl" 32 "$(value canonical_recovery_jsonl_sha256)"
    ...
    [[ "$(header "$transition" parent_commit)" == "$(value parent_commit)" \
        ...
        || die 'R3cu receipt or upstream binding drifted'
    ...
    [[ "$(predecessor_value profile_sha256)" == "$(value profile_sha256)" \
        ...
        && "$(predecessor_value canonical_recovery_manifest_sha256)" == "$(value canonical_recovery_manifest_sha256)" \
        && "$(predecessor_value canonical_recovery_rows_sha256)" == "$(value canonical_recovery_rows_sha256)" \
        && "$(predecessor_value canonical_recovery_tsv_sha256)" == "$(value canonical_recovery_tsv_sha256)" \
        && "$(predecessor_value canonical_recovery_jsonl_sha256)" == "$(value canonical_recovery_jsonl_sha256)" \
        ...
        || die 'R3cu full-vector anchors drifted'
}
...
verify_focused_semantics() {
    [[ "$(report_runnable "$parent_report")" == "$(value parent_focused_runnable)" \
        ...
        || die 'R3cu focused outcome counts drifted'
    awk -F'\t' '!/^#/&&!($1=="path"&&$2=="variant")&&
        !($2~/^(sloppy|strict)$/&&$3==""&&($4==""||$4=="regexp-named-groups")&&
          $5=="normal"&&$6==""&&$7=="unsupported-runtime"&&$8=="runtime"&&
          $9=="Unsupported"&&$10=="eval source containing an unpaired UTF-16 surrogate is not implemented yet"){exit 2}' \
        "$parent_report" || die 'R3cu parent WTF-8 frontier drifted'
    awk -F'\t' '!/^#/&&!($1=="path"&&$2=="variant")&&
        !($2~/^(sloppy|strict)$/&&$3==""&&($4==""||$4=="regexp-named-groups")&&
          $5=="normal"&&$6==""&&$7=="pass"&&$8=="normal"&&$9==""&&$10==""){exit 2}' \
        "$candidate_report" || die 'R3cu candidate semantics drifted'
}
...
verify_quickjs() {
    ...
    if ! (cd -- "$source_dir" && \
        ...
        die 'pinned QuickJS could not execute the R3cu universe'
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])|SKIPPED FEATURE' "$oracle_log" \
        ...
        tail -n 100 "$oracle_log" >&2
        die 'pinned QuickJS no longer passes the R3cu universe'
    fi
}
...
reconstruct_parent_full() {
    ...
    [[ -f "$candidate" && -f "$candidate_json" ]] \
        || die 'cannot reconstruct the R3ct parent without a candidate full vector'
    ...
    ' "$parent_report" "$candidate" >"$output" \
        || die 'could not reconstruct the R3ct parent TSV'
    ...
    ' "${parent_report%.tsv}.jsonl" "$candidate_json" >"$output_json" \
        || die 'could not reconstruct the R3ct parent JSONL'
}

join_candidate_full() {
    local parent=$1 output=$2
    local parent_json=${parent%.tsv}.jsonl output_json=${output%.tsv}.jsonl
    awk -F'\t' -v replacement="$candidate_report" \
        -v summary="# summary $(value candidate_full_summary)" '
        FILENAME==replacement{
            if(!/^#/&&!($1=="path"&&$2=="variant"))new[$1 FS $2]=$0
            next
        }
        /^# summary /{print summary;next}
        !/^#/&&!($1=="path"&&$2=="variant"){
            key=$1 FS $2;if(key in new){print new[key];seen[key]=1;next}
        }
        {print}
        END{for(key in new)if(!(key in seen))exit 2}
    ' "$candidate_report" "$parent" >"$output" \
        || die 'could not join the R3cu candidate TSV'

    awk -v replacement="${candidate_report%.tsv}.jsonl" \
        -v summary="$(summary_json candidate_full)" '
        function field(line,name,value){
            value=line;sub(".*\\\"" name "\\\":\\\"","",value);sub("\\\".*","",value);return value
        }
        FILENAME==replacement{
            if($0~/^\{"kind":"result"/){key=field($0,"path") SUBSEP field($0,"variant");new[key]=$0}
            next
        }
        /^\{"kind":"summary"/{print summary;next}
        /^\{"kind":"result"/{
            key=field($0,"path") SUBSEP field($0,"variant")
            if(key in new){print new[key];seen[key]=1;next}
        }
        {print}
        END{for(key in new)if(!(key in seen))exit 2}
    ' "${candidate_report%.tsv}.jsonl" "$parent_json" >"$output_json" \
        || die 'could not join the R3cu candidate JSONL'
}
...
verify_full_join() {
    ...
    [[ "$(lines "$tmp/full-parent-universe.rows")" == "$(value full_universe_rows)" \
        ...
        || die 'R3cu full universe partition drifted'
    ...
    [[ "$(lines "$tmp/full-parent-recovery.rows")" == "$(value canonical_recovery_variants)" \
        ...
        || die 'R3ct canonical recovery does not bind both R3cu full vectors'
    ...
    ' "$parent" "$candidate") || die 'R3cu full exact join failed'
    ...
    [[ "$counts" == "$expected" ]] || die "R3cu full transition drifted: $counts"
}

bridge_r3cv_successor() {
    [[ -f "$profile" ]] || return 0
    [[ "$(sha "$profile")" != "$(value profile_sha256)" ]] || return 0
    check_file "$baseline" "$baseline_lines" "$baseline_sha"
    ...
    [[ -x "$successor_gate" \
        ...
        && "$(successor_value parent_commit)" == 798e8888dc4a29bce8ddd83674d94cd83628671c \
        && "$(successor_value parent_profile_sha256)" == "$(value profile_sha256)" \
        ...
        && "$(successor_value manifest_paths)" == 35 \
        && "$(successor_value manifest_variants)" == 69 \
        && "$(successor_value full_changed)" == 69 \
        && "$(successor_value full_outcome_changed)" == 69 \
        && "$(successor_value full_detail_only)" == 0 \
        && "$(successor_value full_unchanged)" == 101968 \
        && "$(successor_value full_pass_regressions)" == 0 ]] \
        || die 'R3cv successor does not checksum-bridge the historical R3cu receipt'
    case $mode in
        check) "$successor_gate" --check ;;
        focused) "$successor_gate" ;;
        full) "$successor_gate" --full ;;
    esac
    echo 'Historical R3cu eval receipt is checksum-bridged through the R3cv Array flatten admission.'
    exit 0
}

cd -- "$root"
bridge_r3cv_successor
tmp=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-eval-wtf8.XXXXXX")
...
verify_quickjs
if [[ "$mode" == check ]]; then
    echo 'R3cu inputs verified: 11 paths, 22 variants, QuickJS 22/22, checksum-bound parent/candidate receipts.'
    exit 0
fi
...
if [[ -n "$parent_runner_override" ]]; then
    [[ -x "$parent_runner_override" ]] \
        || die "TEST262_EVAL_WTF8_PARENT_RUNNER is not executable: $parent_runner_override"
...
verify_focused_semantics
if [[ "$mode" != full ]]; then
    echo 'R3cu focused semantics pass: QuickJS 22/22, Oxide 22/22, 22 runtime repairs, zero regressions.'
    exit 0
fi
...
if [[ -f "$parent_full" && -f "${parent_full%.tsv}.jsonl" \
    ...
    if [[ "$reuse_full_reports" == false ]]; then
        join_candidate_full "$parent_full" "$candidate_full"
    fi
else
...
[[ "$(report_runnable "$candidate_full")" == "$(value candidate_full_runnable)" \
    ...
    || die 'R3cu full outcome counts drifted'
verify_full_join "$parent_full" "$candidate_full"
echo 'R3cu full semantics pass: 102037 rows, 22 runtime repairs, 102015 unchanged, zero pass regressions.'
