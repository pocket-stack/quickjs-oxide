#!/usr/bin/env bash
# Reproduce the R3ae ordinary private async class-method Test262 gate.
...
baseline=tests/test262-async-private-class-method-core-baseline.txt
manifest=tests/test262-async-private-class-method-core.txt
admission_profile=tests/test262-async-private-class-method-core.conf
exclusions=tests/test262-async-private-class-method-core-exclusions.tsv
global_profile=compat/test262-oxide.conf
report=target/test262-async-private-class-method-core.tsv
json_report=target/test262-async-private-class-method-core.jsonl
quickjs_log=target/test262-async-private-class-method-core-quickjs.log
...
expect_value() {
    ...
    if [[ "$(read_value "$key")" != "$expected" ]]; then
        echo "error: async private class-method baseline $key drifted" >&2
        exit 1
    fi
}
...
verify_inventory() {
    ...
    if [[ "$(inventory_count "$inventory")" != "$(read_value "${name}_paths")" \
        ...
        echo "error: async private class-method $name inventory drifted" >&2
        exit 1
    fi
}
...
verify_quickjs_oracle() {
    ...
    if ! (cd -- "$source_dir" \
        ...
        cat "$quickjs_log" >&2
        echo "error: pinned QuickJS could not execute the async private class-method cohort" >&2
        exit 1
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])' "$quickjs_log" \
        || ! grep -Fq \
            ...
            "$quickjs_log"; then
        cat "$quickjs_log" >&2
        echo "error: pinned QuickJS no longer passes the async private class-method cohort" >&2
        exit 1
    fi
}
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3ae.XXXXXX")
...
touch "$positive" "$negative" "$async_paths" "$sync_paths" \
    ...
    "$include_occurrences" "$flag_occurrences" "$proxy_paths"
...
expect_value candidate_paths 233
expect_value candidate_direct_paths 54
expect_value candidate_direct_instance_paths 28
expect_value candidate_direct_static_paths 26
expect_value candidate_element_composition_paths 136
...
expect_value excluded_paths 77
expect_value excluded_private_async_generator_paths 68
expect_value excluded_public_async_generator_adjacency_paths 8
expect_value excluded_mixed_staging_paths 1
expect_value admitted_direct_paths 54
expect_value admitted_element_composition_paths 60
expect_value admitted_syntax_paths 40
expect_value admitted_object_negative_paths 2
expect_value admitted_function_to_string_paths 0
expect_value admitted_staging_paths 0
expect_value paths 156
expect_value quickjs_passes 233
expect_value positive_paths 92
expect_value negative_paths 64
expect_value async_paths 86
expect_value sync_paths 70
expect_value double_mode_paths 156
expect_value no_strict_paths 0
expect_value only_strict_paths 0
expect_value variants 312
expect_value sloppy_variants 156
expect_value strict_variants 156
expect_value features 7
expect_value includes 1
expect_value flags 2
expect_value runnable 312
expect_value passes 312
...
if [[ "$(sha256_file "$global_profile")" != "$(read_value global_oxide_profile_sha256)" \
    ...
    echo "error: async private class-method pinned profile or exclusions drifted" >&2
    exit 1
fi
...

{
    for class_form in expressions statements; do
        git -C "$suite" ls-files \
            "test/language/$class_form/class/elements/async-private-method/*.js" \
            "test/language/$class_form/class/elements/async-private-method/**/*.js" \
            "test/language/$class_form/class/elements/async-private-method-static/*.js" \
            "test/language/$class_form/class/elements/async-private-method-static/**/*.js"
        printf '%s\n' \
            "test/language/$class_form/class/elements/private-async-method-name.js" \
            "test/language/$class_form/class/elements/private-static-async-method-name.js" \
            "test/language/$class_form/class/elements/private-methods/prod-private-async-method.js"
        git -C "$suite" ls-files \
            "test/language/$class_form/class/elements/*-rs-static-async-method-privatename-identifier*.js" \
        ...
        git -C "$suite" ls-files \
            ...
            | awk '
                {
                    lower=tolower($0)
                    if ((lower ~ /private.*async/ || lower ~ /async.*private/) \
                        && lower !~ /generator|async-gen|gen-meth/) {
                        print
                    }
                }
            '
...
direct_filter='
    /\/async-private-method(-static)?\// \
        ...
        || /\/private-methods\/prod-private-async-method\.js$/ {
        count++
    }
    END { print count + 0 }
'
if [[ "$(inventory_count "$candidate")" != "$(read_value candidate_paths)" \
    ...
    || "$(awk '
        /\/async-private-method\// \
            ...
            || /\/private-methods\/prod-private-async-method\.js$/ { count++ }
        END { print count + 0 }
    ' "$candidate")" != "$(read_value candidate_direct_instance_paths)" \
    || "$(awk '
        /\/async-private-method-static\// \
            || /\/private-static-async-method-name\.js$/ { count++ }
        END { print count + 0 }
    ' "$candidate")" != "$(read_value candidate_direct_static_paths)" \
    || "$(awk '
        /-rs-static-async-(generator-)?method-privatename-identifier/ { count++ }
        END { print count + 0 }
    ...
    || "$(awk '/\/object\/method-definition\/private-name-early-error-async-fn/ { count++ } END { print count + 0 }' "$candidate")" \
    ...
    echo "error: async private class-method candidate universe drifted" >&2
    exit 1
fi
verify_inventory proxy "$proxy_paths"

if awk -F'\t' \
    'NF != 2 || $1 == "" || $2 == "" { print NR ":" $0; bad=1 } END { exit bad ? 0 : 1 }' \
    "$exclusions" >&2; then
    echo "error: async private class-method exclusions must have two populated TSV columns" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$excluded_paths")" != "$(read_value excluded_paths)" \
    ...
    || "$(awk -F'\t' '$1 == "private_async_generator" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_private_async_generator_paths)" \
    || "$(awk -F'\t' '$1 == "public_async_generator_adjacency" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_public_async_generator_adjacency_paths)" \
    || "$(awk -F'\t' '$1 == "mixed_private_async_generator_staging" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_mixed_staging_paths)" \
    || "$(awk -F'\t' '
        $1 != "private_async_generator" \
            && $1 != "public_async_generator_adjacency" \
            && $1 != "mixed_private_async_generator_staging" { count++ }
        END { print count + 0 }
    ' "$exclusions")" != "0" ]]; then
    echo "error: async private class-method exclusion ledger drifted" >&2
    exit 1
fi
if [[ -n "$(comm -23 "$excluded_paths" "$candidate")" ]]; then
    echo "error: async private class-method exclusion escaped the candidate universe" >&2
    exit 1
fi
...
if [[ -n "$(comm -12 "$manifest" "$excluded_paths")" ]]; then
    echo "error: async private class-method manifest and exclusions overlap" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$manifest")" != "$(read_value paths)" \
    ...
    || "$(awk '
        /-rs-static-async-(generator-)?method-privatename-identifier/ { count++ }
        END { print count + 0 }
    ' "$manifest")" != "$(read_value admitted_element_composition_paths)" \
    || "$(awk '/\/class\/elements\/syntax\// { count++ } END { print count + 0 }' "$manifest")" \
        != "$(read_value admitted_syntax_paths)" \
    || "$(awk '/\/object\/method-definition\/private-name-early-error-async-fn/ { count++ } END { print count + 0 }' "$manifest")" \
    ...
    echo "error: async private class-method manifest drifted" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$variant_keys")" != "$(read_value variants)" \
    ...
    echo "error: async private class-method metadata composition drifted" >&2
    exit 1
fi
...
[[ "$(profile_section execution)" == "async=true" ]] \
    || { echo "error: async private class-method profile must opt into only the async host" >&2; exit 1; }

verify_quickjs_oracle
if "$check_only"; then
    printf 'async private class-method inputs verified: %s candidates - %s explicit exclusions = %s paths; QuickJS %s passes all candidates; Oxide gate covers %s variants\n' \
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: async private class-method report metadata drifted" >&2
    exit 1
fi
...
if [[ "$runner_summary" != "$expected_runner_summary" \
    ...
    echo "error: async private class-method all-pass vector drifted" >&2
    awk -F'\t' '
        !/^#/ && !($1 == "path" && $2 == "variant") && $7 != "pass" {
            print $1 "\t" $2 "\t" $7 "\t" $8 "\t" $9 "\t" $10
        }
...
printf 'async private class-method Test262 gate passes: %s/%s variants across %s audited paths\n' \
