#!/usr/bin/env bash
# Reproduce the R3l bootstrap cohort for private synchronous class generators.
...
baseline=tests/test262-class-private-generator-methods-baseline.txt
manifest=tests/test262-class-private-generator-methods.txt
admission_profile=tests/test262-class-private-generator-methods.conf
global_profile=compat/test262-oxide.conf
report=target/test262-class-private-generator-methods.tsv
json_report=target/test262-class-private-generator-methods.jsonl
workers=${TEST262_WORKERS:-8}

usage() {
    printf 'usage: %s [--check]\n' "${0##*/}"
    printf '  --check  verify the 90/8/82 audit and pinned QuickJS oracle only\n'
}
...
derive_direct_universe_paths() {
    git -C "$suite" ls-files \
        'test/language/expressions/class/elements/gen-private-method/*.js' \
        'test/language/expressions/class/elements/gen-private-method-static/*.js' \
        'test/language/statements/class/elements/gen-private-method/*.js' \
        'test/language/statements/class/elements/gen-private-method-static/*.js' \
        | LC_ALL=C sort
}

derive_excluded_paths() {
    awk -F/ '
        $NF == "yield-identifier-spread-strict.js" ||
        $NF == "yield-spread-obj.js" { print }
    ' | LC_ALL=C sort
}

derive_core_paths() {
    git -C "$suite" ls-files \
        'test/language/expressions/class/elements/private-generator-method-name.js' \
        'test/language/expressions/class/elements/private-methods/prod-private-generator.js' \
        'test/language/expressions/class/elements/private-static-generator-method-name.js' \
        'test/language/expressions/class/elements/syntax/valid/grammar-static-private-gen-meth-prototype.js' \
        'test/language/statements/class/elements/private-generator-method-name.js' \
        'test/language/statements/class/elements/private-methods/prod-private-generator.js' \
        'test/language/statements/class/elements/private-static-generator-method-name.js' \
        'test/language/statements/class/elements/syntax/valid/grammar-static-private-gen-meth-prototype.js' \
        | LC_ALL=C sort
}

derive_early_error_paths() {
    git -C "$suite" ls-files \
        'test/language/expressions/class/elements/syntax/early-errors/*.js' \
        'test/language/expressions/class/elements/syntax/early-errors/delete/*.js' \
        'test/language/statements/class/elements/syntax/early-errors/*.js' \
        'test/language/statements/class/elements/syntax/early-errors/delete/*.js' \
        | awk -F/ '
            $(NF - 1) == "delete" {
                if ($NF ~ /private-method-gen\.js$/) print
                next
            }
            $NF ~ /^(grammar-privatemeth-duplicate-gen|grammar-privatename-whitespace-error-(static-)?gen-meth|grammar-special-meth-contains-super-private-gen|grammar-static-private-gen-meth-(constructor|super)|private-generator-cannot-escape-token)\.js$/ { print }
        ' \
        | LC_ALL=C sort
}
...
verify_inventory() {
    ...
    if [[ "$(printf '%s\n' "$inventory" | wc -l | tr -d '[:space:]')" != "$expected_count" \
        ...
        echo "error: class-private-generator $name inventory drifted" >&2
        exit 1
    fi
}
...
verify_quickjs_oracle() {
    ...
    if ! output=$(cd -- "$source_dir" && ./run-test262 -a -m -c test262.conf -f "${files[@]}" 2>&1); then
        printf '%s\n' "$output" >&2
        echo "error: pinned QuickJS could not execute the class-private-generator cohort" >&2
        exit 1
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])' <<<"$output" \
        || ! grep -Fq "Average memory statistics for $(read_value quickjs_passes) tests:" <<<"$output"; then
        printf '%s\n' "$output" >&2
        echo "error: pinned QuickJS no longer passes the complete class-private-generator cohort" >&2
        exit 1
    fi
}
...
if [[ "$(read_value quickjs)" != "2026-06-04" \
    ...
    || "$(read_value quickjs_passes)" != "82" \
    || "$(read_value candidate_universe_paths)" != "90" \
    || "$(read_value excluded_paths)" != "8" ]]; then
    echo "error: class-private-generator baseline identity drifted" >&2
    exit 1
fi
...
[[ "$actual_paths" == "82" && "$unique_paths" == "82" ]] \
    || { echo "error: class-private-generator manifest cardinality drifted" >&2; exit 1; }
...
[[ "$(manifest_paths | sha256_stream)" == "$(read_value manifest_sha256)" \
    ...
    || { echo "error: class-private-generator manifest content drifted" >&2; exit 1; }

direct_universe_inventory=$(derive_direct_universe_paths)
excluded_inventory=$(printf '%s\n' "$direct_universe_inventory" | derive_excluded_paths)
direct_inventory=$(comm -23 \
    <(printf '%s\n' "$direct_universe_inventory") \
    <(printf '%s\n' "$excluded_inventory"))
core_inventory=$(derive_core_paths)
early_error_inventory=$(derive_early_error_paths)
candidate_universe_inventory=$(printf '%s\n' \
    "$direct_universe_inventory" \
    "$core_inventory" \
    "$early_error_inventory" \
    | LC_ALL=C sort)

if [[ -n "$(comm -23 \
    <(printf '%s\n' "$excluded_inventory") \
    <(printf '%s\n' "$candidate_universe_inventory"))" ]]; then
    echo "error: class-private-generator exclusions escaped the candidate universe" >&2
    exit 1
fi
...
verify_inventory core "$core_inventory"
verify_inventory early_error "$early_error_inventory"
...
compare_array_inventory=$(manifest_paths | awk -F/ '$NF == "yield-spread-arr-multiple.js" { print }')
single_variant_inventory=
variant_keys=
feature_inventory=
while IFS= read -r test_path; do
    [[ -f "$suite/$test_path" ]] \
        || { echo "error: missing class-private-generator path: $test_path" >&2; exit 1; }
    ...
    case "$flag_line" in
        ...
        "flags: [generated, onlyStrict]")
            ...
            ;;
        *)
            echo "error: class-private-generator flags drifted: $test_path: $flag_line" >&2
            exit 1
            ;;
    esac
    if grep -Fq 'negative:' <<<"$metadata" \
        && { ! grep -Fq '  phase: parse' <<<"$metadata" || ! grep -Fq '  type: SyntaxError' <<<"$metadata"; }; then
        echo "error: class-private-generator negative provenance drifted: $test_path" >&2
        exit 1
    fi
done < <(manifest_paths)
...
verify_inventory compare_array "$compare_array_inventory"
[[ "$(printf '%s\n' "$variant_keys" | wc -l | tr -d '[:space:]')" == "$(read_value variants)" \
    && "$(printf '%s\n' "$variant_keys" | sha256_stream)" == "$(read_value keys_sha256)" ]] \
    || { echo "error: class-private-generator variant inventory drifted" >&2; exit 1; }
...
verify_quickjs_oracle
if "$check_only"; then
    printf 'class-private-generator inputs verified: %s candidates - %s exclusions = %s admitted paths; QuickJS %s passes all; Oxide gate covers %s variants\n' \
...
if [[ ${#pending_keys[@]} -ne 0 ]]; then
    printf 'error: class-private-generator Oxide baseline needs refresh before execution: %s\n' \
        "${pending_keys[*]}" >&2
    exit 1
fi
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: class-private-generator report metadata drifted" >&2
    exit 1
fi
...
if [[ "$runner_summary" != "$expected_runner_summary" \
    ...
    echo "error: class-private-generator Test262 classified vector drifted" >&2
    awk -F'\t' '
...
printf 'class-private-generator Test262 gate is exact: %s/%s pass across %s audited paths from a %s-path candidate universe\n' \
    "$actual_passes" "$actual_variants" "$expected_paths" "$(read_value candidate_universe_paths)"
