...
baseline=tests/test262-eval-intrinsic-baseline.txt
manifest=tests/test262-eval-intrinsic.txt
report=target/test262-eval-intrinsic.tsv
json_report=target/test262-eval-intrinsic.jsonl
...
cd -- "$root"
if [[ ! -f "$baseline" ]]; then
    echo "error: eval-intrinsic Test262 baseline is missing: $baseline" >&2
    exit 1
fi
...
if [[ "$actual_manifest_paths" != "$expected_paths" \
    ...
    echo "error: eval-intrinsic manifest cardinality drifted" >&2
    exit 1
fi
if ! awk 'NF && $1 !~ /^#/ { print }' "$manifest" | LC_ALL=C sort -c; then
    echo "error: eval-intrinsic manifest is not bytewise sorted" >&2
    exit 1
fi
actual_manifest=$(awk 'NF && $1 !~ /^#/ { print }' "$manifest" | sha256_stream)
if [[ "$actual_manifest" != "$expected_manifest" ]]; then
    echo "error: eval-intrinsic manifest content drifted" >&2
    exit 1
fi
...
if [[ "$actual_quickjs" != "$expected_quickjs" \
    ...
    echo "error: eval-intrinsic Test262 baseline metadata drifted" >&2
    exit 1
fi
...
if ! diff -u \
    ...
    echo "error: eval-intrinsic report paths drifted from the frozen manifest" >&2
    exit 1
fi
...
if [[ "$actual_keys_hash" != "$expected_keys_hash" ]]; then
    echo "error: eval-intrinsic report keys drifted" >&2
    exit 1
fi
...
if [[ "$actual_summary" != "# summary $expected_summary" \
    ...
    echo "error: eval-intrinsic classified outcomes drifted" >&2
    exit 1
fi
...
if [[ "$actual_jsonl_lines" != "$expected_jsonl_lines" \
    ...
    echo "error: eval-intrinsic classified TSV/JSONL vector drifted" >&2
    exit 1
fi

"$script_dir/check-rust-only.sh"
printf 'eval-intrinsic Test262 vector matches: %s pass of %s variants across %s paths\n' \
