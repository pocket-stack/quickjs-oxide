#!/usr/bin/env bash
# Reproduce the R3ck String.prototype.normalize runtime-parity receipt.
...
baseline=tests/test262-string-normalize-baseline.txt
canonical_baseline=tests/test262-full-baseline.txt
predecessor_baseline=tests/test262-binary-data-global-baseline.txt
upstream=compat/upstream.toml
profile=compat/test262-oxide.conf
universe=tests/test262-string-normalize-universe.txt
supplemental=tests/test262-string-normalize-supplemental.txt
manifest=tests/test262-string-normalize.txt
parent_report=tests/test262-string-normalize-parent.tsv
transition=tests/test262-string-normalize-transitions.tsv
candidate_report=target/test262-string-normalize-candidate.tsv
candidate_full=target/test262-string-normalize-full.tsv
preferred_parent_full=${TEST262_NORMALIZE_PARENT_FULL:-target/test262-binary-data-global-candidate-full.tsv}
generated_parent_full=target/test262-string-normalize-parent-full.tsv
oracle_log=target/test262-string-normalize-quickjs.log
workers=${TEST262_WORKERS:-8}
full_workers=${TEST262_FULL_WORKERS:-2}
reuse_full_reports=${TEST262_REUSE_FULL_REPORTS:-false}

baseline_lines=68
baseline_sha=dec902e83fb7564825788c1a7dff9281341f1b05f92d645666f149aeae93c5e2
predecessor_baseline_lines=95
predecessor_baseline_sha=0d188b3b2c0f65417e02e4c3350077f94665cb6de6e7ac4fc453750e1cfa83d6
...
make_transition() {
    ...
    {
        echo '# Exhaustive R3ck String.prototype.normalize runtime transition.'
...
# R3ck changes runtime behavior without changing the Test262 profile, so the
# current runner cannot replay its own pre-normalize parent. Reconstruct that
# vector from the freshly replayed candidate by substituting the tracked 38-row
# parent receipt. The predecessor's full-vector hashes then authenticate every
# byte, including all 101,999 rows outside the focused gate.
reconstruct_parent_full() {
    ...
    [[ -f "$candidate" && -f "$candidate_json" ]] \
        || die 'cannot reconstruct normalize parent before replaying the candidate full vector'
    ...
    ' "$parent_report" "$candidate" >"$output" \
        || die 'could not reconstruct normalize parent TSV'
    ...
    ' "${parent_report%.tsv}.jsonl" "$candidate_json" >"$output_json" \
        || die 'could not reconstruct normalize parent JSONL'

    [[ "$(sha "$output")" == "$(value parent_full_tsv_sha256)" \
        && "$(sha "$output_json")" == "$(value parent_full_jsonl_sha256)" ]] \
        || die 'reconstructed normalize parent does not match the canonical R3cj vector'
}
...
check_manifests() {
    ...
    [[ -z "$(uniq -d "$tmp/gate.partition")" ]] || die 'normalize manifests overlap'
}
...
check_inputs() {
    ...
    check_file "$parent_report" 49 "$(value parent_focused_tsv_sha256)"
    check_file "${parent_report%.tsv}.jsonl" 40 "$(value parent_focused_jsonl_sha256)"
    check_file "$transition" 44 "$(value transition_sha256)"
    check_manifests
    [[ "$(report_rows "$transition" | sha /dev/stdin)" == "$(value transition_data_sha256)" \
        ...
        || die 'normalize upstream or transition binding drifted'
    [[ "$(predecessor_value candidate_oxide_profile_sha256)" == "$(value oxide_profile_sha256)" \
        ...
        || die 'R3cj predecessor does not checksum-bind the normalize parent'
    [[ "$(canonical_value schema)" == "$(value schema)" \
        ...
        || die 'canonical Test262 baseline does not identify the normalize candidate'
}
...
verify_quickjs() {
    ...
    if ! (
        ...
        die 'pinned QuickJS could not execute the normalize gate'
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])|SKIPPED FEATURE' "$oracle_log" \
        ...
        tail -n 100 "$oracle_log" >&2
        die 'pinned QuickJS no longer passes the normalize gate'
    fi
}

cd -- "$root"
tmp=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-string-normalize.XXXXXX")
trap 'rm -rf -- "$tmp"' EXIT HUP INT TERM
check_inputs
if [[ "$mode" == check ]]; then
    echo 'R3ck normalize inputs verified: 19 paths, 38 variants, checksum-bound R3cj parent.'
    exit 0
fi
...
{
    find "$suite/test/built-ins/String/prototype/normalize" -type f -name '*.js'
    find "$suite/test/staging/sm/String" -maxdepth 1 -type f -name 'normalize-*.js'
} | sed "s#^$suite/##" | sort >"$tmp/derived-universe"
diff -u "$universe" "$tmp/derived-universe"
[[ -f "$suite/$(cat "$supplemental")" ]] \
    || die 'normalize supplemental receiver test disappeared from pinned Test262'

verify_quickjs
run_report "$candidate_report" focused "$workers"
verify_report "$parent_report" "$(value gate_variants)" \
    "$(value gate_keys_sha256)" parent_focused
verify_report "$candidate_report" "$(value gate_variants)" \
...
[[ "$(report_runnable "$parent_report")" == "$(value parent_focused_runnable)" \
    ...
    && "$(report_count fail-runtime "$candidate_report")" == "$(value candidate_focused_fail_runtime)" ]] \
    || die 'normalize focused runnable/pass counts drifted'
...
[[ "$(sha "$tmp/transition.tsv")" == "$(value transition_sha256)" \
    ...
    || die 'normalize focused transition semantics drifted'

if [[ "$mode" != full ]]; then
    check_inputs
    echo 'R3ck normalize focused gate passes: QuickJS 38/38, Oxide 38/38, 20 new passes.'
    exit 0
fi

if [[ "$reuse_full_reports" == false ]]; then
    run_report "$candidate_full" full "$full_workers"
fi
...
verify_report "$parent_full" "$(value full_variants)" \
    "$(value full_keys_sha256)" parent_full
verify_report "$candidate_full" "$(value full_variants)" \
...
[[ "$(report_runnable "$parent_full")" == "$(value parent_full_runnable)" \
    ...
    || die 'normalize full receipt counts drifted'
...
' "$parent_full" "$candidate_full") || die 'normalize full exact join failed'
...
[[ "$join_counts" == "$expected_join" ]] \
    || die "normalize full no-regression join drifted: $join_counts"
check_inputs
echo 'R3ck normalize full gate passes: 102037 rows, 20 new passes, 102017 unchanged, zero pass regressions.'
