...
successor_baseline=tests/test262-realm-hosts-global-baseline.txt
parent=tests/test262-host-gc-global-parent.conf
candidate=tests/test262-host-gc-global-candidate.conf
successor_parent=tests/test262-realm-hosts-global-parent.conf
successor_candidate=tests/test262-realm-hosts-global-candidate.conf
successor_gate=scripts/test-test262-realm-hosts-global.sh
...
successor_sha=01f936b9f5e0b920f10119a73f7e8ea52450863f113fff6542f3f241ed914d75
successor_baseline_sha=04a27c431883633e93cbe4abdd6eb19683ca1dce58050ab9e38365437d5fb472
...
bridge_r3ci_successor() {
    ...
    check_file "$historical_parent_json" 30 "$historical_parent_json_sha"
    check_file "$transition" 33 "$formal_transition_sha"
    check_file "$successor_baseline" 121 "$successor_baseline_sha"
    check_file "$successor_parent" 1272 "$candidate_sha"
    check_file "$successor_candidate" 1274 "$successor_sha"
    ...
    [[ "$(value candidate_oxide_profile_sha256)" == "$candidate_sha" \
        ...
        && "$(successor_value historical_full_summary)" == "$(value candidate_full_summary)" \
        && "$(successor_value historical_to_runtime_parent_full_changed)" == 534 \
        && "$(successor_value runtime_parent_to_candidate_full_outcome_changes)" == 194 \
        && "$(successor_value runtime_parent_to_candidate_full_detail_changes)" == 340 \
        && "$(successor_value runtime_parent_to_candidate_full_pass_regressions)" == 0 ]] \
        || die 'R3ci successor does not checksum-bridge the historical R3ch receipt'
    if [[ "$(sha "$live_profile")" != "$successor_sha" ]]; then
        ...
        echo 'Historical R3ch host-GC receipt is transitively checksum-bridged through the current successor chain.'
        exit 0
    fi
    check_file "$live_profile" 1274 "$successor_sha"
    ...
    echo 'Historical R3ch host-GC receipt is checksum-bridged through the replayed R3ci successor.'
    exit 0
}
