#!/usr/bin/env bash
# Reproduce the complete classified outcome vector for the Reflect milestone.
...
baseline=tests/test262-reflect-baseline.txt
report=target/test262-reflect.tsv
json_report=target/test262-reflect.jsonl
...
run_output=$(cargo run --locked --release --quiet --bin run-test262 -- \
    ...
    --manifest tests/test262-reflect.txt \
    --report "$report" \
    --mode both \
    --workers "$workers" \
...
if [[ "$actual_schema" != "$expected_schema" \
    ...
    echo "error: Reflect Test262 baseline metadata drifted" >&2
...
if [[ "$actual_summary" != "# summary $expected_summary" ]]; then
    echo "error: Reflect Test262 classified summary drifted" >&2
    echo "expected: # summary $expected_summary" >&2
    echo "actual:   $actual_summary" >&2
    exit 1
fi
if [[ " $expected_summary " != *" pass=$expected_passes "* ]]; then
    echo "error: Reflect pass count is inconsistent with the pinned summary" >&2
    exit 1
fi
...
if [[ "$actual_tsv" != "$expected_tsv" || "$actual_jsonl" != "$expected_jsonl" ]]; then
    echo "error: Reflect Test262 classified vector drifted" >&2
...
printf 'Reflect Test262 vector matches: %s pass of %s variants\n' \
