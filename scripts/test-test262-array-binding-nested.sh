#!/usr/bin/env bash
# Reproduce the dependency-audited R2r nested array binding Test262 gate.
...
baseline=tests/test262-array-binding-nested-baseline.txt
manifest=tests/test262-array-binding-nested.txt
profile=tests/test262-array-binding-nested.conf
report=target/test262-array-binding-nested.tsv
json_report=target/test262-array-binding-nested.jsonl
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$expected_profile" != "c770387473b6ba2e273ab635182b5f07ae80ad902f48057ba5e2fb4f036c723e" \
    ...
    echo "error: nested array binding baseline metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_paths" != "$expected_paths" || "$unique_paths" != "$expected_paths" ]]; then
    echo "error: nested array binding manifest cardinality drifted" >&2
    exit 1
fi
awk 'NF && $1 !~ /^#/ { print }' "$manifest" | LC_ALL=C sort -c
actual_manifest=$(awk 'NF && $1 !~ /^#/ { print }' "$manifest" | sha256_stream)
if [[ "$actual_manifest" != "$expected_manifest" ]]; then
    echo "error: nested array binding manifest content drifted" >&2
    exit 1
fi
if [[ "$(sha256_file "$manifest")" != "$expected_manifest_file" ]]; then
    echo "error: nested array binding manifest file drifted" >&2
    exit 1
fi
if [[ "$(sha256_file "$profile")" != "$expected_profile" ]]; then
    echo "error: nested array binding scoped capability profile drifted" >&2
    exit 1
fi
while IFS= read -r test_path; do
    if [[ ! -f "$suite/$test_path" ]]; then
        echo "error: pinned nested array binding path is missing: $test_path" >&2
        exit 1
    fi
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: nested array binding report metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_keys" != "$expected_keys" ]]; then
    echo "error: nested array binding path/variant key set drifted" >&2
    exit 1
fi
...
if [[ "$actual_passes" != "$expected_passes" \
    ...
    echo "error: nested array binding Test262 classified vector drifted" >&2
...
printf 'nested array binding Test262 gate passes: %s/%s variants across %s paths\n' \
