#!/usr/bin/env bash
# Reproduce the R3n Promise.race/try/withResolvers cohort.
...
baseline=tests/test262-promise-race-try-with-resolvers-baseline.txt
manifest=tests/test262-promise-race-try-with-resolvers.txt
admission_profile=tests/test262-promise-race-try-with-resolvers.conf
global_profile=compat/test262-oxide.conf
report=target/test262-promise-race-try-with-resolvers.tsv
json_report=target/test262-promise-race-try-with-resolvers.jsonl
...
verify_inventory() {
    ...
    if [[ "$(printf '%s\n' "$inventory" | wc -l | tr -d '[:space:]')" != "$expected_count" \
        ...
        echo "error: Promise race/try/withResolvers $name inventory drifted" >&2
        exit 1
    fi
}
...
verify_quickjs_oracle() {
    ...
    if ! output=$(cd -- "$source_dir" && ./run-test262 -a -m -c test262.conf -f "${files[@]}" 2>&1); then
        printf '%s\n' "$output" >&2
        echo "error: pinned QuickJS could not execute the Promise race/try/withResolvers cohort" >&2
        exit 1
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])' <<<"$output" \
        || ! grep -Fq "Average memory statistics for $(read_value quickjs_passes) tests:" <<<"$output"; then
        printf '%s\n' "$output" >&2
        echo "error: pinned QuickJS no longer passes the Promise race/try/withResolvers cohort" >&2
        exit 1
    fi
}
...
if [[ "$(read_value quickjs)" != "2026-06-04" \
    ...
    || "$(read_value paths)" != "112" \
    || "$(read_value variants)" != "224" \
    || "$(read_value quickjs_passes)" != "112" \
    || "$(read_value race_variants)" != "188" \
    || "$(read_value try_variants)" != "24" \
    || "$(read_value with_resolvers_variants)" != "12" \
    || "$(read_value async_variants)" != "132" \
    || "$(read_value sync_variants)" != "92" \
    || "$(read_value adjacency_variants)" != "10" ]]; then
    echo "error: Promise race/try/withResolvers baseline identity drifted" >&2
    exit 1
fi

actual_paths=$(manifest_paths | wc -l | tr -d '[:space:]')
unique_paths=$(manifest_paths | LC_ALL=C sort -u | wc -l | tr -d '[:space:]')
[[ "$actual_paths" == "112" && "$unique_paths" == "112" ]] \
    || { echo "error: Promise race/try/withResolvers manifest cardinality drifted" >&2; exit 1; }
...
[[ "$(manifest_paths | sha256_stream)" == "$(read_value manifest_sha256)" \
    ...
    || { echo "error: Promise race/try/withResolvers manifest content drifted" >&2; exit 1; }

derived_manifest=$(git -C "$suite" ls-files \
    'test/built-ins/Promise/race/*.js' \
    'test/built-ins/Promise/try/*.js' \
    'test/built-ins/Promise/withResolvers/*.js' \
    | awk -F/ 'NF == 5' \
    | LC_ALL=C sort)
diff -u <(printf '%s\n' "$derived_manifest") <(manifest_paths)

race_inventory=$(manifest_paths | awk '$0 ~ "^test/built-ins/Promise/race/"')
try_inventory=$(manifest_paths | awk '$0 ~ "^test/built-ins/Promise/try/"')
with_resolvers_inventory=$(manifest_paths | awk '$0 ~ "^test/built-ins/Promise/withResolvers/"')
async_inventory=
sync_inventory=
adjacency_inventory=
variant_keys=
...
while IFS= read -r test_path; do
    [[ -f "$suite/$test_path" ]] \
        || { echo "error: missing Promise race/try/withResolvers path: $test_path" >&2; exit 1; }
    metadata=$(metadata_block "$test_path")
    if grep -Fq 'negative:' <<<"$metadata"; then
        echo "error: Promise race/try/withResolvers cohort unexpectedly gained a negative test: $test_path" >&2
        exit 1
    fi
    ...
    case "$flag_line" in
        ...
        "flags: [async]")
            ...
            ;;
        *)
            echo "error: Promise race/try/withResolvers flags drifted: $test_path: $flag_line" >&2
            exit 1
            ;;
    esac
    variant_keys+=$'\n'"$test_path"$'\t'sloppy$'\n'"$test_path"$'\t'strict
    if grep -Eq 'Promise\.all[[:space:]]*\(|\.finally[[:space:]]*\(' "$suite/$test_path"; then
        adjacency_inventory+=$'\n'"$test_path"
    fi
done < <(manifest_paths)
...
adjacency_inventory=$(printf '%s\n' "$adjacency_inventory" | sed '/^$/d' | LC_ALL=C sort)
...
verify_inventory race "$race_inventory"
verify_inventory try "$try_inventory"
verify_inventory with_resolvers "$with_resolvers_inventory"
verify_inventory async "$async_inventory"
verify_inventory sync "$sync_inventory"
verify_inventory adjacency "$adjacency_inventory"
[[ "$(printf '%s\n' "$feature_inventory" | wc -l | tr -d '[:space:]')" == "$(read_value features)" \
    && "$(printf '%s\n' "$feature_inventory" | sha256_stream)" == "$(read_value features_sha256)" ]] \
    || { echo "error: Promise race/try/withResolvers metadata features drifted" >&2; exit 1; }
[[ "$(printf '%s\n' "$include_inventory" | wc -l | tr -d '[:space:]')" == "$(read_value includes)" \
    && "$(printf '%s\n' "$include_inventory" | sha256_stream)" == "$(read_value includes_sha256)" ]] \
    || { echo "error: Promise race/try/withResolvers includes drifted" >&2; exit 1; }
[[ "$(printf '%s\n' "$variant_keys" | wc -l | tr -d '[:space:]')" == "$(read_value variants)" \
    && "$(printf '%s\n' "$variant_keys" | sha256_stream)" == "$(read_value keys_sha256)" ]] \
    || { echo "error: Promise race/try/withResolvers variant inventory drifted" >&2; exit 1; }

diff -u <(profile_section features | LC_ALL=C sort) <(printf '%s\n' "$feature_inventory")
[[ -z "$(profile_section audited-negative-tests)" ]] \
    || { echo "error: Promise race/try/withResolvers profile admitted negative tests" >&2; exit 1; }
[[ "$(profile_section execution)" == "async=true" ]] \
    || { echo "error: Promise race/try/withResolvers profile must opt into only the async host" >&2; exit 1; }
[[ "$(sha256_file "$global_profile")" == "$(read_value global_oxide_profile_sha256)" \
    ...
    || { echo "error: Promise race/try/withResolvers capability profile drifted" >&2; exit 1; }
...
verify_quickjs_oracle
if "$check_only"; then
    printf 'Promise race/try/withResolvers inputs verified: %s complete paths, including %s Promise.all/finally adjacency paths; QuickJS %s passes all; Oxide gate covers %s variants\n' \
        "$(read_value paths)" \
        "$(read_value adjacency_paths)" \
        "$(read_value quickjs)" \
        "$(read_value variants)"
    exit 0
fi
...
if [[ ${#pending_keys[@]} -ne 0 ]]; then
    printf 'error: Promise race/try/withResolvers Oxide baseline needs refresh before execution: %s\n' \
        "${pending_keys[*]}" >&2
    exit 1
fi
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: Promise race/try/withResolvers report metadata drifted" >&2
    exit 1
fi
...
if [[ "$runner_summary" != "$expected_runner_summary" \
    ...
    echo "error: Promise race/try/withResolvers Test262 classified vector drifted" >&2
    awk -F'\t' '
...
printf 'Promise race/try/withResolvers Test262 gate is exact: %s/%s pass, %s expected failures across %s complete paths\n' \
    "$actual_passes" "$actual_variants" "$actual_failures" "$(read_value paths)"
