#!/usr/bin/env bash
# Reproduce the R3ag object async-generator method Test262 gate.
...
baseline=tests/test262-async-generator-object-method-core-baseline.txt
manifest=tests/test262-async-generator-object-method-core.txt
admission_profile=tests/test262-async-generator-object-method-core.conf
exclusions=tests/test262-async-generator-object-method-core-exclusions.tsv
global_profile=compat/test262-oxide.conf
report=target/test262-async-generator-object-method-core.tsv
json_report=target/test262-async-generator-object-method-core.jsonl
quickjs_log=target/test262-async-generator-object-method-core-quickjs.log
...
expect_value() {
    ...
    if [[ "$(read_value "$key")" != "$expected" ]]; then
        echo "error: async-generator object-method baseline $key drifted" >&2
        exit 1
    fi
}
...
verify_inventory() {
    ...
    if [[ "$(inventory_count "$inventory")" != "$(read_value "${name}_paths")" \
        ...
        echo "error: async-generator object-method $name inventory drifted" >&2
        exit 1
    fi
}
...
verify_quickjs_oracle() {
    ...
    if ! (cd -- "$source_dir" \
        ...
        cat "$quickjs_log" >&2
        echo "error: pinned QuickJS could not execute the object method cohort" >&2
        exit 1
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])' "$quickjs_log" \
        || ! grep -Fq \
            ...
            "$quickjs_log"; then
        cat "$quickjs_log" >&2
        echo "error: pinned QuickJS no longer passes the object method cohort" >&2
        exit 1
    fi
}
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3ag.XXXXXX")
...
expect_value candidate_paths 113
expect_value candidate_function_to_string_paths 2
expect_value candidate_root_async_generator_paths 98
expect_value candidate_forbidden_extension_paths 5
expect_value candidate_private_name_paths 2
expect_value candidate_mislabeled_async_method_paths 6
expect_value candidate_variants 216
expect_value excluded_paths 67
expect_value excluded_destructuring_paths 4
expect_value excluded_yield_star_paths 58
expect_value excluded_for_await_paths 2
expect_value excluded_private_name_paths 2
expect_value excluded_proxy_paths 1
expect_value excluded_variants 134
expect_value paths 46
expect_value quickjs_passes 113
expect_value positive_paths 23
expect_value negative_paths 23
expect_value async_paths 18
expect_value sync_paths 28
expect_value double_mode_paths 36
expect_value no_strict_paths 8
expect_value only_strict_paths 2
expect_value variants 82
expect_value sloppy_variants 44
expect_value strict_variants 38
expect_value features 7
expect_value includes 2
expect_value flags 4
expect_value runnable 82
expect_value passes 82
...
if [[ "$(sha256_file "$global_profile")" != "$(read_value global_oxide_profile_sha256)" \
    ...
    echo "error: async-generator object-method pinned profile or exclusions drifted" >&2
    exit 1
fi
...
git -C "$suite" ls-files \
    ...
    'test/built-ins/Function/prototype/toString/proxy-async-generator-method-definition.js' \
    'test/language/expressions/object/method-definition/async-gen-*.js' \
    'test/language/expressions/object/method-definition/forbidden-ext/b1/async-gen-meth-*.js' \
    'test/language/expressions/object/method-definition/forbidden-ext/b2/async-gen-meth-*.js' \
    ...
    'test/language/expressions/object/method-definition/async-meth-array-destructuring-param-strict-body.js' \
    ...
    'test/language/expressions/object/method-definition/async-meth-object-destructuring-param-strict-body.js' \
    'test/language/expressions/object/method-definition/async-meth-rest-param-strict-body.js' \
    'test/language/expressions/object/method-definition/async-meth-rest-params-trailing-comma-early-error.js' \
...
if [[ "$(inventory_count "$candidate")" != "$(read_value candidate_paths)" \
    ...
    || "$(awk '/^test\/language\/expressions\/object\/method-definition\/async-gen-.*\.js$/ { count++ } END { print count + 0 }' "$candidate")" \
        != "$(read_value candidate_root_async_generator_paths)" \
    || "$(awk '/\/forbidden-ext\// { count++ } END { print count + 0 }' "$candidate")" \
        != "$(read_value candidate_forbidden_extension_paths)" \
    || "$(awk '/private-name/ { count++ } END { print count + 0 }' "$candidate")" \
        != "$(read_value candidate_private_name_paths)" \
    || "$(awk '/\/async-meth-/ { count++ } END { print count + 0 }' "$candidate")" \
        != "$(read_value candidate_mislabeled_async_method_paths)" ]]; then
    echo "error: async-generator object-method candidate universe drifted" >&2
    exit 1
fi
...
if awk -F'\t' \
    ...
    "$exclusions" >&2; then
    echo "error: async-generator object-method exclusions need two populated columns" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$excluded_paths")" != "$(read_value excluded_paths)" \
    ...
    || "$(awk -F'\t' '$1 == "private_name" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_private_name_paths)" \
    || "$(awk -F'\t' '$1 == "proxy" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_proxy_paths)" \
    || "$(awk -F'\t' '
        $1 != "destructuring" && $1 != "yield_star" && $1 != "for_await" \
            && $1 != "private_name" && $1 != "proxy" { count++ }
        END { print count + 0 }
    ' "$exclusions")" != "0" ]]; then
    echo "error: async-generator object-method exclusion ledger drifted" >&2
    exit 1
fi
if [[ -n "$(comm -23 "$excluded_paths" "$candidate")" ]]; then
    echo "error: async-generator object-method exclusion escaped candidate universe" >&2
    exit 1
fi
...
if [[ -n "$(comm -12 "$manifest" "$excluded_paths")" ]]; then
    echo "error: async-generator object-method manifest and exclusions overlap" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$manifest")" != "$(read_value paths)" \
    ...
    echo "error: async-generator object-method manifest drifted" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$candidate_variant_keys")" != "$(read_value candidate_variants)" \
    ...
    echo "error: async-generator object-method variant partition drifted" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$sloppy_paths")" != "$(read_value sloppy_variants)" \
    ...
    echo "error: async-generator object-method metadata composition drifted" >&2
    exit 1
fi
...
[[ "$(profile_section execution)" == "async=true" ]] \
    || { echo "error: object-method profile must opt into only the async host" >&2; exit 1; }

verify_quickjs_oracle
if "$check_only"; then
    printf 'async-generator object-method inputs verified: %s candidates - %s exclusions = %s paths; QuickJS %s passes all candidates; Oxide gate covers %s variants\n' \
...
if [[ "$(read_header quickjs)" != "$(read_value quickjs)" \
    ...
    echo "error: async-generator object-method report metadata drifted" >&2
    exit 1
fi
...
if [[ "$runner_summary" != "$expected_runner_summary" \
    ...
    echo "error: async-generator object-method all-pass vector drifted" >&2
    awk -F'\t' '
        !/^#/ && !($1 == "path" && $2 == "variant") && $7 != "pass" {
            print $1 "\t" $2 "\t" $7 "\t" $8 "\t" $9 "\t" $10
        }
...
repeat_report=target/test262-async-generator-object-method-core-repeat.tsv
five_report=target/test262-async-generator-object-method-core-workers-5.tsv
...
printf 'async-generator object-method Test262 gate passes: %s/%s variants across %s audited paths; %s/%s/5-worker reports are byte-identical\n' \
