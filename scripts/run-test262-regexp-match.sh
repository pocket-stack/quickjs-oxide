#!/usr/bin/env bash
# Reproduce the complete classified outcome vector for the RegExp match slice.
...
baseline=tests/test262-regexp-match-baseline.txt
manifest=tests/test262-regexp-match.txt
report=target/test262-regexp-match.tsv
json_report=target/test262-regexp-match.jsonl
...
expected_r1c_full_tsv=$(read_value r1c_full_tsv_sha256)
expected_r1c_selected=$(read_value r1c_selected_sha256)
expected_r1c_variants=$(read_value r1c_variants)
expected_r1c_summary=$(read_value r1c_summary)
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$expected_runnable" != "202" \
    || "$expected_passes" != "184" \
    || "$expected_manifest" != "2438976641de3a6fe33d7b8bfab36522ab2d61f6d879eb725dcd10779017deb9" \
    || "$expected_r1c_full_tsv" != "27982c6cbd922e61c9798d7532316f0d382c858bd01039c2700697ddb76b33c9" \
    || "$expected_r1c_selected" != "309d815d6dedded573ba282b76f2fb97cbd9364b2000df08144a7b85e520c3b7" \
    || "$expected_r1c_variants" != "208" \
    || "$expected_r1c_summary" != "fail-runtime=76 pass=2 unsupported-feature=130" ]]; then
    echo "error: RegExp match R1c provenance metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_manifest_paths" != "$expected_paths" \
    ...
    echo "error: RegExp match manifest cardinality drifted" >&2
...
if ! awk 'NF && $1 !~ /^#/ { print }' "$manifest" | LC_ALL=C sort -c; then
    echo "error: RegExp match manifest is not bytewise sorted" >&2
    exit 1
fi
actual_manifest=$(awk 'NF && $1 !~ /^#/ { print }' "$manifest" | sha256_stream)
if [[ "$actual_manifest" != "$expected_manifest" ]]; then
    echo "error: RegExp match manifest content drifted" >&2
...
if [[ "$actual_quickjs" != "$expected_quickjs" \
    ...
    echo "error: RegExp match Test262 baseline metadata drifted" >&2
...
if ! diff -u \
    ...
    echo "error: RegExp match Test262 report keys drifted from the frozen manifest" >&2
    exit 1
fi

actual_summary=$(tail -n 1 "$report")
if [[ "$actual_summary" != "# summary $expected_summary" ]]; then
    echo "error: RegExp match Test262 classified summary drifted" >&2
...
if [[ "$expected_passes" == 0 ]]; then
    if [[ " $expected_summary " == *" pass="* ]]; then
        echo "error: zero-pass RegExp match baseline unexpectedly records a pass outcome" >&2
        exit 1
    fi
elif [[ " $expected_summary " != *" pass=$expected_passes "* ]]; then
    echo "error: RegExp match pass count is inconsistent with the pinned summary" >&2
    exit 1
fi
...
if [[ "$actual_tsv" != "$expected_tsv" || "$actual_jsonl" != "$expected_jsonl" ]]; then
    echo "error: RegExp match Test262 classified vector drifted" >&2
...
printf 'RegExp match Test262 vector matches: %s pass of %s variants across %s paths\n' \
