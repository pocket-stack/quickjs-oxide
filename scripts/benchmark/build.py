...
def main():
    ...
    parser.add_argument("--stack-vm", action="store_true", help="build the opt-in owned stack driver")
    ...
    for name, target, enabled in [("plain", args.plain_target, []), ("profiling", args.profile_target, ["profiling"])]:
        if args.stack_vm:
            enabled.append("stack-vm")
        ...
        manifest = {"schema": "oxide-build-v1", "mode": name, "vm_configuration": "stack-vm" if args.stack_vm else "default", "features": enabled, "commit": revision, "binary_sha256": digest(binary),
