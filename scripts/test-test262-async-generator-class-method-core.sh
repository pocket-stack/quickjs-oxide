#!/usr/bin/env bash
# Reproduce the R3ah public class async-generator method Test262 gate.
...
baseline=tests/test262-async-generator-class-method-core-baseline.txt
manifest=tests/test262-async-generator-class-method-core.txt
admission_profile=tests/test262-async-generator-class-method-core.conf
exclusions=tests/test262-async-generator-class-method-core-exclusions.tsv
global_profile=compat/test262-oxide.conf
report=target/test262-async-generator-class-method-core.tsv
json_report=target/test262-async-generator-class-method-core.jsonl
quickjs_log=target/test262-async-generator-class-method-core-quickjs.log
...
expect_value() {
    ...
    if [[ "$(read_value "$key")" != "$expected" ]]; then
        echo "error: async-generator class-method baseline $key drifted" >&2
        exit 1
    fi
}
...
verify_inventory() {
    ...
    if [[ "$(inventory_count "$inventory")" != "$(read_value "${name}_paths")" \
        ...
        echo "error: async-generator class-method $name inventory drifted" >&2
        exit 1
    fi
}
...
verify_quickjs_oracle() {
    ...
    if ! (cd -- "$source_dir" \
        ...
        cat "$quickjs_log" >&2
        echo "error: pinned QuickJS could not execute the class method cohort" >&2
        exit 1
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])' "$quickjs_log" \
        || ! grep -Fq \
            ...
            "$quickjs_log"; then
        cat "$quickjs_log" >&2
        echo "error: pinned QuickJS no longer passes the class method cohort" >&2
        exit 1
    fi
}
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3ah.XXXXXX")
...
excluded_destructuring_scope=$tmp_dir/excluded-destructuring-scope.txt
excluded_private_composition=$tmp_dir/excluded-private-composition.txt
...
expect_value candidate_paths 573
expect_value candidate_direct_paths 396
expect_value candidate_function_to_string_paths 4
expect_value candidate_contextual_paths 1
expect_value candidate_element_composition_paths 160
expect_value candidate_syntax_paths 12
expect_value candidate_variants 1118
expect_value excluded_paths 256
expect_value excluded_yield_star_paths 232
expect_value excluded_for_await_paths 8
expect_value excluded_destructuring_scope_paths 8
expect_value excluded_private_composition_paths 8
expect_value excluded_variants 512
expect_value paths 317
expect_value quickjs_passes 573
expect_value positive_paths 236
expect_value negative_paths 81
expect_value async_paths 216
expect_value sync_paths 101
expect_value double_mode_paths 289
expect_value no_strict_paths 20
expect_value only_strict_paths 8
expect_value variants 606
expect_value sloppy_variants 309
expect_value strict_variants 297
expect_value features 15
expect_value includes 3
expect_value flags 4
expect_value runnable 606
expect_value passes 606
...
if [[ "$(sha256_file "$global_profile")" != "$(read_value global_oxide_profile_sha256)" \
    ...
    echo "error: async-generator class-method pinned profile or exclusions drifted" >&2
    exit 1
fi
...

{
    git -C "$suite" ls-files \
        ...
        'test/language/expressions/class/async-gen-method-static/*.js' \
        'test/language/statements/class/async-gen-method/*.js' \
        'test/language/statements/class/async-gen-method-static/*.js'
    git -C "$suite" ls-files \
        'test/built-ins/Function/prototype/toString/async-generator-method-class-*.js'
    printf '%s\n' test/language/statements/class/async-gen-meth-escaped-async.js
    for class_form in expressions statements; do
        git -C "$suite" ls-files \
            "test/language/$class_form/class/elements/same-line-async-gen-*.js" \
            "test/language/$class_form/class/elements/after-same-line-static-async-gen-*.js"
        for syntax_path in \
            early-errors/grammar-special-meth-contains-super-async-gen.js \
            early-errors/grammar-special-meth-ctor-async-gen.js \
            early-errors/grammar-static-async-gen-meth-prototype.js \
            early-errors/grammar-static-async-gen-meth-super.js \
            valid/grammar-special-prototype-async-gen-meth-valid.js \
            valid/grammar-static-ctor-async-gen-meth-valid.js; do
            git -C "$suite" ls-files \
                "test/language/$class_form/class/elements/syntax/$syntax_path"
        done
    done
...
if [[ "$(inventory_count "$candidate")" != "$(read_value candidate_paths)" \
    ...
    || "$(awk '
        /test\/language\/(expressions|statements)\/class\/async-gen-method(-static)?\// {
            count++
        }
        END { print count + 0 }
    ' "$candidate")" != "$(read_value candidate_direct_paths)" \
    ...
    || "$(awk '/\/class\/async-gen-meth-escaped-async\.js$/ { count++ } END { print count + 0 }' "$candidate")" \
        != "$(read_value candidate_contextual_paths)" \
    || "$(awk '
        /\/class\/elements\/(same-line-async-gen|after-same-line-static-async-gen)-/ {
            count++
        }
        END { print count + 0 }
    ' "$candidate")" != "$(read_value candidate_element_composition_paths)" \
    || "$(awk '/\/class\/elements\/syntax\// { count++ } END { print count + 0 }' "$candidate")" \
        != "$(read_value candidate_syntax_paths)" ]]; then
    echo "error: async-generator class-method candidate universe drifted" >&2
    exit 1
fi
...
if awk -F'\t' \
    ...
    "$exclusions" >&2; then
    echo "error: async-generator class-method exclusions need two populated columns" >&2
    exit 1
fi
...
awk -F'\t' '$1 == "for_await" { print $2 }' "$exclusions" \
    > "$excluded_for_await"
awk -F'\t' '$1 == "destructuring_scope" { print $2 }' "$exclusions" \
    > "$excluded_destructuring_scope"
awk -F'\t' '$1 == "private_composition" { print $2 }' "$exclusions" \
    > "$excluded_private_composition"
...
if [[ "$(inventory_count "$excluded_paths")" != "$(read_value excluded_paths)" \
    ...
    || "$(awk -F'\t' '$1 == "destructuring_scope" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_destructuring_scope_paths)" \
    || "$(awk -F'\t' '$1 == "private_composition" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_private_composition_paths)" \
    || "$(awk -F'\t' '
        $1 != "yield_star" && $1 != "for_await" \
            && $1 != "destructuring_scope" && $1 != "private_composition" {
                count++
            }
        END { print count + 0 }
    ' "$exclusions")" != "0" ]]; then
    echo "error: async-generator class-method exclusion ledger drifted" >&2
    exit 1
fi
verify_inventory excluded_yield_star "$excluded_yield_star"
verify_inventory excluded_for_await "$excluded_for_await"
verify_inventory excluded_destructuring_scope "$excluded_destructuring_scope"
verify_inventory excluded_private_composition "$excluded_private_composition"
if [[ -n "$(comm -23 "$excluded_paths" "$candidate")" ]]; then
    echo "error: async-generator class-method exclusion escaped candidate universe" >&2
    exit 1
fi
...
if [[ -n "$(comm -12 "$manifest" "$excluded_paths")" ]]; then
    echo "error: async-generator class-method manifest and exclusions overlap" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$manifest")" != "$(read_value paths)" \
    ...
    echo "error: async-generator class-method manifest drifted" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$candidate_variant_keys")" != "$(read_value candidate_variants)" \
    ...
    echo "error: async-generator class-method variant partition drifted" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$sloppy_paths")" != "$(read_value sloppy_variants)" \
    ...
    echo "error: async-generator class-method metadata composition drifted" >&2
    exit 1
fi
...
[[ "$(profile_section execution)" == "async=true" ]] \
    || { echo "error: class-method profile must opt into only the async host" >&2; exit 1; }

verify_quickjs_oracle
if "$check_only"; then
    printf 'async-generator class-method inputs verified: %s candidates - %s exclusions = %s paths; QuickJS %s passes all candidates; Oxide gate covers %s variants\n' \
...
if [[ "$(read_header quickjs)" != "$(read_value quickjs)" \
    ...
    echo "error: async-generator class-method report metadata drifted" >&2
    exit 1
fi
...
if [[ "$runner_summary" != "$expected_runner_summary" \
    ...
    echo "error: async-generator class-method all-pass vector drifted" >&2
    awk -F'\t' '
        !/^#/ && !($1 == "path" && $2 == "variant") && $7 != "pass" {
            print $1 "\t" $2 "\t" $7 "\t" $8 "\t" $9 "\t" $10
        }
...
repeat_report=target/test262-async-generator-class-method-core-repeat.tsv
five_report=target/test262-async-generator-class-method-core-workers-5.tsv
...
printf 'async-generator class-method Test262 gate passes: %s/%s variants across %s audited paths; %s/%s/5-worker reports are byte-identical\n' \
