#!/usr/bin/env bash
# Reproduce the dependency-audited R2s object binding Test262 gate.
...
baseline=tests/test262-object-binding-baseline.txt
manifest=tests/test262-object-binding.txt
profile=tests/test262-object-binding.conf
report=target/test262-object-binding.tsv
json_report=target/test262-object-binding.jsonl
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$expected_profile" != "aa6cdca241b5f0be7eb202461ba80e44132f917a66480f1c04225cedc410d0d7" \
    || "$expected_schema" != "test262-canonical-classified-v2" \
    || "$expected_mode" != "both" \
    || "$expected_timeout_ms" != "30000" \
    || "$expected_paths" != "324" \
    || "$expected_variants" != "648" \
    || "$expected_runnable" != "648" \
    || "$expected_passes" != "648" \
    || "$expected_failures" != "0" \
    || "$expected_unsupported" != "0" \
    || "$expected_skipped" != "0" ]]; then
    echo "error: object binding baseline metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_paths" != "$expected_paths" || "$unique_paths" != "$expected_paths" ]]; then
    echo "error: object binding manifest cardinality drifted" >&2
    exit 1
fi
awk 'NF && $1 !~ /^#/ { print }' "$manifest" | LC_ALL=C sort -c
actual_manifest=$(awk 'NF && $1 !~ /^#/ { print }' "$manifest" | sha256_stream)
if [[ "$actual_manifest" != "$expected_manifest" ]]; then
    echo "error: object binding manifest content drifted" >&2
    exit 1
fi
if [[ "$(sha256_file "$manifest")" != "$expected_manifest_file" ]]; then
    echo "error: object binding manifest file drifted" >&2
    exit 1
fi
if [[ "$(sha256_file "$profile")" != "$expected_profile" ]]; then
    echo "error: object binding scoped capability profile drifted" >&2
    exit 1
fi
while IFS= read -r test_path; do
    if [[ ! -f "$suite/$test_path" ]]; then
        echo "error: pinned object binding path is missing: $test_path" >&2
        exit 1
    fi
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: object binding report metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_keys" != "$expected_keys" ]]; then
    echo "error: object binding path/variant key set drifted" >&2
    exit 1
fi
...
if [[ "$actual_passes" != "$expected_passes" \
    ...
    echo "error: object binding Test262 classified vector drifted" >&2
...
printf 'object binding Test262 gate passes: %s/%s variants across %s paths\n' \
