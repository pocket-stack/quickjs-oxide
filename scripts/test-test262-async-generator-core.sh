#!/usr/bin/env bash
# Reproduce the R3af ordinary async-generator core Test262 gate.
...
baseline=tests/test262-async-generator-core-baseline.txt
manifest=tests/test262-async-generator-core.txt
admission_profile=tests/test262-async-generator-core.conf
exclusions=tests/test262-async-generator-core-exclusions.tsv
global_profile=compat/test262-oxide.conf
report=target/test262-async-generator-core.tsv
json_report=target/test262-async-generator-core.jsonl
quickjs_log=target/test262-async-generator-core-quickjs.log
...
expect_value() {
    ...
    if [[ "$(read_value "$key")" != "$expected" ]]; then
        echo "error: async-generator core baseline $key drifted" >&2
        exit 1
    fi
}
...
verify_inventory() {
    ...
    if [[ "$(inventory_count "$inventory")" != "$(read_value "${name}_paths")" \
        ...
        echo "error: async-generator core $name inventory drifted" >&2
        exit 1
    fi
}
...
verify_quickjs_oracle() {
    ...
    if ! (cd -- "$source_dir" \
        ...
        cat "$quickjs_log" >&2
        echo "error: pinned QuickJS could not execute the async-generator core cohort" >&2
        exit 1
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])' "$quickjs_log" \
        || ! grep -Fq \
            ...
            "$quickjs_log"; then
        cat "$quickjs_log" >&2
        echo "error: pinned QuickJS no longer passes the async-generator core cohort" >&2
        exit 1
    fi
}
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3af.XXXXXX")
...
candidate_destructuring=$tmp_dir/candidate-destructuring.txt
excluded_destructuring=$tmp_dir/excluded-destructuring.txt
for_of_source_paths=$tmp_dir/for-of-source.txt
admitted_for_of_source_paths=$tmp_dir/admitted-for-of-source.txt
...
touch "$positive" "$negative" "$async_paths" "$sync_paths" \
    ...
    "$include_occurrences" "$flag_occurrences" "$for_of_source_paths"
...
expect_value candidate_paths 1008
expect_value candidate_async_generator_function_paths 23
expect_value candidate_async_generator_prototype_paths 48
expect_value candidate_async_iterator_prototype_paths 4
expect_value candidate_function_to_string_paths 9
expect_value candidate_expression_paths 623
expect_value candidate_statement_paths 301
expect_value candidate_destructuring_paths 564
expect_value candidate_for_of_source_paths 1
expect_value candidate_variants 1970
expect_value excluded_paths 765
expect_value excluded_destructuring_paths 564
expect_value excluded_yield_star_paths 185
expect_value excluded_for_await_paths 6
expect_value excluded_method_syntax_paths 5
expect_value excluded_proxy_paths 2
expect_value excluded_realm_or_host_paths 3
expect_value excluded_variants 1530
expect_value admitted_async_generator_function_paths 21
expect_value admitted_async_generator_prototype_paths 48
expect_value admitted_async_iterator_prototype_paths 4
expect_value admitted_function_to_string_paths 2
expect_value admitted_expression_paths 122
expect_value admitted_statement_paths 46
expect_value admitted_destructuring_paths 0
expect_value admitted_for_of_source_paths 1
expect_value paths 243
expect_value quickjs_passes 1008
expect_value positive_paths 167
expect_value negative_paths 76
expect_value async_paths 117
expect_value sync_paths 126
expect_value double_mode_paths 197
expect_value no_strict_paths 33
expect_value only_strict_paths 13
expect_value variants 440
expect_value sloppy_variants 230
expect_value strict_variants 210
expect_value features 12
expect_value includes 6
expect_value flags 4
expect_value runnable 440
expect_value passes 440
...
if [[ "$(sha256_file "$global_profile")" != "$(read_value global_oxide_profile_sha256)" \
    ...
    echo "error: async-generator core pinned profile or exclusions drifted" >&2
    exit 1
fi
...
git -C "$suite" ls-files \
    'test/built-ins/AsyncGeneratorFunction/*.js' \
    'test/built-ins/AsyncGeneratorFunction/**/*.js' \
    'test/built-ins/AsyncGeneratorPrototype/*.js' \
    'test/built-ins/AsyncGeneratorPrototype/**/*.js' \
    'test/built-ins/AsyncIteratorPrototype/Symbol.asyncIterator/*.js' \
    'test/built-ins/Function/prototype/toString/*async-generator*.js' \
    'test/language/expressions/async-generator/*.js' \
    'test/language/expressions/async-generator/**/*.js' \
    'test/language/statements/async-generator/*.js' \
    'test/language/statements/async-generator/**/*.js' \
    | LC_ALL=C sort -u > "$candidate"
LC_ALL=C sort -c "$candidate"

# Return through an active destructuring iterator needs the later async
# IteratorClose milestone, so every destructuring path is explicitly excluded.
awk '/\/dstr\// || /destructuring-param-strict-body\.js$/' \
    "$candidate" > "$candidate_destructuring"

# The sole ordinary `for-of` source occurrence is a harness loop outside the
# async-generator body. Pin it exactly so an active for-of return-unwind test
# cannot enter this milestone unnoticed.
while IFS= read -r test_path; do
    if sed '/^\/\*---$/,/^---\*\/$/d' "$suite/$test_path" \
        | grep -Eq 'for[[:space:]]*\([^)]*[[:space:]]of[[:space:]]'; then
        printf '%s\n' "$test_path"
    fi
done < "$candidate" > "$for_of_source_paths"

if [[ "$(inventory_count "$candidate")" != "$(read_value candidate_paths)" \
    || "$(sha256_file "$candidate")" != "$(read_value candidate_sha256)" \
    || "$(awk '/^test\/built-ins\/AsyncGeneratorFunction\// { count++ } END { print count + 0 }' "$candidate")" \
        != "$(read_value candidate_async_generator_function_paths)" \
    || "$(awk '/^test\/built-ins\/AsyncGeneratorPrototype\// { count++ } END { print count + 0 }' "$candidate")" \
        != "$(read_value candidate_async_generator_prototype_paths)" \
    || "$(awk '/^test\/built-ins\/AsyncIteratorPrototype\/Symbol\.asyncIterator\// { count++ } END { print count + 0 }' "$candidate")" \
        != "$(read_value candidate_async_iterator_prototype_paths)" \
    ...
    || "$(awk '/^test\/language\/expressions\/async-generator\// { count++ } END { print count + 0 }' "$candidate")" \
        != "$(read_value candidate_expression_paths)" \
    || "$(awk '/^test\/language\/statements\/async-generator\// { count++ } END { print count + 0 }' "$candidate")" \
        != "$(read_value candidate_statement_paths)" \
    || "$(inventory_count "$candidate_destructuring")" \
        != "$(read_value candidate_destructuring_paths)" \
    || "$(sha256_file "$candidate_destructuring")" \
        != "$(read_value candidate_destructuring_sha256)" \
    || "$(inventory_count "$for_of_source_paths")" \
        != "$(read_value candidate_for_of_source_paths)" \
    || "$(sha256_file "$for_of_source_paths")" \
        != "$(read_value candidate_for_of_source_sha256)" ]]; then
    echo "error: async-generator core candidate universe drifted" >&2
    exit 1
fi
diff -u \
...
if awk -F'\t' \
    ...
    "$exclusions" >&2; then
    echo "error: async-generator core exclusions must have two populated TSV columns" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$excluded_paths")" != "$(read_value excluded_paths)" \
    ...
    || "$(awk -F'\t' '$1 == "destructuring" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_destructuring_paths)" \
    ...
    || "$(awk -F'\t' '$1 == "method_syntax" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_method_syntax_paths)" \
    || "$(awk -F'\t' '$1 == "proxy" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_proxy_paths)" \
    || "$(awk -F'\t' '$1 == "realm_or_host" { count++ } END { print count + 0 }' "$exclusions")" \
        != "$(read_value excluded_realm_or_host_paths)" \
    || "$(awk -F'\t' '
        $1 != "destructuring" && $1 != "yield_star" && $1 != "for_await" \
            && $1 != "method_syntax" && $1 != "proxy" \
            && $1 != "realm_or_host" { count++ }
        END { print count + 0 }
    ' "$exclusions")" != "0" ]]; then
    echo "error: async-generator core exclusion ledger drifted" >&2
    exit 1
fi
if [[ -n "$(comm -23 "$excluded_paths" "$candidate")" ]]; then
    echo "error: async-generator core exclusion escaped the candidate universe" >&2
    exit 1
fi
awk -F'\t' '$1 == "destructuring" { print $2 }' \
    "$exclusions" > "$excluded_destructuring"
diff -u "$candidate_destructuring" "$excluded_destructuring"
...
if [[ -n "$(comm -12 "$manifest" "$excluded_paths")" ]]; then
    echo "error: async-generator core manifest and exclusions overlap" >&2
    exit 1
fi
LC_ALL=C sort -c "$manifest"
comm -12 "$for_of_source_paths" "$manifest" > "$admitted_for_of_source_paths"
if [[ "$(inventory_count "$manifest")" != "$(read_value paths)" \
    ...
    || "$(awk '/^test\/built-ins\/AsyncGeneratorFunction\// { count++ } END { print count + 0 }' "$manifest")" \
        != "$(read_value admitted_async_generator_function_paths)" \
    || "$(awk '/^test\/built-ins\/AsyncGeneratorPrototype\// { count++ } END { print count + 0 }' "$manifest")" \
        != "$(read_value admitted_async_generator_prototype_paths)" \
    || "$(awk '/^test\/built-ins\/AsyncIteratorPrototype\/Symbol\.asyncIterator\// { count++ } END { print count + 0 }' "$manifest")" \
        != "$(read_value admitted_async_iterator_prototype_paths)" \
    || "$(awk '/^test\/built-ins\/Function\/prototype\/toString\// { count++ } END { print count + 0 }' "$manifest")" \
        != "$(read_value admitted_function_to_string_paths)" \
    || "$(awk '/^test\/language\/expressions\/async-generator\// { count++ } END { print count + 0 }' "$manifest")" \
        != "$(read_value admitted_expression_paths)" \
    || "$(awk '/^test\/language\/statements\/async-generator\// { count++ } END { print count + 0 }' "$manifest")" \
        != "$(read_value admitted_statement_paths)" \
    || "$(awk '/\/dstr\// || /destructuring-param-strict-body\.js$/ { count++ } END { print count + 0 }' "$manifest")" \
        != "$(read_value admitted_destructuring_paths)" \
    || "$(inventory_count "$admitted_for_of_source_paths")" \
        != "$(read_value admitted_for_of_source_paths)" \
    || "$(sha256_file "$admitted_for_of_source_paths")" \
        != "$(read_value admitted_for_of_source_sha256)" ]]; then
    echo "error: async-generator core manifest drifted" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$candidate_variant_keys")" != "$(read_value candidate_variants)" \
    ...
    || "$(sha256_file "$excluded_variant_keys")" != "$(read_value excluded_keys_sha256)" ]]; then
    echo "error: async-generator core candidate/exclusion variants drifted" >&2
    exit 1
fi
...
if [[ -n "$(comm -12 "$variant_keys" "$excluded_variant_keys")" ]]; then
    echo "error: async-generator core admitted and excluded variants overlap" >&2
    exit 1
fi
...
if [[ "$(inventory_count "$variant_keys")" != "$(read_value variants)" \
    ...
    echo "error: async-generator core metadata composition drifted" >&2
    exit 1
fi
...
[[ "$(profile_section execution)" == "async=true" ]] \
    || { echo "error: async-generator core profile must opt into only the async host" >&2; exit 1; }

verify_quickjs_oracle
if "$check_only"; then
    printf 'async-generator core inputs verified: %s candidates - %s explicit exclusions = %s paths; QuickJS %s passes all candidates; Oxide gate covers %s variants\n' \
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: async-generator core report metadata drifted" >&2
    exit 1
fi
...
if [[ "$runner_summary" != "$expected_runner_summary" \
    ...
    echo "error: async-generator core all-pass vector drifted" >&2
    awk -F'\t' '
        !/^#/ && !($1 == "path" && $2 == "variant") && $7 != "pass" {
            print $1 "\t" $2 "\t" $7 "\t" $8 "\t" $9 "\t" $10
        }
...
printf 'async-generator core Test262 gate passes: %s/%s variants across %s audited paths\n' \
