#!/usr/bin/env bash
# Freeze pinned QuickJS 2026-06-04 Promise.try, Promise.withResolvers, and
# Promise.race behavior and optionally compare a quickjs-oxide qjs binary.

set -euo pipefail

script_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
root=$(CDPATH= cd -- "$script_dir/.." && pwd)
fixture=$root/tests/fixtures/r3n_promise_static.js
expected=$root/tests/fixtures/r3n_promise_static.quickjs-2026-06-04.txt
fixture_sha256=2bc2a52869d42f314614905f4ac750b87064d6e44cbcfdcb20b3703522bdd0b2
expected_sha256=0da636dbcf08f6d6ec112b439a54ec3d6b0816fff34f1381516a5cad3789f16d
oxide=${OXIDE_QJS:-}
...
verify_hash() {
    ...
    local actual
    if [[ ! -f "$path" ]]; then
        echo "error: R3n oracle input is missing: $path" >&2
        exit 1
    fi
    actual=$(sha256_file "$path")
    if [[ "$actual" != "$pinned" ]]; then
        echo "error: R3n oracle input hash drifted: $path" >&2
...
run_engine() {
    ...
    if ! "$engine" "$@" >"$output" 2>"$errors"; then
        echo "error: $label failed to execute the R3n oracle fixture" >&2
...
compare_transcript() {
    ...
    if ! cmp -s -- "$expected" "$actual"; then
        echo "error: $label R3n transcript drifted" >&2
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3n-oracle.XXXXXX")
trap 'rm -rf -- "$tmp_dir"' EXIT HUP INT TERM

{
    sed '$d' "$fixture"
    printf '\nr3nDone.then(\n'
    printf '    function () { print(r3nTranscript.join("\\n")); },\n'
...
if [[ -n "$oxide" ]]; then
    ...
    echo "R3n Promise static differential passed: quickjs-oxide matches QuickJS 2026-06-04"
else
    echo "R3n Promise static oracle passed: pinned QuickJS 2026-06-04 transcript is stable"
fi
