#!/usr/bin/env bash
# Freeze the QuickJS 2026-06-04 public class-initialization semantics and,
# optionally, compare a quickjs-oxide qjs binary against the same transcript.

set -euo pipefail

script_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
root=$(CDPATH= cd -- "$script_dir/.." && pwd)
fixture=$root/tests/fixtures/r3g_class_public_init.js
expected=$root/tests/fixtures/r3g_class_public_init.quickjs-2026-06-04.txt
fixture_sha256=de89a48a4363fd62d90cc17c7043a6958d5ae572bdc80b602a89a1b17e81a962
expected_sha256=f97cf1523a3d8d7e7c818b260843544d20a741851844328337100f3e6ceea26b
oxide=${OXIDE_QJS:-}
...
verify_hash() {
    ...
    local actual
    if [[ ! -f "$path" ]]; then
        echo "error: R3g oracle input is missing: $path" >&2
        exit 1
    fi
    actual=$(sha256_file "$path")
    if [[ "$actual" != "$pinned" ]]; then
        echo "error: R3g oracle input hash drifted: $path" >&2
...
run_engine() {
    ...
    if ! "$engine" "$@" >"$output" 2>"$errors"; then
        echo "error: $label failed to execute the R3g oracle fixture" >&2
...
compare_transcript() {
    ...
    if ! cmp -s -- "$expected" "$actual"; then
        echo "error: $label R3g transcript drifted" >&2
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3g-oracle.XXXXXX")
trap 'rm -rf -- "$tmp_dir"' EXIT HUP INT TERM

{
    cat "$fixture"
    printf '\nprint(r3gTranscript.join("\\n"));\n'
...
if [[ -n "$oxide" ]]; then
    ...
    echo "R3g class public-init differential passed: quickjs-oxide matches QuickJS 2026-06-04"
else
    echo "R3g class public-init oracle passed: pinned QuickJS 2026-06-04 transcript is stable"
fi
