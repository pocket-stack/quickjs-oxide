#!/usr/bin/env bash
# Reproduce the R3o complete Promise.prototype.finally cohort.
...
baseline=tests/test262-promise-finally-baseline.txt
manifest=tests/test262-promise-finally.txt
admission_profile=tests/test262-promise-finally.conf
global_profile=compat/test262-oxide.conf
report=target/test262-promise-finally.tsv
json_report=target/test262-promise-finally.jsonl
...
verify_inventory() {
    ...
    if [[ "$(printf '%s\n' "$inventory" | wc -l | tr -d '[:space:]')" != "$expected_count" \
        ...
        echo "error: Promise.finally $name inventory drifted" >&2
        exit 1
    fi
}
...
verify_quickjs_oracle() {
    ...
    if ! output=$(cd -- "$source_dir" && ./run-test262 -a -m -c test262.conf -f "${files[@]}" 2>&1); then
        printf '%s\n' "$output" >&2
        echo "error: pinned QuickJS could not execute the Promise.finally cohort" >&2
        exit 1
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])' <<<"$output" \
        || ! grep -Fq "Average memory statistics for $(read_value quickjs_passes) tests:" <<<"$output"; then
        printf '%s\n' "$output" >&2
        echo "error: pinned QuickJS no longer passes the Promise.finally cohort" >&2
        exit 1
    fi
}
...
if [[ "$(read_value quickjs)" != "2026-06-04" \
    ...
    || "$(read_value paths)" != "29" \
    || "$(read_value variants)" != "58" \
    || "$(read_value quickjs_passes)" != "29" \
    || "$(read_value async_variants)" != "24" \
    || "$(read_value sync_variants)" != "34" \
    || "$(read_value proxy_variants)" != "2" ]]; then
    echo "error: Promise.finally baseline identity drifted" >&2
    exit 1
fi

actual_paths=$(manifest_paths | wc -l | tr -d '[:space:]')
unique_paths=$(manifest_paths | LC_ALL=C sort -u | wc -l | tr -d '[:space:]')
[[ "$actual_paths" == "29" && "$unique_paths" == "29" ]] \
    || { echo "error: Promise.finally manifest cardinality drifted" >&2; exit 1; }
...
[[ "$(manifest_paths | sha256_stream)" == "$(read_value manifest_sha256)" \
    ...
    || { echo "error: Promise.finally manifest content drifted" >&2; exit 1; }

derived_manifest=$(git -C "$suite" ls-files \
    'test/built-ins/Promise/prototype/finally/*.js' \
    | awk -F/ 'NF == 6' \
    | LC_ALL=C sort)
...
while IFS= read -r test_path; do
    [[ -f "$suite/$test_path" ]] \
        || { echo "error: missing Promise.finally path: $test_path" >&2; exit 1; }
    metadata=$(metadata_block "$test_path")
    if grep -Fq 'negative:' <<<"$metadata"; then
        echo "error: Promise.finally cohort unexpectedly gained a negative test: $test_path" >&2
        exit 1
    fi
    ...
    case "$flag_line" in
        ...
        "flags: [async]")
            ...
            ;;
        *)
            echo "error: Promise.finally flags drifted: $test_path: $flag_line" >&2
            exit 1
            ;;
    esac
    variant_keys+=$'\n'"$test_path"$'\t'sloppy$'\n'"$test_path"$'\t'strict
    ...
    if grep -Eq 'Promise\.(all|race)[[:space:]]*\(' "$suite/$test_path"; then
        echo "error: Promise.finally cohort unexpectedly gained an all/race dependency: $test_path" >&2
        exit 1
    fi
done < <(manifest_paths)
...
[[ "$(sha256_file "$global_profile")" == "$(read_value global_oxide_profile_sha256)" \
    ...
    || { echo "error: Promise.finally capability profile drifted" >&2; exit 1; }
...
verify_quickjs_oracle
if "$check_only"; then
    printf 'Promise.finally inputs verified: %s complete paths, including %s Proxy adjacency path; QuickJS %s passes all; Oxide gate covers %s variants\n' \
        "$(read_value paths)" \
...
if [[ ${#pending_keys[@]} -ne 0 ]]; then
    printf 'error: Promise.finally Oxide baseline needs refresh before execution: %s\n' \
        "${pending_keys[*]}" >&2
    exit 1
fi
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: Promise.finally report metadata drifted" >&2
    exit 1
fi
...
if [[ "$runner_summary" != "$expected_runner_summary" \
    ...
    echo "error: Promise.finally Test262 classified vector drifted" >&2
    awk -F'\t' '
...
printf 'Promise.finally Test262 gate is exact: %s/%s pass, %s expected failures across %s complete paths\n' \
