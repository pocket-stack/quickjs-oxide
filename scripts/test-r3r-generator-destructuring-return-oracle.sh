#!/usr/bin/env bash
# Freeze QuickJS 2026-06-04 generator-return unwinding through active array
# destructuring iterators and, optionally, compare quickjs-oxide against it.

set -euo pipefail

script_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
root=$(CDPATH= cd -- "$script_dir/.." && pwd)
fixture=$root/tests/fixtures/r3r_generator_destructuring_return.js
expected=$root/tests/fixtures/r3r_generator_destructuring_return.quickjs-2026-06-04.txt
fixture_sha256=05d8e677e984df2a9accb0c56ddb6f2e06ba6d3b2d2d08a51d4ba48811463398
expected_sha256=4e39206df0f8213845227839ad1986759f12566e570a4820265a40e239add715
oxide=${OXIDE_QJS:-}
...
verify_hash() {
    ...
    local actual
    if [[ ! -f "$path" ]]; then
        echo "error: R3r oracle input is missing: $path" >&2
        exit 1
    fi
    actual=$(sha256_file "$path")
    if [[ "$actual" != "$pinned" ]]; then
        echo "error: R3r oracle input hash drifted: $path" >&2
...
run_engine() {
    ...
    if ! "$engine" "$@" >"$output" 2>"$errors"; then
        echo "error: $label failed to execute the R3r oracle fixture" >&2
...
compare_transcript() {
    ...
    if ! cmp -s -- "$expected" "$actual"; then
        echo "error: $label R3r transcript drifted" >&2
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3r-oracle.XXXXXX")
trap 'rm -rf -- "$tmp_dir"' EXIT HUP INT TERM

{
    cat "$fixture"
    printf '\nprint(r3rTranscript.join("\\n"));\n'
...
if [[ -n "$oxide" ]]; then
    ...
    echo "R3r generator-destructuring-return differential passed: quickjs-oxide matches QuickJS 2026-06-04"
else
    echo "R3r generator-destructuring-return oracle passed: pinned QuickJS 2026-06-04 transcript is stable"
fi
