#!/usr/bin/env bash
# Freeze the QuickJS 2026-06-04 Promise reaction and runtime-job semantics and,
# optionally, compare a future Promise-capable quickjs-oxide qjs binary.

set -euo pipefail

script_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
root=$(CDPATH= cd -- "$script_dir/.." && pwd)
fixture=$root/tests/fixtures/r3m_promise_jobs.js
expected=$root/tests/fixtures/r3m_promise_jobs.quickjs-2026-06-04.txt
fixture_sha256=3a9bbd74e97aeae96883a47c801fb5256b8e0625879c0be74aa386642fa1c0b2
expected_sha256=2251b9a554d3645bad15916617cf420ddd054e4117e9fcd7bf6ee692d5ac9e2d
oxide=${OXIDE_QJS:-}
...
usage() {
    ...
    printf '                (requires a host print() and draining the runtime job queue)\n'
}
...
verify_hash() {
    ...
    local actual
    if [[ ! -f "$path" ]]; then
        echo "error: R3m oracle input is missing: $path" >&2
        exit 1
    fi
    actual=$(sha256_file "$path")
    if [[ "$actual" != "$pinned" ]]; then
        echo "error: R3m oracle input hash drifted: $path" >&2
...
run_engine() {
    ...
    if ! "$engine" "$@" >"$output" 2>"$errors"; then
        echo "error: $label failed to execute the R3m oracle fixture" >&2
...
compare_transcript() {
    ...
    if ! cmp -s -- "$expected" "$actual"; then
        echo "error: $label R3m transcript drifted" >&2
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3m-oracle.XXXXXX")
trap 'rm -rf -- "$tmp_dir"' EXIT HUP INT TERM

{
    sed '$d' "$fixture"
    printf '\nr3mDone.then(\n'
    printf '    function () { print(r3mTranscript.join("\\n")); },\n'
...
if [[ -n "$oxide" ]]; then
    ...
    echo "R3m Promise/job differential passed: quickjs-oxide matches QuickJS 2026-06-04"
else
    echo "R3m Promise/job oracle passed: pinned QuickJS 2026-06-04 transcript is stable"
fi
