#!/usr/bin/env bash
# Freeze QuickJS 2026-06-04 private synchronous-generator semantics and,
# optionally, compare a quickjs-oxide qjs binary against the transcript.

set -euo pipefail

script_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
root=$(CDPATH= cd -- "$script_dir/.." && pwd)
fixture=$root/tests/fixtures/r3l_class_private_generators.js
expected=$root/tests/fixtures/r3l_class_private_generators.quickjs-2026-06-04.txt
fixture_sha256=5af87d8181536da15ba5458ab97698e40d5df953955751bb74656a95a5dd382f
expected_sha256=ff79f3ed6798a77b04e1baec6a6e022a46538f0e463707298cee894487c1a2dc
oxide=${OXIDE_QJS:-}
...
verify_hash() {
    ...
    local actual
    if [[ ! -f "$path" ]]; then
        echo "error: R3l oracle input is missing: $path" >&2
        exit 1
    fi
    actual=$(sha256_file "$path")
    if [[ "$actual" != "$pinned" ]]; then
        echo "error: R3l oracle input hash drifted: $path" >&2
...
run_engine() {
    ...
    if ! "$engine" "$@" >"$output" 2>"$errors"; then
        echo "error: $label failed to execute the R3l oracle fixture" >&2
...
compare_transcript() {
    ...
    if ! cmp -s -- "$expected" "$actual"; then
        echo "error: $label R3l transcript drifted" >&2
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3l-oracle.XXXXXX")
trap 'rm -rf -- "$tmp_dir"' EXIT HUP INT TERM

{
    cat "$fixture"
    printf '\nprint(r3lTranscript.join("\\n"));\n'
...
if [[ -n "$oxide" ]]; then
    ...
    echo "R3l class private-generator differential passed: quickjs-oxide matches QuickJS 2026-06-04"
else
    echo "R3l class private-generator oracle passed: pinned QuickJS 2026-06-04 transcript is stable"
fi
