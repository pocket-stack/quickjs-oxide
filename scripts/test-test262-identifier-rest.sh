#!/usr/bin/env bash
# Reproduce the dependency-audited R2x synchronous identifier-rest gate.
...
baseline=tests/test262-identifier-rest-baseline.txt
manifest=tests/test262-identifier-rest.txt
profile=tests/test262-identifier-rest.conf
report=target/test262-identifier-rest.tsv
json_report=target/test262-identifier-rest.jsonl
...
if [[ "$expected_quickjs" != "2026-06-04" \
    ...
    || "$expected_profile" != "da6a76cb6338019f5c233e252bf6d40b7f3eb5c4235a6967cf78f9a74917dced" \
    || "$expected_schema" != "test262-canonical-classified-v2" \
    || "$expected_mode" != "both" \
    || "$expected_timeout_ms" != "30000" \
    || "$expected_paths" != "34" \
    || "$expected_variants" != "65" \
    || "$expected_runnable" != "65" \
    || "$expected_passes" != "65" \
    || "$expected_failures" != "0" \
    || "$expected_unsupported" != "0" \
    || "$expected_skipped" != "0" ]]; then
    echo "error: identifier-rest baseline metadata drifted" >&2
    exit 1
fi
...
if [[ "$actual_paths" != "$expected_paths" || "$unique_paths" != "$expected_paths" ]]; then
    echo "error: identifier-rest manifest cardinality drifted" >&2
    exit 1
fi
...
if [[ "$actual_manifest" != "$expected_manifest" \
    ...
    echo "error: identifier-rest manifest content drifted" >&2
    exit 1
fi
if [[ "$(sha256_file "$profile")" != "$expected_profile" ]]; then
    echo "error: identifier-rest scoped capability profile drifted" >&2
    exit 1
fi
diff -u \
...
for excluded in \
    ...
    test/language/rest-parameters/object-pattern.js \
    test/language/rest-parameters/with-new-target.js; do
    if awk -v excluded="$excluded" '$0 == excluded { found=1 } END { exit !found }' "$manifest"; then
        echo "error: identifier-rest manifest admitted a later frontier: $excluded" >&2
        exit 1
    fi
done
while IFS= read -r test_path; do
    if [[ ! -f "$suite/$test_path" ]]; then
        echo "error: pinned identifier-rest path is missing: $test_path" >&2
        exit 1
    fi
done < <(awk 'NF && $1 !~ /^#/ { print }' "$manifest")
...
if [[ "$(read_header quickjs)" != "$expected_quickjs" \
    ...
    echo "error: identifier-rest report metadata drifted" >&2
    exit 1
fi
...
if [[ "$expected_keys" == PENDING_* || "$expected_tsv" == PENDING_* || "$expected_jsonl" == PENDING_* ]]; then
    echo "error: identifier-rest report hashes await the verified R2x run" >&2
...
if [[ "$actual_passes" != "$expected_passes" \
    ...
    echo "error: identifier-rest Test262 classified vector drifted" >&2
    awk -F'\t' '!/^#/ && !($1 == "path" && $2 == "variant") && $7 != "pass" { print $1 "\t" $2 "\t" $7 "\t" $8 "\t" $9 "\t" $10 }' "$report" >&2
    exit 1
fi

printf 'identifier-rest Test262 gate passes: %s/%s variants across %s paths\n' \
