...
mkdir -p -- "$program_dir/test262" "$program_dir/quickjs"
sed \
    ...
    "$tool_dir/prepare-test262.sh" >"$program_dir/test262/prepare-test262.sh"
chmod +x "$program_dir/test262/prepare-test262.sh"

cat >"$program_dir/quickjs/build-quickjs-oracle.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
EOF
chmod +x "$program_dir/quickjs/build-quickjs-oracle.sh"
...
for unsafe_setting in \
    ...
    assert_failure "unsafe caller Git environment: $unsafe_name" env \
        ...
        QJS_ORACLE_CACHE="$caller_env_cache" "$program_dir/test262/prepare-test262.sh"
done
...
assert_success_path "$base_suite" env PATH="$test_path" \
    ...
    "$program_dir/test262/prepare-test262.sh"
...
assert_success_path "$base_suite" env GIT_PAGER=cat PATH="$test_path" \
    QJS_ORACLE_CACHE="$base_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_success_path "$index_suite" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$index_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_success_path "$default_hook_suite" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$default_hook_cache" "$program_dir/test262/prepare-test262.sh"
...
for unsafe_index_kind in symlink directory fifo lock; do
    ...
    assert_failure "$expected_index_error" env PATH="$test_path" \
        ...
        QJS_ORACLE_CACHE="$unsafe_index_cache" "$program_dir/test262/prepare-test262.sh"
done
...
for unsafe_config_kind in symlink directory fifo lock worktree; do
    ...
    assert_failure "$expected_config_error" env PATH="$test_path" \
        FAKE_BUILD_LOG="$unsafe_control_build_log" \
        QJS_ORACLE_CACHE="$unsafe_config_cache" "$program_dir/test262/prepare-test262.sh"
done
...
for unsafe_exclude_kind in symlink directory fifo lock; do
    ...
    assert_failure "$expected_exclude_error" env PATH="$test_path" \
        FAKE_BUILD_LOG="$unsafe_control_build_log" \
        QJS_ORACLE_CACHE="$unsafe_exclude_cache" "$program_dir/test262/prepare-test262.sh"
done
...
assert_failure "not a regular pinned git checkout" env PATH="$test_path" \
    FAKE_BUILD_LOG="$unsafe_control_build_log" \
    QJS_ORACLE_CACHE="$external_git_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_failure "not a regular pinned git checkout" env PATH="$test_path" \
    FAKE_BUILD_LOG="$unsafe_control_build_log" \
    QJS_ORACLE_CACHE="$non_git_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_failure "paths or bytes differ" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$tamper_cache" "$program_dir/test262/prepare-test262.sh"
...
for unsafe_head_kind in wrong symlink fifo lock; do
    ...
    assert_failure "$expected_head_error" env PATH="$test_path" \
        ...
        QJS_ORACLE_CACHE="$unsafe_head_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_failure "paths or bytes differ" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$missing_cache" "$program_dir/test262/prepare-test262.sh"
extra_cache=$(copy_cache extra-newline)
printf 'extra\n' >"$extra_cache/$source_name/test262/extra
name"
assert_failure "paths or bytes differ" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$extra_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_failure "symlink or special" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$symlink_cache" "$program_dir/test262/prepare-test262.sh"
fifo_cache=$(copy_cache fifo)
mkfifo "$fifo_cache/$source_name/test262/unsafe-fifo"
assert_failure "symlink or special" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$fifo_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_failure "executable mode differs" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$mode_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_failure "config checksum mismatch" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$config_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_failure "replacement refs" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$replace_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_failure "replacement refs" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$replace_symlink_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_failure "unsafe Test262 git metadata" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$alternate_cache" "$program_dir/test262/prepare-test262.sh"
graft_cache=$(copy_cache grafts)
printf '%s\n' "$expected_commit" >"$graft_cache/$source_name/test262/.git/info/grafts"
assert_failure "unsafe Test262 git metadata" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$graft_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_failure "unsafe Test262 git metadata" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$attrs_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_failure "nested .git path" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$nested_cache" "$program_dir/test262/prepare-test262.sh"
...
assert_failure "refusing stale Test262 lock" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$stale_cache" "$program_dir/test262/prepare-test262.sh"
...
for suffix in a b; do
    env PATH="$test_path" FAKE_BUILD_SLEEP=1 FAKE_BUILD_LOG="$concurrent_log" \
        ...
        "$program_dir/test262/prepare-test262.sh" >"$test_root/concurrent.$suffix.out" \
...
mkdir -p -- "$clone_failure_program/test262" "$clone_failure_program/quickjs"
sed "s|^test262_url=.*|test262_url=$test_root/missing-origin.git|" \
    "$program_dir/test262/prepare-test262.sh" >"$clone_failure_program/test262/prepare-test262.sh"
cp -- "$program_dir/quickjs/build-quickjs-oracle.sh" "$clone_failure_program/quickjs/build-quickjs-oracle.sh"
chmod +x "$clone_failure_program/test262/prepare-test262.sh" "$clone_failure_program/quickjs/build-quickjs-oracle.sh"
assert_failure "does not appear to be a git repository" env PATH="$test_path" QJS_ORACLE_CACHE="$clone_failure_cache" \
    "$clone_failure_program/test262/prepare-test262.sh"
...
assert_failure "fake combined build failure" env PATH="$test_path" FAKE_BUILD_FAIL=1 \
    QJS_ORACLE_CACHE="$build_failure_cache" "$program_dir/test262/prepare-test262.sh"
...
(CDPATH='' cd -- "$test_root" && \
    assert_success_path "$relative_cache/$source_name/test262" env PATH="$test_path" \
        QJS_ORACLE_CACHE=relative-cache "$program_dir/test262/prepare-test262.sh")
...
assert_success_path "$poison_source/test262" env PATH="$test_path" \
    QJS_ORACLE_CACHE="$poison_cache" "$program_dir/test262/prepare-test262.sh"
...
done < <(grep -lF './run-test262' "$root"/scripts/{checks,test262,quickjs,unicode,web}/*.sh | LC_ALL=C sort)
[[ $run_file_count -eq 0 && $run_call_count -eq 0 ]] || \
...
done < <(grep -lF "\$source_dir/qjs" "$root"/scripts/{checks,test262,quickjs,unicode,web}/*.sh | LC_ALL=C sort)
[[ $qjs_file_count -eq 0 ]] || fail "source_dir/qjs consumer inventory drifted: $qjs_file_count"

grep -F -- '--test262-oracles' "$root/scripts/quickjs/test-host-gc-reentrant-oracle.sh" >/dev/null || \
...
done < <(grep -lE '\$\{MAKE:-make\}.*run-test262' "$root"/scripts/{checks,test262,quickjs,unicode,web}/*.sh | LC_ALL=C sort)
