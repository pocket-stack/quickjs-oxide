...
check_harness() {
    ...
    [[ "$test_count" == 912 ]] \
        || die "oracle test count drifted: expected 912, found $test_count"
    host_test_count=0
...
EOF
    ...
    [[ "$default_test_count" == 907 ]] \
        || die "default oracle test count drifted: expected 907, found $default_test_count"
...
check_compiled_harness() {
    ...
    [[ "$default_count" == 907 ]] \
        || die "compiled default oracle list drifted: expected 907, found $default_count"
    ...
    [[ "$host_count" == 912 ]] \
        || die "compiled host oracle list drifted: expected 912, found $host_count"
