...
check_harness() {
    ...
    [[ "$test_count" == 909 ]] \
        || die "oracle test count drifted: expected 909, found $test_count"
    host_test_count=0
...
EOF
    ...
    [[ "$default_test_count" == 904 ]] \
        || die "default oracle test count drifted: expected 904, found $default_test_count"
...
check_compiled_harness() {
    ...
    [[ "$default_count" == 904 ]] \
        || die "compiled default oracle list drifted: expected 904, found $default_count"
    ...
    [[ "$host_count" == 909 ]] \
        || die "compiled host oracle list drifted: expected 909, found $host_count"
