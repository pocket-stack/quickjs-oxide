#!/usr/bin/env bash
# Reproduce the complete DataView capability admission and its
...
baseline=tests/test262-data-view-global-baseline.txt
canonical_baseline=tests/test262-full-baseline.txt
parent_profile=tests/test262-data-view-global-parent.conf
candidate_profile=tests/test262-data-view-global-candidate.conf
live_profile=compat/test262-oxide.conf
universe_manifest=tests/test262-data-view-universe.txt
activation_manifest=tests/test262-data-view-global-activation.txt
reason_only_manifest=tests/test262-data-view-global-reason-only.txt
config_skipped_manifest=tests/test262-data-view-global-config-skipped.txt
focused_manifest=tests/test262-data-view.txt
supplemental_manifest=tests/test262-data-view-global-supplemental.txt
supplemental_oracle_log=target/test262-data-view-global-supplemental-quickjs.log
transition_receipt=tests/test262-data-view-global-transitions.tsv
parent_report=target/test262-data-view-global-parent.tsv
parent_json_report=target/test262-data-view-global-parent.jsonl
candidate_report=target/test262-data-view-global-candidate.tsv
candidate_json_report=target/test262-data-view-global-candidate.jsonl
parent_full_report=target/test262-data-view-global-parent-full.tsv
parent_full_json_report=target/test262-data-view-global-parent-full.jsonl
candidate_full_report=target/test262-data-view-global-candidate-full.tsv
candidate_full_json_report=target/test262-data-view-global-candidate-full.jsonl
...
usage() {
    ...
    printf '  --bless       bless the 380-row tag transition receipt\n'
...
expect_value() {
    ...
    [[ "$actual" == "$expected" ]] || {
        printf 'error: R3cb baseline identity drifted for %s: %s != %s\n' \
...
json_report_keys() {
    ...
    awk -v report="$report" '
        function fail(message) {
            printf "error: R3cb JSONL report %s: %s\n", report, message >"/dev/stderr"
            failed=1
            exit 2
        }
        /^\{"kind":"metadata",/ {
...
json_result_projection() {
    ...
    awk -v report="$report" '
        function fail(message) {
            printf "error: R3cb JSONL projection %s: %s\n", report, message >"/dev/stderr"
            failed=1
            exit 2
        }
...
verify_report() {
    ...
    [[ -f "$json_report" \
        ...
        && "$(report_rows "$report" | wc -l | tr -d '[:space:]')" \
            == "$expected_rows" ]] || {
        printf 'error: R3cb report metadata drifted: %s\n' "$report" >&2
        exit 1
    }
    ...
    [[ "$(head -n 1 "$json_report")" \
        ...
        printf 'error: R3cb JSONL metadata drifted: %s\n' "$json_report" >&2
        exit 1
    }
}
...
verify_partition_transition() {
    ...
    ' <(printf '%s\n' "$before_rows") \
        <(printf '%s\n' "$candidate_rows"); then
        printf 'error: R3cb %s %s transition drifted\n' \
            "$encoding" "$kind" >&2
        exit 1
    fi
}
...
verify_manifest() {
    ...
    [[ "$count" == "$(read_value "${prefix}_paths")" \
        ...
        printf 'error: R3cb %s manifest drifted\n' "$prefix" >&2
        exit 1
    }
}
...
verify_supplemental_quickjs() {
    local runner=$source_dir/run-test262 test_path
    local -a files=()
    [[ -x "$runner" ]] || "${MAKE:-make}" -C "$source_dir" run-test262 >&2
    while IFS= read -r test_path; do
        files+=("test262/$test_path")
    done < <(manifest_paths "$supplemental_manifest")

    if ! (
        cd -- "$source_dir"
        ./run-test262 -m -c test262.conf -a -T "$workers" -f "${files[@]}"
    ) >"$root/$supplemental_oracle_log" 2>&1; then
        tail -n 100 "$supplemental_oracle_log" >&2
        echo 'error: pinned QuickJS could not execute the DataView supplemental cohort' >&2
        exit 1
    fi
    if grep -Eq '(^|[[:space:]])FAILED($|[[:space:]])|SKIPPED FEATURE' \
            "$supplemental_oracle_log" \
        || ! grep -Fq \
            "Average memory statistics for $(read_value supplemental_variants) tests:" \
            "$supplemental_oracle_log"; then
        tail -n 100 "$supplemental_oracle_log" >&2
        echo 'error: pinned QuickJS no longer passes the DataView supplemental cohort' >&2
        exit 1
    fi
}
...
expect_value parent_oxide_profile_sha256 63f139b1a74da9a6114180593770dbcc86bb84fbafab5731f59e1387175c5a6a
expect_value candidate_oxide_profile_sha256 b51eee39825e3325effab1c326df30b999e636f67c8ce7bb800f0afdc2d8eab4
expect_value parent_features 93
expect_value candidate_features 94
expect_value added_features 1
expect_value audited_negative_tests 1154
expect_value universe_paths 190
expect_value universe_variants 380
expect_value activation_paths 98
expect_value activation_variants 196
expect_value reason_only_paths 79
expect_value reason_only_variants 158
expect_value config_skipped_paths 13
expect_value config_skipped_variants 26
expect_value global_transition_receipt tests/test262-data-view-global-transitions.tsv
expect_value global_transition_rows 380
expect_value global_transition_activation_rows 196
expect_value global_transition_reason_only_rows 158
expect_value global_transition_unchanged_config_rows 26
expect_value global_expected_parent_tag_runnable 0
expect_value global_expected_parent_tag_summary 'skipped-feature=26 unsupported-feature=354'
expect_value global_expected_candidate_tag_runnable 196
expect_value global_expected_candidate_tag_summary 'pass=196 skipped-feature=26 unsupported-feature=158'
expect_value global_full_variants 102037
expect_value global_full_universe_rows 380
expect_value global_full_activation_rows 196
expect_value global_full_reason_only_rows 158
expect_value global_full_config_skipped_rows 26
expect_value global_full_non_universe_rows 101657
expect_value global_full_changed_rows 354
expect_value global_full_outcome_changed_rows 196
expect_value global_full_detail_only_rows 158
expect_value global_full_unchanged_rows 101683
expect_value global_previous_pass_regressions 0
expect_value global_parent_full_runnable 63592
expect_value global_parent_full_passes 63073
expect_value global_parent_full_unsupported_feature 15074
expect_value parent_full_tsv_sha256 2db7d8772074f90de6525cd51ffcd43ea3bf906d78e7c938d452cd6cac21a216
expect_value parent_full_jsonl_sha256 5c201991551f3bb3f03f5a5b232cff0b2470969ae440bc942c324ba4fc5d57a3
expect_value global_expected_candidate_full_runnable 63788
expect_value global_expected_candidate_full_passes 63269
expect_value global_expected_candidate_full_unsupported_feature 14878
...
for required in \
    ...
    "$focused_manifest" "$supplemental_manifest"
do
    [[ -f "$required" ]] || {
        printf 'error: missing R3cb asset: %s\n' "$required" >&2
        exit 1
    }
done
...
[[ "$(sha256_file "$parent_profile")" \
    ...
    echo "error: R3cb frozen profile identity drifted" >&2
    exit 1
}
diff -u <(printf '%s\n' "$parent_negatives") \
    <(printf '%s\n' "$candidate_negatives")
...
[[ -z "$(comm -23 <(printf '%s\n' "$parent_features") \
    <(printf '%s\n' "$candidate_features"))" ]] || {
    echo "error: R3cb candidate removed a parent feature" >&2
    exit 1
}
[[ -z "$(comm -23 <(printf '%s\n' "$candidate_features") \
    <(printf '%s\n' "$live_features"))" ]] || {
    echo "error: live profile removed an R3cb candidate feature" >&2
    exit 1
}
...
[[ -z "$(comm -23 <(printf '%s\n' "$candidate_negatives") \
    <(printf '%s\n' "$live_negatives"))" ]] || {
    echo "error: live profile removed an R3cb negative audit" >&2
    exit 1
}
diff -u <(printf '%s\n' "$candidate_execution") \
    <(printf '%s\n' "$live_execution")
...
verify_live_profile_stable() {
    ...
    [[ "$current_profile" == "$live_profile_sha256" \
        ...
        echo 'error: live R3cb profile changed while the gate was running' >&2
        exit 1
    }
}
...
if [[ "$live_profile_sha256" != "$(read_value candidate_oxide_profile_sha256)" \
    ...
    echo 'error: live profile differs without advancing R3cb capabilities' >&2
    exit 1
fi

# Derive the complete DataView tag universe from the pinned metadata instead
# of trusting a self-consistent checked-in manifest and receipt.
metadata_tmp=$(mktemp target/test262-data-view-global-metadata.XXXXXX)
metadata_tsv_tmp=$(mktemp target/test262-data-view-global-metadata-tsv.XXXXXX)
derived_paths_tmp=$(mktemp target/test262-data-view-global-paths.XXXXXX)
derived_keys_tmp=$(mktemp target/test262-data-view-global-keys.XXXXXX)
...
[[ "$(wc -l <"$metadata_tmp" | tr -d '[:space:]')" \
    ...
    && "$(sha256_file "$source_dir/test262.conf")" \
        == "$(read_value test262_config_sha256)" ]] || {
    echo 'error: R3cb pinned metadata or QuickJS config drifted' >&2
    exit 1
}
awk -F'\t' -v paths="$derived_paths_tmp" -v keys="$derived_keys_tmp" '
    ...
    !has($4, "DataView") { next }
    {
        if (has($3, "module") || has($3, "raw") ||
            has($3, "onlyStrict") || has($3, "noStrict") ||
            $5 != "" || $6 != "") bad=1
        print $1 >paths
        print $1 "\tsloppy" >keys
        print $1 "\tstrict" >keys
    ...
    END { if (bad) exit 1 }
' "$metadata_tsv_tmp" || {
    echo 'error: R3cb DataView metadata contract drifted' >&2
    exit 1
}
for sorted_inventory in "$derived_paths_tmp" "$derived_keys_tmp"; do
    LC_ALL=C sort -c "$sorted_inventory"
    [[ -z "$(uniq -d "$sorted_inventory")" ]] || {
        printf 'error: duplicate R3cb metadata inventory entry: %s\n' \
            "$sorted_inventory" >&2
        exit 1
    }
done
...
[[ "$(wc -l <"$derived_paths_tmp" | tr -d '[:space:]')" \
    ...
    echo 'error: R3cb derived DataView universe drifted' >&2
    exit 1
}
...
[[ "$(printf '%s\n' "$partition_paths" | uniq -d | wc -l \
        | tr -d '[:space:]')" == 0 ]] || {
    echo "error: R3cb path partitions overlap" >&2
    exit 1
}
diff -u <(printf '%s\n' "$universe_paths") \
    <(printf '%s\n' "$partition_paths")

focused_activation_paths=$(comm -12 \
    <(printf '%s\n' "$activation_paths") \
    <(manifest_paths "$focused_manifest" | LC_ALL=C sort -u))
[[ "$(printf '%s\n' "$focused_activation_paths" | wc -l \
        | tr -d '[:space:]')" == "$(read_value focused_activation_paths)" \
    && "$(printf '%s\n' "$focused_activation_paths" | sha256_stream)" \
        == "$(read_value focused_activation_sha256)" ]] || {
    echo 'error: R3cb focused DataView activation intersection drifted' >&2
    exit 1
}
diff -u <(printf '%s\n' "$activation_paths") \
    <(printf '%s\n%s\n' "$focused_activation_paths" \
        "$(manifest_paths "$supplemental_manifest")" | LC_ALL=C sort)

if [[ "$mode" == check ]]; then
    "$script_dir/test-test262-data-view.sh" --check
    verify_supplemental_quickjs
    verify_live_profile_stable
    printf 'R3cb inputs verified: %s paths = %s activation + %s reason-only + %s config-skip\n' \
...
TEST262_WORKERS="$workers" \
    "$script_dir/test-test262-data-view.sh"
verify_supplemental_quickjs
...
if [[ "$tag_pending_count" != 0 \
    ...
    echo "error: R3cb tag receipt is only partially PENDING" >&2
    exit 1
fi
if [[ "$tag_pending_count" != 0 && "$mode" != bless ]]; then
    printf 'error: R3cb tag baseline needs --bless: %s\n' \
...
[[ "$parent_runnable" == "$(read_value global_expected_parent_tag_runnable)" \
    ...
    echo "error: R3cb parent tag vector drifted" >&2
    exit 1
}
...
[[ "$candidate_runnable" \
    ...
    echo "error: R3cb candidate tag vector drifted" >&2
    exit 1
}
...
{
    printf '# R3cb exhaustive DataView global admission transition.\n'
...
[[ "$transition_counts" == \
    ...
    echo "error: R3cb transition receipt partition drifted" >&2
    exit 1
}
...
if [[ "$mode" == bless ]]; then
    ...
    verify_live_profile_stable
    printf 'R3cb tag baseline blessed: %s activation, %s reason-only, %s unchanged config variants\n' \
...
[[ -f "$transition_receipt" \
    ...
    echo "error: R3cb tag or transition receipt drifted" >&2
    exit 1
}
cmp -s "$transition_tmp" "$transition_receipt" || {
    echo "error: R3cb checked-in transition receipt drifted" >&2
    exit 1
}
rm -f -- "$transition_tmp"
transition_tmp=
...
if [[ "$mode" == tag ]]; then
    verify_live_profile_stable
    printf 'R3cb tag gate is exact: %s outcome changes, %s detail-only changes, %s unchanged skips\n' \
        "$(read_value global_transition_activation_rows)" \
        "$(read_value global_transition_reason_only_rows)" \
        "$(read_value global_transition_unchanged_config_rows)"
    exit 0
...
if [[ "$full_pending_count" != 0 \
    ...
    echo "error: R3cb full receipt is only partially PENDING" >&2
    exit 1
fi
if [[ "$full_pending_count" != 0 && "$mode" != bless-full ]]; then
    printf 'error: R3cb full baseline needs --bless-full: %s\n' \
...
if [[ "$mode" == bless-full ]]; then
    cmp -s "$candidate_profile" "$live_profile" || {
        echo "error: R3cb can bless full receipts only while its candidate is live" >&2
        exit 1
    }
...
[[ "$(read_value_from "$canonical_baseline" schema)" == "$(read_value schema)" \
    ...
    echo 'error: canonical full baseline metadata is not the R3cb contract' >&2
    exit 1
}
if [[ "$expected_canonical_state" != descendant ]]; then
    ...
    printf 'error: canonical full baseline is not the expected R3cb %s vector\n' \
...
[[ "$(printf '%s\n' "$parent_full_keys" | sha256_stream)" \
    ...
    echo "error: R3cb authoritative parent full vector drifted" >&2
    exit 1
}
...
[[ "$candidate_full_runnable" \
    ...
    echo "error: R3cb candidate full vector drifted" >&2
    exit 1
}
...
[[ "$(printf '%s\n' "$parent_full_universe" | wc -l | tr -d '[:space:]')" \
    ...
    && "$(printf '%s\n' "$candidate_non_universe" | wc -l \
        | tr -d '[:space:]')" == "$(read_value global_full_non_universe_rows)" ]] || {
    echo "error: R3cb full universe partition drifted" >&2
    exit 1
}
...
' "$parent_full_report" "$candidate_full_report") || {
    echo "error: R3cb complete keyed join failed" >&2
    exit 1
}
...
[[ "$full_rows" == "$(read_value global_full_variants)" \
    ...
    echo "error: R3cb full keyed join count drifted" >&2
    exit 1
}
...
if [[ "$mode" == bless-full ]]; then
    ...
    verify_live_profile_stable
    printf 'R3cb full transition blessed: %s outcome + %s detail-only changes, %s rows unchanged, zero regressions\n' \
...
do
    ...
    actual=${entry#*:}
    [[ "$actual" == "$(read_value "$key")" ]] || {
        printf 'error: R3cb full receipt drifted: %s\n' "$key" >&2
        exit 1
    }
done

verify_live_profile_stable
printf 'R3cb full transition is exact: %s outcome + %s detail-only changes, %s rows unchanged, zero regressions\n' \
    "$outcome_changed_rows" "$detail_only_rows" "$unchanged_rows"
