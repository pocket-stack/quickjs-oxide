#!/usr/bin/env bash
# Freeze QuickJS 2026-06-04 RegExp.escape and Annex B control-escape
# semantics and, optionally, compare quickjs-oxide against them.

set -euo pipefail

script_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
root=$(CDPATH= cd -- "$script_dir/.." && pwd)
fixture=$root/tests/fixtures/r3s_regexp_escape_control.js
expected=$root/tests/fixtures/r3s_regexp_escape_control.quickjs-2026-06-04.txt
fixture_sha256=babb9f0e94a7f4e3cf62ad25faf923dc86adb9248db36f081b4b2e7667c6f784
expected_sha256=c6226637ca00cfcef2c436cb64442d8264ba18553aba31baffe70a34d48f480f
oxide=${OXIDE_QJS:-}
...
verify_hash() {
    ...
    local actual
    if [[ ! -f "$path" ]]; then
        echo "error: R3s oracle input is missing: $path" >&2
        exit 1
    fi
    actual=$(sha256_file "$path")
    if [[ "$actual" != "$pinned" ]]; then
        echo "error: R3s oracle input hash drifted: $path" >&2
...
run_engine() {
    ...
    if ! "$engine" "$@" >"$output" 2>"$errors"; then
        echo "error: $label failed to execute the R3s oracle fixture" >&2
...
compare_transcript() {
    ...
    if ! cmp -s -- "$expected" "$actual"; then
        echo "error: $label R3s transcript drifted" >&2
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/quickjs-oxide-r3s-oracle.XXXXXX")
trap 'rm -rf -- "$tmp_dir"' EXIT HUP INT TERM

{
    cat "$fixture"
    printf '\nprint(r3sTranscript.join("\\n"));\n'
...
if [[ -n "$oxide" ]]; then
    ...
    echo "R3s RegExp.escape/control-escape differential passed: quickjs-oxide matches QuickJS 2026-06-04"
else
    echo "R3s RegExp.escape/control-escape oracle passed: pinned QuickJS 2026-06-04 transcript is stable"
fi
