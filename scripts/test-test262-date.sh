#!/usr/bin/env bash
# Reproduce the complete classified outcome vector for the Date milestone.
...
baseline=tests/test262-date-baseline.txt
report=target/test262-date.tsv
json_report=target/test262-date.jsonl
...
run_output=$(cargo run --locked --release --quiet --bin run-test262 -- \
    ...
    --manifest tests/test262-date.txt \
    --report "$report" \
    --mode both \
    --workers "$workers" \
...
if [[ "$actual_schema" != "$expected_schema" \
    ...
    echo "error: Date Test262 baseline metadata drifted" >&2
...
if [[ "$actual_summary" != "# summary $expected_summary" ]]; then
    echo "error: Date Test262 classified summary drifted" >&2
...
if [[ "$expected_passes" == 0 ]]; then
    if [[ " $expected_summary " == *" pass="* ]]; then
        echo "error: zero-pass Date baseline unexpectedly records a pass outcome" >&2
        exit 1
    fi
elif [[ " $expected_summary " != *" pass=$expected_passes "* ]]; then
    echo "error: Date pass count is inconsistent with the pinned summary" >&2
    exit 1
fi
...
if [[ "$actual_tsv" != "$expected_tsv" || "$actual_jsonl" != "$expected_jsonl" ]]; then
    echo "error: Date Test262 classified vector drifted" >&2
...
printf 'Date Test262 vector matches: %s pass of %s variants\n' \
