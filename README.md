...
The `unsafe`-free engine is runnable but incomplete. The current R3ag milestone
adds ordinary object-literal async-generator methods by composing QuickJS's
Method grammar and existing async-generator driver. Pinned QuickJS passes all
113 candidate paths; 67 explicit frontier exclusions leave 46 paths and 82/82
passing Oxide variants. Class/private async-generator methods, `yield*`, `for
await`, async iterator closing, modules, Proxy, and broad built-in coverage
remain incomplete. The conservative full vector gains four passes to
43,680/102,037 with no previous-pass regression. Pinned QuickJS is the test
oracle, never a product dependency. See the status documents for detailed
bookkeeping.
